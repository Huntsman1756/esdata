"""Routers para Ley 11/2009 de SOCIMI.

Endpoints de consulta de la normativa SOCIMI (Sociedades de Inversion
en el Mercado Inmobiliario) y sus articulos.
"""
...
router = APIRouter(prefix="/v1/socimi", tags=["socimi"])


@router.get("", operation_id="list_socimi", response_model=NormasListResponse)
async def list_socimi():
    """Listar normas SOCIMI."""
    with db_session() as db:
        rows = db.execute(
            text(
                ...
                FROM norma
                WHERE regulacion_relacionada = 'socimi' OR codigo = 'SOCIMI'
                ORDER BY codigo
                """
            )
        ).mappings()
...
@router.get(
    "/micro-obligaciones",
    operation_id="list_socimi_micro_obligaciones",
)
async def list_socimi_micro_obligaciones():
    """Listar micro-obligaciones SOCIMI."""
    with db_session() as db:
        rows = db.execute(
            text(
                """
                ...
                WHERE regulacion_relacionada = 'socimi'
                ORDER BY codigo ASC
                """
            )
        ).mappings()
...
@router.get("/{codigo}", operation_id="get_socimi_norma", response_model=NormaSchema)
async def get_socimi_norma(codigo: str):
    """Obtener detalle de una norma SOCIMI."""
    with db_session() as db:
        row = (
            db.execute(
                text(
        ...
        if not row:
            raise HTTPException(status_code=404, detail={"error": "Norma SOCIMI no encontrada"})
        return dict(row)


@router.get("/{codigo}/articulos", operation_id="list_socimi_articulos", response_model=ArticulosListResponse,
            summary="Lista articulos de una norma SOCIMI")
async def list_socimi_articulos(codigo: str, tipo: str | None = Query(None, description="Filtrar por tipo de articulo")):
    """Listar articulos de una norma SOCIMI."""
    with db_session() as db:
        ...
        if not rows:
            ...
            if not existe_norma:
                raise HTTPException(
                    status_code=404, detail={"error": "Norma SOCIMI no encontrada"}
                )
...
@router.get(
    ...
    operation_id="get_socimi_articulo",
    response_model=ArticuloDetailSchema,
    summary="Detalle de un articulo SOCIMI",
)
async def get_socimi_articulo(codigo: str, numero: str, vigente_en: str | None = Query(None, description="Fecha de vigencia (YYYY-MM-DD)")):
    """Obtener detalle de un articulo SOCIMI."""
    with db_session() as db:
        ...
        if not row:
            raise HTTPException(status_code=404, detail={"error": "Articulo SOCIMI no encontrado"})
        return {
            "norma": row["codigo"],
            "numero": row["numero"],
            "texto": row["texto"],
...
@router.get(
    ...
    operation_id="get_socimi_articulo_historial",
)
async def get_socimi_articulo_historial(codigo: str, numero: str):
    """Historial de versiones de un articulo SOCIMI."""
    with db_session() as db:
        rows = db.execute(
            text(
                """

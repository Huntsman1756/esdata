...
def test_aeat_dirty_detection_ignores_plain_src_class_text():
    ...
    assert (
        is_dirty_aeat_text('<span class="nav"><script src="/static_files/common/script/aeat.07.js"></script></span>')
        is True
    )
...
async def test_cc_articulo_detail_with_vigente_en_after_legalize_seed(integration_db):
    ...
    with integration_db.begin() as conn:
        conn.execute(
            text(
                "DELETE FROM version_articulo WHERE articulo_id IN (SELECT id FROM articulo WHERE norma_id IN (SELECT id FROM norma WHERE codigo = 'CC'))"
            )
        )
...
async def test_lec_articulo_detail_with_vigente_en_after_legalize_seed(integration_db):
    ...
    with integration_db.begin() as conn:
        conn.execute(
            text(
                "DELETE FROM version_articulo WHERE articulo_id IN (SELECT id FROM articulo WHERE norma_id IN (SELECT id FROM norma WHERE codigo = 'LEC'))"
            )
        )
...
async def test_et_articulo_detail_with_vigente_en_after_legalize_seed(integration_db):
    ...
    with integration_db.begin() as conn:
        conn.execute(
            text(
                "DELETE FROM version_articulo WHERE articulo_id IN (SELECT id FROM articulo WHERE norma_id IN (SELECT id FROM norma WHERE codigo = 'ET'))"
            )
        )
...
async def test_lsc_articulo_detail_with_vigente_en_after_legalize_seed(integration_db):
    ...
    with integration_db.begin() as conn:
        conn.execute(
            text(
                "DELETE FROM version_articulo WHERE articulo_id IN (SELECT id FROM articulo WHERE norma_id IN (SELECT id FROM norma WHERE codigo = 'LSC'))"
            )
        )
...
async def test_lc_articulo_detail_with_vigente_en_after_legalize_seed(integration_db):
    ...
    with integration_db.begin() as conn:
        conn.execute(
            text(
                "DELETE FROM version_articulo WHERE articulo_id IN (SELECT id FROM articulo WHERE norma_id IN (SELECT id FROM norma WHERE codigo = 'LC'))"
            )
        )
...
async def test_lirpf_articulo_detail_with_vigente_en_after_legalize_seed(integration_db):
    ...
    with integration_db.begin() as conn:
        conn.execute(
            text(
                "DELETE FROM version_articulo WHERE articulo_id IN (SELECT id FROM articulo WHERE norma_id IN (SELECT id FROM norma WHERE codigo = 'LIRPF'))"
            )
        )

...
@pytest.mark.asyncio
async def test_status_keeps_cdi_weekly_cron_healthy_with_three_day_lag():
    app, _ = _get_app_and_engine()
    _seed_sync_log(
        "cron-cdi-weekly",
        finished_at=datetime.now(UTC) - timedelta(hours=72),
    ...
    assert response.status_code == 200
    worker = response.json()["workers"]["cron-cdi-weekly"]
    assert worker["status"] == "ok"
    assert worker["stale"] is False


@pytest.mark.asyncio
async def test_status_keeps_psd2_weekly_cron_healthy_with_three_day_lag():
    app, _ = _get_app_and_engine()
    _seed_sync_log(
        "cron-psd2-weekly",
        finished_at=datetime.now(UTC) - timedelta(hours=72),
    ...
    assert response.status_code == 200
    worker = response.json()["workers"]["cron-psd2-weekly"]
    assert worker["status"] == "ok"
    assert worker["stale"] is False


@pytest.mark.asyncio
async def test_status_keeps_official_regulatory_references_healthy_with_three_day_lag():
    app, _ = _get_app_and_engine()
    _seed_sync_log(
    ...
    assert response.status_code == 200
    worker = response.json()["workers"]["official-regulatory-references"]
    assert worker["status"] == "ok"
    assert worker["stale"] is False


@pytest.mark.asyncio
async def test_status_keeps_monthly_pgc_cron_healthy_with_twenty_day_lag():
    app, _ = _get_app_and_engine()
    _seed_sync_log(
    ...
    assert response.status_code == 200
    worker = response.json()["workers"]["cron-pgc-boe-monthly"]
    assert worker["status"] == "ok"
    assert worker["stale"] is False


@pytest.mark.asyncio
async def test_status_keeps_source_expansion_weekly_crons_healthy_with_three_day_lag():
    app, _ = _get_app_and_engine()
    for worker in ("cron-ofac-sdn-weekly", "cron-mica-weekly"):
        _seed_sync_log(
            worker,
            finished_at=datetime.now(UTC) - timedelta(hours=72),
            status="ok",
    ...
    assert workers["cron-ofac-sdn-weekly"]["stale"] is False
    assert workers["cron-mica-weekly"]["stale"] is False


@pytest.mark.asyncio
async def test_status_keeps_giin_monthly_cron_healthy_with_twenty_day_lag():
    app, _ = _get_app_and_engine()
    _seed_sync_log(
        "cron-giin-monthly",
        finished_at=datetime.now(UTC) - timedelta(days=20),
    ...
    assert response.status_code == 200
    worker = response.json()["workers"]["cron-giin-monthly"]
    assert worker["status"] == "ok"
    assert worker["stale"] is False

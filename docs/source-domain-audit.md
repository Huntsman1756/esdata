...
- VPS tras `S-09`: commit `667252a` desplegado; `alembic upgrade head` aplicado; run acotado EUR-Lex MiFID II registra `articles_expected=93`, `articles_parsed=92`, `quality_status=partial`; API devuelve esos counters junto a `coverage_status=article_text_available`; `mcp_validation_suite.py` y `mcp_deep_contract_audit.py` -> `ok=true`.

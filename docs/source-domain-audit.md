...
| EUR-Lex | `norma`, `articulo`, `version_articulo` con `tipo_fuente='eurlex'` | `norma=32`, `articulo=0`, `version_articulo=0`; worker informa `fetch_articles=False`, `discovery_enabled=False` | `worker-eurlex` y `cron-eurlex-weekly` OK con `rows_processed=0` y sin errores | API/MCP `listar_eurlex/get_eurlex` responde con `coverage_status`, `verified`, `completeness`, `articulos_total`, `evidence_notice` | Parcial-controlado: metadata oficial disponible; articulado profundo no cargado y debe responder `evidence_limited` |

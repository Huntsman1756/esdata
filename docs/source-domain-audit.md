...
| BOE diario no consolidado | `documento_interpretativo`, `source_revision` | S-10 implementa ingesta desde `diario_boe/xml.php?id=<BOE-ID>`; despliegue debe verificar `BOE-B/S/N` recientes | `cron-boe-diario-daily` nuevo, diario 06:30 Europe/Madrid, discovery por sumario oficial o `BOE_DIARIO_IDS` | API/MCP `listar_boe_diario/get_boe_diario`; XML oficial con texto => `complete/official_exact`; PDF fallback => `partial/official_best_effort` | Implementado pendiente de evidencia productiva post-despliegue |

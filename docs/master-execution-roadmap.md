...
- 2026-05-13 11:27 Europe/Madrid - `[EN CURSO LOCAL / BLOQUEADO VPS]` D-01 AEAT Modelo 296: el parser local extrae 114 campos del PDF oficial `DR_296_2024.pdf`; se prepara `aeat_current_designs.py` para reemplazar filas `diseno_registro_*` existentes solo en el modelo 296, evitando que datos semilla/parciales bloqueen la recarga oficial. Verificacion local: `pytest apps/workers/tests/test_aeat_current_designs.py -q` => `14 passed`; `compileall` OK; probe real del PDF AEAT devuelve `114` campos. Bloqueo: SSH `deploy@212.227.227.64` deniega `publickey`, y Compose local no puede consultar DB porque `DATABASE_URL` no esta configurado; no se puede ejecutar aun el worker ni confirmar conteo en `modelo_casilla` productivo. Siguiente paso exacto: recuperar acceso SSH al VPS, desplegar commit D-01, ejecutar `worker-aeat-current-designs`, comprobar `296 casillas > 10`, y solo entonces marcar `D-01 passes=true`.

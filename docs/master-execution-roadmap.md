...
- 2026-05-27 - `[EN CURSO]` `HERMES-AEAT-BATCH-ADJUDICATION-01`: sustituir revision humana modelo-a-modelo por adjudicacion automatica read-only de informes Hermes AEAT. Nuevo objetivo: `adjudicate_aeat_hermes_batch.py --verify-sources --latest-per-model` valida schema, admission gate, URL oficial, locator y excerpt contra contenido oficial vivo; para fuentes binarias oficiales (`.pdf`, `.xls`, `.xlsx`, `.zip`) verifica alcanzabilidad y no exige excerpt literal; autoacepta solo evidencia no afirmativa (`conflict`, `stale_suspected`, no afirmable) y envia a humano unicamente candidatos afirmables, rewrites o rechazos. Primer run VPS sobre 6 JSON existentes: 0 autoaceptados, 5 `needs_report_rewrite` por excerpts HTML/BOE parafraseados y 1 `reject_report` por schema antiguo invalido; se endurece `scripts/hermes_curator/prompts/aeat_model_json.md` para exigir excerpts literales verificables y no narrativos. Smoke posterior del modelo `128` detecta salida Hermes con diff previo y bloque final; se endurece `extract_aeat_hermes_json.py` para usar el ultimo bloque parseable y no fallar por ruido de diff. El batch incorpora `--latest-per-model`, sugerencia automatica de excerpt literal cuando Hermes usa titulo/parafrasis no literal, y solo bloquea fallos de fuentes referenciadas por `official_source_claims` (fuentes no usadas quedan como warning). No escribe DB, no promueve `resolved_strong`, no relaja `ASSERTABLE_DIRECT_OFFICIAL`.

...
- 2026-05-14 10:05 Europe/Madrid - `[EN CURSO LOCAL]` CNMV coverage contract: se corrige la interpretacion de "65 archivos CNMV" como si fuera universo completo. Produccion verificada: `72` documentos CNMV (`65` `circular_cnmv`, `7` `documento_cnmv`), estados `19 vigente`, `23 vigente_modificado`, `30 derogado`; tablas relacionadas `documento_cnmv_version=72`, `cnmv_regulation_link=155`, `cnmv_obligation_link=47`. Cambio local en curso: nuevo `/v1/cnmv/coverage` con familias oficiales cargadas/parciales/no disponibles, docs `docs/operations/cnmv-coverage-map-2026-05-14.md`, manual y `mcp_validation_suite`. Siguiente paso exacto: ejecutar tests, desplegar a VPS y verificar suite MCP con el nuevo contrato.

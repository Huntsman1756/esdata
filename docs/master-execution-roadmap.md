...
- 2026-05-18 22:15 Europe/Madrid - `[EN CURSO]` Sprint J CRS/Modelo 289 en rama `feat/sprint-j-crs-289`. J-01 completado: inspeccionado esquema productivo de tablas `modelo_*` en VPS y documentado en `docs/schema-modelo-tables.md`. Modelo 289 queda identificado como `aeat_modelo.id=95` y `modelo_campana.id=95` para campana `2025`. Hallazgo operativo: `modelo_normativa` y `modelo_trigger_keyword` se enlazan por `modelo_id`; `modelo_instruccion`, `modelo_regla_inclusion`, `modelo_casilla` y `modelo_recurso` se enlazan por `campana_id`. No se cargaron datos en J-01. Siguiente paso exacto: J-02 cargar RD 1021/2015, LGT DA 22 y DAC2 en `modelo_normativa` usando `modelo_id=95` y fuente oficial por fila.

...
- Reclamo 2026-05-06 Task 5 P0 MCP stabilization: `[EN CURSO]` estabilizar semantica de busqueda MCP/API con aislamiento por fuente, fallback filtrado, filtro de doctrina por tipo y respuesta directa de consulta compatible con schemas. Archivos reclamados: `apps/api/services/search.py`, `apps/api/tests/test_search_semantics.py`, `apps/api/tests/test_smoke.py` si necesario, `apps/api/routers/consulta.py` si necesario.
- Reclamo 2026-05-06 Task 6 P0 MCP stabilization: `[COMPLETADA]` `apps/workers/dgt.py` queda protegido con advisory lock no bloqueante (`pg_try_advisory_lock`) y release en `finally`, con compatibilidad para fake engines sin `.url` ni context manager real. Evidencia fresca: `$env:PYTHONPATH='apps/workers'; python -m pytest apps/workers/tests/test_dgt.py -q` -> `19 passed`; `python -m ruff check apps/workers/dgt.py apps/workers/tests/test_dgt.py --select E,F,I` -> `All checks passed!`. Archivos tocados: `apps/workers/dgt.py`, `apps/workers/tests/test_dgt.py`, `docs/master-execution-roadmap.md`.
- Reclamo 2026-05-06 Task 7 P0 MCP stabilization: `[COMPLETADA]` Dockerfiles API/worker verificados con runtime `USER app`, bases Python fijadas por digest y sin copias de `.env`/secretos; `infra/deploy/Dockerfile.ops` endurecido con base Python fijada y `USER app`; guardrail estatica anadida. Evidencia fresca: `python -m pytest scripts/tests/test_dockerfile_security.py -q` -> `5 passed`; `python -m ruff check scripts/tests/test_dockerfile_security.py --select E,F,I` -> `All checks passed!`. Archivos tocados: `infra/deploy/Dockerfile.ops`, `scripts/tests/test_dockerfile_security.py`, `docs/master-execution-roadmap.md`. Riesgos restantes: no se ejecuto Docker build por scope de verificacion estatica.
- Reclamo 2026-05-06 Task 8 P0 MCP stabilization: `[COMPLETADA]` verificacion consolidada ejecutada sin tocar produccion/VPS/live DB/Docker. Evidencia fresca tras quality review: focused blockers (`apps/api/tests/test_p0_schema_safety.py apps/api/tests/test_webhook_verification.py`) -> `19 passed`; suite API focalizada ampliada con webhook route -> `104 passed, 3 warnings`; suite workers focalizada -> `79 passed, 6 warnings`; infra estatica -> `5 passed`; ruff consolidado de Python modificado (`$files = git diff --name-only -- '*.py'; python -m ruff check $files --select E,F,I`) -> `All checks passed!`; check prohibido `CREATE TABLE IF NOT EXISTS` en `apps/api/*.py` + `apps/api/**/*.py` -> `NO_MATCHES`; check prohibido `railway up|branches: [main]` en `.github/workflows/deploy.yml` -> `NO_MATCHES`. Remediado: fixtures/API search, routers `obligaciones`/`empresas`, schema de modelos, worker fake-engine, webhook runtime DDL, workflow Railway desactivado, alias `rerank_score` en citas, DDL de gobernanza movido fuera del runtime API y bootstrap SQLite test-only sin `IF NOT EXISTS`, `source_freshness_snapshot` anclado a Alembic/RLS y validacion PostgreSQL de todas las tablas governance, `/webhooks/generic` corregido para helpers sync con test de firma+idempotencia. Archivos tocados por Task 8: `.github/workflows/deploy.yml`, `SECURITY_BASELINE.md`, `alembic/versions/20260506_0001_p0_rls_current_tables.py`, `apps/api/main.py`, `apps/api/mcp_server.py`, `apps/api/routers/buscar.py`, `apps/api/routers/consulta.py`, `apps/api/routers/doctrina.py`, `apps/api/routers/legislacion.py`, `apps/api/routers/modelos.py`, `apps/api/routers/webhooks.py`, `apps/api/schemas.py`, `apps/api/services/modelos.py`, `apps/api/services/persistence.py`, `apps/api/services/search.py`, `apps/api/services/webhook_verification.py`, `apps/api/tests/conftest.py`, `apps/api/tests/governance_bootstrap.py`, `apps/api/tests/test_governance_http.py`, `apps/api/tests/test_integration.py`, `apps/api/tests/test_mcp_audit.py`, `apps/api/tests/test_mcp_contract.py`, `apps/api/tests/test_persistence.py`, `apps/api/tests/test_p0_schema_safety.py`, `apps/api/tests/test_reranker.py`, `apps/api/tests/test_search_semantics.py`, `apps/api/tests/test_smoke.py`, `apps/api/tests/test_webhook_verification.py`, `apps/workers/aeat_models.py`, `apps/workers/dgt.py`, `apps/workers/dgt_doctrina.py`, `apps/workers/runtime.py`, `apps/workers/tests/test_aeat_models.py`, `apps/workers/tests/test_dgt.py`, `infra/deploy/Dockerfile.ops`, `scripts/tests/test_dockerfile_security.py`, `docs/master-execution-roadmap.md`. No se hizo commit.

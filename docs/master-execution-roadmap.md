...
- 2026-05-31 - `[EN CURSO]` `AEAT-PRIORITY-PARTIAL-EVIDENCE-GATE-01`: abierto tras priorizacion explicita del usuario sobre AEAT. Objetivo inmediato: no promover AEAT como bloque unico, sino endurecer el gate para modelos prioritarios con evidencia oficial parcial ya cargada. Archivos reclamados: `scripts/maintenance/mcp_validation_suite.py`, `scripts/tests/test_maintenance_agents.py`, `docs/master-execution-roadmap.md`. Diagnostico VPS inicial: `303`, `202`, `190` y antiguos `target` (`180`, `184`, `232`, `347`, `349`, `390`) tienen recursos oficiales con hash/procedencia y casillas; `303` sigue con `campana_safe_to_assert=false`, por lo que el cierre debe ser `partial traceable`, no `complete`.

...
- 2026-05-24 - `[EN CURSO LOCAL]` Sprint O Ralph para desbloquear siguientes historias fiscales tras AEAT IRNR 216/296. O-01 cerrada localmente: produccion acredita claves oficiales `296` para dividendos (`CLAVE_RENTA 1`) e intereses (`CLAVE_RENTA 2`) con URL/hash/captura; migracion `20260524_0089_aeat_irnr_income_type_rules` persiste reglas `CONDICIONAL` para ambas rentas; `/v1/modelos/por-supuesto` proyecta evidencia de `modelo_clave` solo con `source_hash` y `capture_date`, manteniendo `evidence_limited`/`verified=false`; `216` no se granulariza por falta de clave de renta equivalente. Validacion O-01: `python -m pytest apps/api/tests/test_modelos_truth_contract.py apps/api/tests/test_alembic_integrity.py -q` => `40 passed`. Siguiente paso exacto: commitear O-01 y ejecutar O-02, auditando CDI sin usar convenio como fuente de obligacion domestica.

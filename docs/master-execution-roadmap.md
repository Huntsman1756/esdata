...
- 2026-06-01 - `[COMPLETADO LOCAL / VPS PENDIENTE]` `AEAT-MODELO-190-INTERNAL-EVIDENCE-01`: cerrado localmente el ciclo separado de auditoria interna del modelo `190`, sin tocar la campana ya resuelta en `0108`. Diagnostico VPS previo: campana `190/2025` ya devuelve `resolved_strong` por carril `operational`, pero las 4 claves A-D existentes en `modelo_clave` no tenian `source_url/source_hash/capture_date`, `modelo_instruccion` estaba vacio y `modelo_regla_inclusion` estaba vacio. Fuente verificada: PDF oficial AEAT `DISENOS_LOGICOS_190_2025.pdf`, hash SHA-256 `a7d1092f78620431812354e560a5146a3ae244e0aed69d9d58c353370ba0134d`, `content_length=1110488`, `capture_date=2026-06-01`. Cambio local: migracion Alembic `20260601_0109_aeat_190_internal_evidence` sustituye las claves legacy A-D por claves oficiales de percepcion A-D con provenance completa, anade 3 instrucciones internas sobre diseno/clave/subclave y 4 reglas `CONDICIONAL` para el uso de esas claves; `mcp_validation_suite.py` anade `aeat_modelo_190_internal_keys_rules_traceable_contract`. Limite: no cambia `campana_resolution_status`, no toca `CAMPAIGN_BEARING_RESOURCE_TYPES`, no promueve `obligacion_perfil`, no marca aplicabilidad por perfil como completa. Validacion local: TDD rojo->verde; `test_alembic_integrity.py` + `test_maintenance_agents.py` => `64 passed`; `py_compile` OK; Ruff focal `F,I` OK; `git diff --check` OK; `alembic heads` unico en `20260601_0109`.

...
- **Sesion 2026-05-02 — Auditoria integral VPS + cron + modelos AEAT**: `[PARTIAL]` — verificacion fresca del VPS productivo completada. Estado observado: `api`, `web`, `postgres`, `prometheus`, `alertmanager` y workers continuos arriba; timers `systemd` activos para `boe`, `modelos`, `dgt`, `teac`, `bdns`, `borme`, `bde`, `cendoj`, `aepd`, `eurlex`, `cnmv` y `sepblac`; `worker_last_errors=0.0` para workers y crons revisados; Prometheus sin alertas activas y Alertmanager sin grupos activos. Fixes aplicados y empujados: (1) `/status` normaliza aliases historicos de `worker-modelos`; (2) `worker-modelos` y `cron-modelos-daily` degradan timeouts de recursos oficiales AEAT a `partial` no fatal con mensaje `Skipped N AEAT official resources after fetch failures`; (3) `WorkerSyncErrors` ignora `worker-modelos` mientras convive con `cron-modelos-daily`; (4) las notificaciones Telegram `resolved` explican que el detalle inferior describe la ultima condicion que disparo la alerta. Hallazgos confirmados: los `cron-*` oneshot fuera de `cron-modelos-daily` necesitaban red `esdata-internal` para resolver `postgres`; faltaban timers para `aepd`, `cendoj`, `eurlex` y `bde` y quedaron instalados en el VPS. Precaucion operativa: `infra/observability/alertmanager.yml` es plantilla con `${TELEGRAM_*}`; en el VPS debe seguirse usando la version renderizada antes de reiniciar Alertmanager.
...
- Reclamo adicional: `[EN CURSO]` intervencion de produccion `worker-modelos` para cerrar `WorkerSyncErrors` reales en VPS. Hipotesis verificada: `apps/workers/aeat_models.py` consume `url_info` malformadas (`ttps://...`) y recursos externos no oficiales como si fueran recursos AEAT, contaminando `sync_log` con `errors > 0`. Archivos reclamados: `docs/master-execution-roadmap.md`, `apps/workers/aeat_models.py`, `apps/workers/tests/test_aeat_models.py`, `infra/observability/alerts.yml`, `docs/operations/runbooks/deploy-compose.md`.
- Archivos afectados:

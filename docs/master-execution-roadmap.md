...
- 2026-05-24 - `[EN CURSO]` `MCP-DATA-04`: RTS1/RTS2 para `sociedad_valores`, con evidencia primaria o fail-closed explicito. RED primero: confirmar en VPS/read-only `rts1_rts2_obligations_all_verified`, `sociedad_valores_rts1_evidence_notice_verified` dentro de `profile_applicability_contracts` y el arrastre sobre `sociedad_valores_verified_ge_24`; inventariar `obligacion_perfil` para `sociedad_valores` con normas `32017R0587`/`32017R0583` y probes `/v1/perfil/sociedad_valores/obligaciones?dominio=CNMV`, `/v1/norma/eu?termino=32017R0587`, `/v1/norma/eu?termino=32017R0583`. No tocar CASP, emisor token ni `all_profiles_pct_verified_ge_70`; `verified=true` solo si fuente primaria demuestra norma y aplicabilidad exacta al perfil.

...
- 2026-05-31 - `[EN CURSO LOCAL]` `P0.6-ANSWER-VERIFIABILITY-01`: endurecer fiabilidad de respuestas MCP/API para que toda respuesta accionable pueda verificarse desde la propia respuesta. Alcance: propagar `source_url`/`source_hash` a `cited_chunks` y `claim_citations` de `/v1/consulta`, anadir `result_metadata.source_verification`, forzar `safe_to_answer=false` si no hay fuente verificable, endurecer politica MCP y crear `scripts/maintenance/response_verifiability_gate.py`.

...
- 2026-05-27 - `[EN CURSO]` `HERMES-AEAT-ADJUDICATION-HISTORY-01`: persistir historico de madurez del adjudicador Hermes AEAT para medir tendencia, no ejecuciones aisladas. `adjudicate_aeat_hermes_batch.py` incorpora `--history-dir` para escribir JSON timestamped con `run_metadata`, hashes/versiones de schema/prompt/adjudicator, Git head, metricas agregadas, ratios (`repaired_excerpt_ratio`, `rewrite_ratio`, `blocking_error_ratio`, `unused_source_warning_ratio`, `assertable_candidate_ratio`) y drilldown por modelo solo para diagnostico. Regla: el dashboard primario es agregado; el drilldown se mira solo cuando el agregado degrada.

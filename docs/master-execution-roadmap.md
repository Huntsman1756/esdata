...
- 2026-05-18 23:55 Europe/Madrid - `[EN CURSO]` Sprint J CRS/Modelo 289 en rama `feat/sprint-j-crs-289`. J-01 a J-07 completados. Modelo 289: normativa `4`, instrucciones `5`, reglas inclusion/exclusion `6`, keywords `12`, casillas `161`. El catalogo AEAT expone ahora `reglas_inclusion_count` y mantiene separacion estricta: no devuelve `obligation_context`. VPS tras rebuild `api`/`ops`: `mcp_validation_suite.py --read-only --base-url http://api:8000` => `ok=true`; `mcp_deep_contract_audit.py --base-url http://api:8000` => `ok=true`. Siguiente paso exacto: J-08 verificacion final completa, informe Sprint J, roadmap final y push de `feat/sprint-j-crs-289`.

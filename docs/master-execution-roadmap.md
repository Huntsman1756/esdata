...
- 2026-05-31 - `[EN CURSO LOCAL]` `FINAL-PRODUCT-ACCEPTANCE-GATE-01`: abierto por decision explicita del usuario para convertir el cierre transversal en gate ejecutable, no en otro plan documental. Archivos bajo reclamo: `scripts/maintenance/final_product_acceptance_gate.py`, `scripts/tests/test_final_product_acceptance_gate.py`, `docs/master-execution-roadmap.md`. Alcance: smoke/gate HTTP contra `/health`, `/status` y una muestra canonica multi-dominio (BOE, AEAT, doctrina DGT/TEAC, CNMV, EUR-Lex/ESMA, MiCA/CASP, screening y dominios parciales AEPD/SEPBLAC/BDE) con salida non-zero si falta fuente, estado de evidencia, fail-closed explicito en respuestas vacias, o salud operativa. Criterios de aceptacion: tests unitarios del gate pasan; `ruff` focal OK; `py_compile` OK; `git diff --check` OK; ejecucion VPS contra API productiva genera JSON/Markdown y cualquier fallo queda clasificado como gap real antes de declarar cierre. Comandos de verificacion: `python -m pytest scripts\tests\test_final_product_acceptance_gate.py -q`; `python -m ruff check scripts\maintenance\final_product_acceptance_gate.py scripts\tests\test_final_product_acceptance_gate.py --select F,I`; `python -m py_compile scripts\maintenance\final_product_acceptance_gate.py`; `python scripts\maintenance\final_product_acceptance_gate.py --base-url http://127.0.0.1:8000 --api-key <key> --json-out <path> --md-out <path>`. Commit de apertura requerido: `feat(final): open product acceptance gate`.

...
- 2026-05-31 - `[COMPLETADO LOCAL]` `V2-BDNS-BORME-STRUCTURED-INGESTION-01`: abierta por decision explicita del usuario tras revisar repos publicos (`bdns-fetch`, `bdns_etl`, `borme_db`, `mcp-govern` y relacionados) para reaprovechar patrones sin copiar codigo GPL/AGPL. Cambios: `worker-bdns` pagina endpoints oficiales estructurados de BDNS (`convocatorias`, `concesiones` y familias relacionadas), persiste metadatos/procedencia exacta en `documento_interpretativo`, mantiene `/v1/bdns` fail-closed con filtros estructurados y `safe_to_answer=false`; `worker-borme` conserva API oficial BOE/sumario, persiste metadatos heuristicas de empresas/personas/cargos desde PDF oficial y `/v1/borme` filtra por empresa relacionada; variables de despliegue, matriz de cobertura, registro de fuentes y OpenAPI GPT regenerados. Validacion local: tests focales `44 passed`; doc artifacts `20 passed`; `source_assurance_gate.py` OK; `verify-doc-contracts.py` OK; Ruff focal OK; `py_compile` OK; `git diff --check` OK.

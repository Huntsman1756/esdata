...
- 2026-05-31 - `[EN CURSO LOCAL]` `RELEASE-CLOSEOUT-GATE-01`: cierre final del gate util v1 antes de agotar limite operativo. Alcance: diagnosticar y eliminar ambiguedad de `legal-sources SKIPPED`, limpiar warning conocido de permisos en `ops-health`, normalizar URLs BOE `http://www.boe.es` a `https://www.boe.es` en artefactos Hermes AEAT, y fijar contrato de cierre fail-closed en `RELEASE-CLOSEOUT.md`/`PROJECT_STATE.md` sin ampliar el claim de producto. Criterio: si un criterio no puede verificarse, el gate falla; 124/126/128 solo pueden salir de `CONFLICT`/`campana_safe_to_assert=false`/`human_review_required=true` con evidencia oficial directa y verificable.

...
- 2026-05-20 07:05 Europe/Madrid - `[EN CURSO]` Auditoria stale workers A-04b en rama `fix/full-audit-stale-workers-20260520`. A-04 queda bloqueada tras derivar 14 persistent workers y detectar 10 OK, 3 timeouts largos (`worker-cnmv`, `worker-dgt`, `worker-modelos`) y 1 bug real en `worker-eurlex`: duplicado `norma_boe_id_key` para `EUR-CELEX-32014L0065`, seguido de duplicado dead-letter `uq_dead_letter_worker_entity`. Se anade historia `A-04b` al PRD como bloqueante antes de A-05: corregir upsert EUR-Lex idempotente y dead-letter idempotente, luego reejecutar smoke focal de `worker-eurlex`. Informe: `docs/persistent-worker-smoke-20260520.md`. Siguiente paso exacto: A-04b.

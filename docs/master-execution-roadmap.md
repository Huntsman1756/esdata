...
- 2026-06-02 - `[COMPLETADO LOCAL]` `AEAT-MODELO-190-SUBCLAVES-0113`: preparada la migracion de datos `20260602_0113_aeat_190_subclaves_percepcion` para cargar las 77 subclaves oficiales del Modelo `190/2025` como filas `modelo_clave` de `nivel=2` bajo los padres B, C, E, F, G, H, I, K y L creados en `0111` y jerarquizados por `0112`. Fuente unica: PDF oficial AEAT `DISENOS_LOGICOS_190_2025.pdf`, hash SHA-256 `a7d1092f78620431812354e560a5146a3ae244e0aed69d9d58c353370ba0134d`, `capture_date=2026-06-01`. Datos: conteos exactos B=5, C=9, E=4, F=7, G=8, H=4, I=3, K=5, L=32; las descripciones normalizadas eliminan cabeceras/pies del PDF y quedan en ASCII para evitar deuda de encoding. Contrato: lookup de `parent_id` por codigo de clave, sin IDs hardcodeados; `tipo_clave/tipo='SUBCLAVE_PERCEPCION'`, `nivel=2`, `source_url/source_hash/capture_date` por fila; validacion interna exige `COUNT(*) = 77`, codigos exactos por clave via `array_agg(codigo ORDER BY codigo)`, descripciones no vacias y sin marcadores de cabecera. `mcp_validation_suite.py` anade `aeat_modelo_190_subclaves_77_hierarchical_contract`. Limites: no cambia campana, no toca `obligacion_perfil`, no promueve `safe_to_answer`, `verified` ni `completeness`, y no altera claves principales A-L. Validacion local: TDD rojo->verde; `test_alembic_integrity.py` + `test_maintenance_agents.py` => `68 passed`; `py_compile` OK; Ruff focal `F,I` OK; `git diff --check` OK; `alembic heads` unico en `20260602_0113`. Pendiente: aplicar 0113 en VPS sobre el corpus real ya migrado hasta 0112 y ejecutar gates.

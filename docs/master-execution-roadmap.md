...
- 2026-05-27 - `[EN CURSO]` `HERMES-AEAT-SIGNAL-QUALITY-210-113-01`: reducir ruido de madurez detectado por el resumen de tendencias sin relajar el contrato fiscal. Diagnostico: los ultimos JSON de `210` y `113` mantienen `blocking_error_ratio=0`, pero `210` concentra reparaciones de excerpt por textos internos/cacheados en `official_sources.excerpt`, y `113` incluye una fuente oficial no usada que falla verificacion. Objetivo: endurecer validator/prompt para bloquear excerpts oficiales con terminos internos (`cache`, `metadata`, `MCP`, `ESData`, `modelo_recurso`, `campana_activa`, `campana_persistida`) y obligar a que `official_sources` contenga solo fuentes usadas por `official_source_claims`, salvo reports sin claims. No se escriben datos productivos ni se promueve campana.

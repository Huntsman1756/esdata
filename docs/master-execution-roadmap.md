...
- 2026-05-24 - `[EN CURSO]` Despliegue VPS del contrato `MCP-DATA-02`: versionar y publicar los cambios locales de registry/OpenAPI/fail-closed 289, reconstruir servicios `api` y `ops`, y reejecutar `mcp_validation_suite.py --read-only --base-url http://api:8000` + `mcp_deep_contract_audit.py --base-url http://api:8000` en produccion para confirmar que los fallos del Modelo 289 pasan a estado aceptado/fail-closed y que los fallos restantes quedan separados como `MCP-DATA-03+`.

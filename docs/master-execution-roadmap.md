...
- 2026-05-24 - `[EN CURSO]` `MCP-DATA-05`: MiCA/CASP y `emisor_token`, separando CASP, ART base y EMT parcial. RED primero: confirmar en VPS/read-only `casp_obligations_all_verified`, `emisor_token_obligations_all_verified`, `emisor_token_art_base_obligations_completa` y su arrastre en `eu_norm_contracts`; inventariar total, `verified`, `completeness`, `source_url`, `source_hash`, `capture_date` y articulos MiCA por perfil. No tocar `sociedad_valores_verified_ge_24` ni `all_profiles_pct_verified_ge_70` como objetivo directo; `verified=true` solo si fuente primaria demuestra norma/articulo y aplicabilidad exacta al perfil.

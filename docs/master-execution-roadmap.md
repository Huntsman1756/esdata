...
- Estado del agente activo: slice local de scheduling y alertas cerrado; el repo y las docs activas ya quedan alineados con `/srv/esdata`, `docker compose run --rm --no-deps` y `worker_stale_status`.
- Reclamo actual: ninguno. Slice CI PR #41 preparado en `G:\_Proyectos\esdata\.worktrees\mcp-compliance-remediation` con fixes minimos para link checker, security-audit lightweight, Postgres CI con `pgvector`, mypy `esdata_common` y scratch tests; verificacion local: docs verifier tests 14 passed, ruff scoped OK, format scoped OK, mypy OK, `--security-only` y docs artifact gate OK. Pendiente CI remoto y merge del PR.
- Siguiente paso exacto: pushear `fix/mcp-compliance-remediation`, observar GitHub Actions de PR #41 y mergear solo si todos los checks requeridos quedan verdes.

- Nota 2026-05-05: slice de hardening minimo de scheduling y alertas `[COMPLETA]` cerrado en `G:\_Proyectos\esdata\.worktrees\next-task`. Resultado: `infra/deploy/systemd/esdata-job@.service` fija `WorkingDirectory=/srv/esdata` y ejecuta `docker compose ... run --rm --no-deps %i`; `infra/deploy/docker-compose.prod.yml` deja los `cron-*` con DB en `esdata-internal`; `infra/observability/alerts.yml` cambia `WorkerSilent` a `worker_stale_status == 1` para distinguir incidentes reales `daily` del ruido falso en `weekly`; y las docs activas de deploy/ops quedan alineadas con la misma raiz operativa. Evidencia fresca del cierre: `python -m pytest scripts/tests/test_deploy_hetzner.py -q` -> `14 passed`; `python -m ruff check scripts/tests/test_deploy_hetzner.py --select F,I` -> `All checks passed!`; lectura del repo activo confirma `/srv/esdata`, `--no-deps` y `worker_stale_status` en los artefactos tocados. Riesgo residual explicitado: si el VPS quedo parcheado manualmente antes de estos commits, conviene normalizar `/srv/esdata` con `git pull --ff-only origin main` para eliminar drift entre repo y runtime.

### Historial MCP [HISTORICAL]

- Nota 2026-05-05: Fase 6.8 `[COMPLETA]` cerrada en `G:\_Proyectos\esdata\.worktrees\next-task`. Resultado: `docs/operations/runbooks/mcp-release-gate.md` deja de describir solo un gate minimo y pasa a distinguir explicitamente tres estados de decision para MCP: `GO minimo`, `GO confianza alta` y `NO-GO`. El runbook fija que `GO confianza alta` exige, sobre el mismo estado del repo, la matriz MCP ampliada ya cerrada en `6.5`, la regresion factual MCP cerrada en `6.6`, la validacion explicita de `stdio MCP` cerrada en `6.7` y evidencia suficiente de comportamiento conservador / fail-closed en respuestas sensibles. `docs/master-execution-roadmap.md` deja el frente MCP `6.5+` como cerrado: el repositorio ya no queda solo remediado, sino endurecido hasta confianza `alta` para el scope MCP soportado hoy. Evidencia fresca del cierre: lectura del workflow vigente confirmando que `test-python` incluye `test_mcp_transport.py`, `test_mcp_audit.py`, `test_http_mcp_audit_phase_1_1.py`, `test_mcp_truth_regressions.py` y `test_mcp_stdio_audit.py`; lectura del runbook actualizado confirmando el criterio explicito `GO minimo` / `GO confianza alta` / `NO-GO`; `python scripts/maintenance/verify-doc-contracts.py` -> `docs contracts verified`. Riesgo residual explicitado: cualquier ampliacion futura en este frente ya seria nueva cobertura MCP o ampliacion de dominio, no la brecha principal entre confianza `media-alta` y `alta`.

- Nota 2026-05-05: Fase 6.7 `[COMPLETA]` cerrada en `G:\_Proyectos\esdata\.worktrees\next-task`. Resultado: `.github/workflows/ci.yml` anade un gate explicito para `stdio MCP` con `python -m pytest apps/api/tests/test_mcp_stdio_audit.py -q` dentro de `test-python`, despues de la regresion factual MCP y antes del barrido general Python. `docs/operations/runbooks/mcp-release-gate.md` deja ahora explicito que, como `stdio MCP` es una superficie soportada en el repo activo, su validacion ya no es opcional en el criterio MCP endurecido, y documenta por separado la evidencia esperada de `HTTP MCP` y `stdio MCP`. La verificacion local del slice no requirio reparaciones de runtime: la suite existente `apps/api/tests/test_mcp_stdio_audit.py` ya cubria correlacion de `request_id`, conservacion de `tool_name` y fail-closed cuando la persistencia de auditoria se rompe. Evidencia fresca del cierre: lectura del workflow actualizado confirmando el nuevo paso `Run stdio MCP audit regression tests`; lectura del runbook actualizado confirmando la separacion explicita entre validacion `HTTP MCP` y `stdio MCP`; `python -m pytest apps/api/tests/test_mcp_stdio_audit.py -q` -> `3 passed`; `python scripts/maintenance/verify-doc-contracts.py` -> `docs contracts verified`. Riesgo residual explicitado: el repo ya valida explicitamente ambas superficies MCP soportadas, pero el runbook sigue definiendo un gate minimo endurecido y no un criterio final de confianza `alta`; eso queda para `Fase 6.8`.

- Nota 2026-05-05: Fase 6.6 `[COMPLETA]` cerrada en `G:\_Proyectos\esdata\.worktrees\next-task`. Resultado: `.github/workflows/ci.yml` anade un gate MCP factual explicito con `python -m pytest apps/api/tests/test_mcp_truth_regressions.py -q` dentro de `test-python`, despues del bloque MCP de transporte/auditoria y antes del barrido general de tests Python. La verificacion local del slice no requirio reparaciones: `apps/api/tests/test_mcp_truth_regressions.py` ya fija cinco preguntas MCP historicamente peligrosas y exige respuestas fail-closed con `NO VERIFICADO`, y `apps/api/tests/test_modelos_truth_contract.py` sigue verde como soporte del truth contract de modelos AEAT. Evidencia fresca del cierre: lectura del workflow actualizado confirmando el nuevo paso `Run MCP factual regression tests`; `python -m pytest apps/api/tests/test_mcp_truth_regressions.py -q` -> `5 passed`; `python -m pytest apps/api/tests/test_modelos_truth_contract.py -q` -> `10 passed`. Riesgo residual explicitado: la confianza factual mejora materialmente, pero `stdio MCP` sigue fuera del gate explicito de release/CI endurecido; eso queda para `Fase 6.7`.

- Nota 2026-05-05: Fase 6.5 `[COMPLETA]` cerrada en `G:\_Proyectos\esdata\.worktrees\next-task`. Resultado: `.github/workflows/ci.yml` amplia el bloque MCP de `test-python` con `python -m pytest apps/api/tests/test_mcp_transport.py -q`, `python -m pytest apps/api/tests/test_mcp_audit.py -q` y `python -m pytest apps/api/tests/test_http_mcp_audit_phase_1_1.py -q`, manteniendo visibles los checks previos de doc contracts, private y contract. Durante el slice se corrigieron dos defectos reales que impedian que esa matriz fuera fiable en este entorno: (1) `apps/api/services/` y `apps/api/routers/` no tenian `__init__.py`, lo que dejaba que imports top-level resolvieran contra otro proyecto local; y (2) el bridge HTTP MCP no reenviaba `x-request-id` / `x-user-id` a las subrequests REST, por lo que el audit trail MCP persistia `request_id='unknown'`. Ademas, `apps/api/tests/test_mcp_transport.py` y `apps/api/tests/test_mcp_audit.py` quedaron realineados con la superficie HTTP MCP canonica y con un harness uvicorn-backed reutilizable en `apps/api/tests/_mcp_http_transport_harness.py`, evitando el falso verde de `consulta_fiscal` via HTTP MCP y el falso rojo de `ASGITransport` sin session manager inicializado. Evidencia fresca del cierre: lectura del workflow actualizado confirmando los tres comandos nuevos; `python -m pytest apps/api/tests/test_mcp_transport.py -q` -> `2 passed`; `python -m pytest apps/api/tests/test_mcp_audit.py -q` -> `1 passed`; `python -m pytest apps/api/tests/test_http_mcp_audit_phase_1_1.py -q` -> `13 passed`. Riesgo residual explicitado: la matriz MCP ya cubre mejor transporte y auditoria HTTP, pero la confianza alta sigue pendiente porque la regresion factual MCP aun no es un gate explicito de CI; eso queda para `Fase 6.6`.

- Nota 2026-05-05: slice documental `6.5+` `[COMPLETA]` cerrado en `G:\_Proyectos\esdata\.worktrees\next-task`. Resultado: `docs/reference/mcp-remediation-plan.md` ya define la continuacion `6.5`, `6.6`, `6.7` y `6.8` como endurecimiento de confianza posterior al cierre base `6.1`-`6.4`; `docs/operations/runbooks/mcp-release-gate.md` deja explicito que el gate actual es el minimo correcto del estado presente y que no equivale todavia a confianza `alta`; y este roadmap vuelve a tener un siguiente paso exacto MCP en `Fase 6.5`. Evidencia fresca del cierre: lectura de los tres markdowns activos actualizados y comprobacion de consistencia entre remediation plan, roadmap y runbook. Riesgo residual explicitado: este slice no endurece todavia CI, runtime ni tests; solo deja documentado el trabajo pendiente para pasar de confianza `media-alta` a `alta`.
...
- Nota 2026-05-04 10:20Z: Fase 3.3 `[COMPLETA]` cerrada en `G:\_Proyectos\esdata\.worktrees\next-task`. Resultado: el runtime de `modelos` ya deriva `completeness`/`verified` desde un truth contract compartido en vez de inferir verificacion operativa solo porque un sub-endpoint devuelve filas locales. `apps/api/services/modelos.py` materializa/consume las lecturas necesarias para evitar `sqlite3.OperationalError: database is locked` al persistir `query_audit`, y `apps/api/routers/modelos.py` alinea la auditoria E2E de `casillas`, `claves`, `instrucciones` y `fuentes-oficiales` con la misma verdad que `/v1/modelos/{codigo}` y `/campana-operativa`. `apps/api/tests/test_modelos_truth_contract.py` anade regresiones especificas para `model 100` en esos sub-endpoints. Evidencia fresca del cierre: `python -m pytest apps/api/tests/test_modelos_truth_contract.py -q` -> `10 passed`; `python -m pytest apps/api/tests/test_http_mcp_audit_phase_1_1.py -q` -> `13 passed`; `python -m pytest apps/api/tests/test_smoke.py -q -k "modelo_articulos_endpoint or modelo_campana_operativa_endpoint_devuelve_payload_valido or modelos_campanas_operativas_endpoint_devuelve_envelope_valido or modelo_detalle_con_fuente"` -> `4 passed`; `python -m ruff check apps/api/routers/modelos.py apps/api/services/modelos.py apps/api/tests/test_modelos_truth_contract.py apps/api/tests/test_smoke.py apps/api/tests/test_http_mcp_audit_phase_1_1.py` -> `All checks passed!`.
- Nota 2026-05-04 10:55Z: Fase 3.4 `[COMPLETA]` cerrada en `G:\_Proyectos\esdata\.worktrees\next-task`. Resultado: `scripts/maintenance/check_model_data_quality.py` ya ejecuta un gate pequeno y determinista para detectar hostnames no canonicos, provenance debil en `modelo_articulo`, pseudo-norma sospechosa y drift entre `origen_metadato` / `estado_metadato`, separando chequeos estaticos de seeds/scripts AEAT de chequeos DB-backed sobre `aeat_modelo` y tablas `modelo_*`. `.github/workflows/ci.yml` queda cableado para ejecutar este gate y el test de regresion dedicado dentro de `test-python` tras `alembic upgrade heads`. Evidencia fresca local del cierre: `python -m pytest scripts/tests/test_check_model_data_quality.py -q` -> `10 passed`; `python scripts/maintenance/check_model_data_quality.py --static-only` -> `Model data quality OK`; `python -m ruff check scripts/maintenance/check_model_data_quality.py scripts/tests/test_check_model_data_quality.py` -> `All checks passed!`.
- Nota 2026-05-04 12:55Z: Fase 4.1 `[COMPLETA]` cerrada en `G:\_Proyectos\esdata\.worktrees\next-task`. Resultado: DGT deja de sobrecargar `source_revision` con sentinels de cola y pasa a usar `dgt_queue` como cola dedicada para `pending`, `processed` y `empty`. `apps/workers/dgt.py` mueve seed/discovery/procesado de cola fuera de `content_hash_sha256`; `source_revision` vuelve a quedar reservada para hashes reales via `check_content_changed()` y `record_revision()`. Se anade migracion `alembic/versions/20260504_0057_dgt_queue_split.py` para crear `dgt_queue`, backfillear filas DGT desde `source_revision` y borrar sentinels no hash heredados; `apps/api/tests/test_alembic_integrity.py` fija esa revision. `apps/workers/tests/test_dgt.py` anade regresiones para demostrar que `_ensure_dgt_queue()` ya no crea sentinels en `source_revision`, que un sync exitoso deja `dgt_queue.status='processed'` con hash SHA-256 real en `source_revision`, y que un documento fuera de target queda `dgt_queue.status='empty'` sin fila sentinel residual. Evidencia fresca del cierre: `python -m pytest apps/workers/tests/test_dgt.py -q` -> `17 passed`; `python -m pytest apps/api/tests/test_alembic_integrity.py -q` -> `7 passed`.
- Nota 2026-05-04: Fase 4.2 `[COMPLETA]` cerrada en `G:\_Proyectos\esdata\.worktrees\next-task`. Resultado: `apps/workers/runtime.py` anade `finalize_partial_sync_status(...)` como helper minimo compartido para derivar `status` y `error_msg` finales desde un contador de faltantes; `apps/workers/aeat_models.py` reutiliza ese helper sin cambiar el contrato observable de AEAT; `apps/workers/cnmv.py` cuenta ya solo fallos reales de fetch/download por documento y cierra `partial` con `Skipped N CNMV documents after fetch failures`; y `apps/workers/dgt.py` degrada a `partial` solo cuando el run concluye que faltan documentos reales (`search` sin resultados), sin contar documentos fuera de target ni errores transitorios que dejan la cola pendiente para retry. `apps/workers/tests/test_runtime.py`, `apps/workers/tests/test_aeat_models.py`, `apps/workers/tests/test_cnmv.py` y `apps/workers/tests/test_dgt.py` fijan la regresion de ese contrato. Evidencia fresca del cierre: `python -m pytest apps/workers/tests/test_runtime.py apps/workers/tests/test_aeat_models.py apps/workers/tests/test_cnmv.py apps/workers/tests/test_dgt.py -q` -> `143 passed, 1 skipped`. Riesgo residual explicitado: `python -m ruff check apps/workers/runtime.py apps/workers/aeat_models.py apps/workers/cnmv.py apps/workers/dgt.py apps/workers/tests/test_runtime.py apps/workers/tests/test_aeat_models.py apps/workers/tests/test_cnmv.py apps/workers/tests/test_dgt.py` sigue rojo con `48 errors` de deuda heredada a nivel de archivo completo y no bloquea el cierre funcional de `4.2`.
- Nota 2026-05-04: Fase 4.3 `[COMPLETA]` cerrada en `G:\_Proyectos\esdata\.worktrees\next-task`. Resultado: `alembic/versions/20260504_0058_row_completeness_provenance.py` anade `row_completeness` y `row_provenance` a `modelo_recurso` y `documento_interpretativo` con backfill conservador y constraints de contrato; `apps/api/tests/test_alembic_integrity.py` fija esa revision; `apps/workers/aeat_models.py`, `apps/workers/cnmv.py` y `apps/workers/dgt.py` sellan las filas nuevas exitosas como `complete` + `official_exact`; y `apps/api/services/source_manifest.py` / `apps/api/routers/source_manifest.py` mantienen `/v1/sources/*` en nivel fuente con guardrail explicito en `apps/api/tests/test_source_manifest.py`, incluso cuando existe una tabla row-level con `row_completeness` / `row_provenance`. Evidencia fresca del cierre: `python -m pytest apps/api/tests/test_alembic_integrity.py apps/api/tests/test_source_manifest.py apps/workers/tests/test_aeat_models.py apps/workers/tests/test_cnmv.py apps/workers/tests/test_dgt.py -q` -> `150 passed, 1 skipped`; `python -m ruff check alembic/versions/20260504_0058_row_completeness_provenance.py apps/api/services/source_manifest.py apps/api/routers/source_manifest.py apps/api/tests/test_alembic_integrity.py apps/api/tests/test_source_manifest.py --select F,I` -> `All checks passed!`; `python -m ruff check apps/workers/aeat_models.py apps/workers/cnmv.py apps/workers/dgt.py apps/workers/tests/test_aeat_models.py apps/workers/tests/test_cnmv.py apps/workers/tests/test_dgt.py --select F` -> `All checks passed!`. Riesgo residual explicitado: `source_manifest` sigue source-level por diseno; cualquier agregacion row-level queda fuera de scope hasta una fase dedicada.
- Nota 2026-05-04: Fase 4.4 `[COMPLETA]` cerrada en `G:\_Proyectos\esdata\.worktrees\next-task`. Resultado: `apps/workers/boe.py` ya separa `auto_link_exact` de `auto_link_heuristic` al persistir `documento_articulo`; `apps/api/routers/doctrina.py` y `apps/api/routers/dgt_doctrina.py` solo marcan anclaje fuerte, `verified=true` y `completeness="completa"` cuando existe al menos un enlace exacto (`manual`, `manual_official`, `auto_link_exact`); `apps/api/tests/test_smoke.py` y `apps/api/tests/test_api_dgt_doctrina.py` fijan esos contratos con fixtures aisladas; `apps/api/tests/test_graph_connectivity.py` bloquea la propagacion de `da.metodo_enlace` y `da.confianza_enlace`; y `apps/workers/tests/test_borme.py` deja explicito que `documento_empresa` sigue siendo heuristico en este slice. Evidencia fresca del cierre: `python -m pytest apps/workers/tests/test_boe.py apps/workers/tests/test_borme.py apps/api/tests/test_smoke.py apps/api/tests/test_api_dgt_doctrina.py apps/api/tests/test_graph_connectivity.py -q` -> `115 passed, 1 warning`; `python -m ruff check apps/workers/boe.py apps/workers/tests/test_boe.py apps/workers/tests/test_borme.py apps/api/routers/doctrina.py apps/api/routers/dgt_doctrina.py apps/api/services/graph_connectivity.py apps/api/tests/test_smoke.py apps/api/tests/test_api_dgt_doctrina.py apps/api/tests/test_graph_connectivity.py docs/master-execution-roadmap.md docs/operations/agent-notes.md --select F,I` -> `All checks passed!`. Riesgo residual explicitado: `documento_empresa` no adquiere semantica exacta en `4.4`; cualquier promotion futura requiere un slice dedicado.
- Nota 2026-05-04: Fase 4.5 `[COMPLETA]` cerrada en `G:\_Proyectos\esdata\.worktrees\next-task`. Resultado: `apps/workers/vocabulary_validation.py` ya sanea payloads reales con fallbacks explicitos hacia `VOCABULARY`; `bde.py`, `aepd.py`, `bdns.py`, `sepblac.py`, `teac.py`, `cendoj.py`, `dgt.py` y `cnmv.py` aplican esa validacion en el boundary de escritura de `documento_interpretativo`; y los valores fuera de vocabulario confirmados en este slice (`Banco de España`, `TSJ`, taxonomias locales CNMV) ya degradan a valores permitidos sin romper los workers. Evidencia fresca del cierre: `python -m pytest apps/workers/tests/test_vocabulary_validation.py apps/workers/tests/test_bde.py apps/workers/tests/test_cendoj.py apps/workers/tests/test_cnmv.py apps/workers/tests/test_aepd.py apps/workers/tests/test_bdns.py apps/workers/tests/test_sepblac.py apps/workers/tests/test_dgt.py apps/workers/tests/test_teac.py -q` -> `146 passed, 1 skipped, 2 warnings`; `python -m ruff check apps/workers/vocabulary_validation.py apps/workers/bde.py apps/workers/aepd.py apps/workers/bdns.py apps/workers/sepblac.py apps/workers/teac.py apps/workers/cendoj.py apps/workers/dgt.py apps/workers/cnmv.py apps/workers/tests/test_vocabulary_validation.py apps/workers/tests/test_bde.py apps/workers/tests/test_cendoj.py apps/workers/tests/test_cnmv.py docs/master-execution-roadmap.md docs/operations/agent-notes.md --select F,I` -> `All checks passed!`. Riesgo residual explicitado: la verificacion final sigue emitiendo warnings heredados fuera de scope (`pytest_asyncio` loop scope y `apps/workers/bdns.py` con `datetime.utcnow()`), sin bloquear el cierre funcional de `4.5`.
- Nota 2026-05-05: Fase 5.1 `[COMPLETA]` cerrada en `G:\_Proyectos\esdata\.worktrees\next-task`. Resultado: `scripts/ops/deploy-hetzner.sh` pasa a ser la ruta canonica de despliegue con orden obligatorio `build ops -> up postgres -> alembic upgrade head -> verify_schema.py -> up servicios`; `.github/workflows/deploy-hetzner.yml` documenta ese contrato al delegar en el script canonico; y `docs/deployment/server-installation.md` + `docs/operations/runbooks/deploy-compose.md` quedan alineados con el mismo flujo sin permitir saltarse migraciones. Evidencia fresca del cierre: `python -m pytest scripts/tests/test_deploy_hetzner.py -q` -> `4 passed`; `python -m ruff check scripts/tests/test_deploy_hetzner.py --select F,I` -> `All checks passed!`; `bash -n "scripts/ops/deploy-hetzner.sh"` -> sin errores. Riesgo residual explicitado: este slice no ejecuta un deploy Compose real ni una SSH real; solo fija y verifica el contrato del script, workflow y docs.
- Nota 2026-05-05: Fase 5.2 `[COMPLETA]` cerrada en `G:\_Proyectos\esdata\.worktrees\next-task`. Resultado: `scripts/maintenance/verify_schema.py` deja de validar solo `modelo_campana_operativa` y pasa a cubrir tambien `query_audit_log`, `dgt_queue` y `documento_interpretativo` como gate estructural minimo del runtime actual; ademas exige `query_audit_log.entry_id`, `query_audit_log.created_at` y la unicidad de `dgt_queue(worker_name, source_entity_id)` para evitar falsos verdes sobre rutas reales de auditoria y cola DGT. Evidencia fresca del cierre: `python -m pytest scripts/tests/test_verify_schema.py -q` -> `15 passed`; `python -m ruff check scripts/maintenance/verify_schema.py scripts/tests/test_verify_schema.py --select F,I` -> `All checks passed!`. Riesgo residual explicitado: el gate sigue sin comprobar otros indices/constraints, seeds ni datos reales; ese hardening queda fuera de `5.2`.
- Nota 2026-05-05: Fase 5.3 `[COMPLETA]` cerrada en `G:\_Proyectos\esdata\.worktrees\next-task`. Resultado: `infra/deploy/compose.env.example` queda alineado exactamente con el boundary activo de `infra/deploy/docker-compose.prod.yml`; `scripts/tests/test_compose_env_example.py` fija ese contrato, rechaza duplicados y bloquea `NEXT_PUBLIC_API_BASE_URL` incluso en comentarios; `docs/environment-variables.md` pasa a clasificar variables por `runtime deploy`, `code-only` y `legacy/no cableada`; y `docs/deployment/server-installation.md` / `.env.example` dejan de contradecir el flujo activo basado en `infra/deploy/.env.prod`. Evidencia fresca del cierre: `python -m pytest scripts/tests/test_compose_env_example.py -q` -> `3 passed`; `docker compose --env-file infra/deploy/compose.env.example -f infra/deploy/docker-compose.prod.yml config` -> resuelve correctamente. Riesgo residual explicitado: siguen existiendo variables fuera del deploy activo usadas por codigo, tests o tooling local; `5.3` las reclasifica y documenta, no las elimina del repo.
- Nota 2026-05-05: Fase 5.4 `[COMPLETA]` cerrada en `G:\_Proyectos\esdata\.worktrees\next-task`. Resultado: el deploy canonico ya arranca exactamente el worker set continuo definido por `infra/deploy/docker-compose.prod.yml` (`worker-boe`, `worker-dgt`, `worker-teac`, `worker-modelos`, `worker-bdns`, `worker-borme`, `worker-cnmv`, `worker-sepblac`, `worker-cendoj`, `worker-eurlex`, `worker-bde`, `worker-aepd`) sin mezclar servicios profile-gated como `worker-aeat`; `scripts/tests/test_deploy_hetzner.py` fija ese contrato por TDD; `docs/deployment/server-installation.md` y `docs/operations/runbooks/deploy-compose.md` documentan el mismo set y dejan ejecutable el flujo real (`/status` con `X-API-Key`, raiz `/srv/esdata`, cron jobs `systemd` sobre el mismo root); y `infra/deploy/systemd/esdata-job@.service` ya usa la misma raiz operativa que la guia de instalacion. Evidencia fresca del cierre: `python -m pytest scripts/tests/test_deploy_hetzner.py -q` -> `6 passed`; `bash -n "scripts/ops/deploy-hetzner.sh"` -> sin errores. Riesgo residual explicitado: `5.4` alinea el set y la documentacion del deploy activo, pero no introduce nuevos timers versionados para todos los `cron-*` ni cambia el scope funcional de cada worker.
- Nota 2026-05-05: Fase 5.5 `[COMPLETA]` cerrada en `G:\_Proyectos\esdata\.worktrees\next-task`. Resultado: el contrato operativo del env-file runtime ya queda cerrado de extremo a extremo fuera del checkout: `scripts/ops/deploy-hetzner.sh`, `scripts/ops/backup-postgres.sh` y `infra/deploy/systemd/esdata-job@.service` usan `/etc/esdata/esdata.env` por defecto; `infra/deploy/docker-compose.prod.yml` elimina la ultima dependencia Compose a `env_file` repo-local en `worker-aeat`; `infra/deploy/compose.env.example` queda como plantilla autoritativa del deploy activo; `.env.example` pasa a documentarse solo como inventario amplio sin autoridad de deploy; y los runbooks activos (`docs/deployment/vps-trial-deploy.md`, `docs/operations/OPERATIONS.md`, `docs/operations/runbooks/worker-aeat.md`, `docs/environment-variables.md`) ya reflejan el flujo canonico `bash scripts/ops/deploy-hetzner.sh`, `alembic upgrade head`, el worker set continuo real y el uso explicito de claves exportadas para verificaciones manuales con `curl`. Evidencia fresca del cierre: `python -m pytest scripts/tests/test_deploy_hetzner.py -q` -> `10 passed`; `python -m pytest scripts/tests/test_compose_env_example.py -q` -> `3 passed`; `python -m ruff check scripts/tests/test_deploy_hetzner.py --select F,I` -> `All checks passed!`; `bash -n "scripts/ops/deploy-hetzner.sh"` -> sin errores; `docker compose --env-file "infra/deploy/compose.env.example" -f "infra/deploy/docker-compose.prod.yml" config` -> resuelve correctamente. Riesgo residual explicitado: sigue pendiente `Fase 6.1` para alinear docs e integraciones MCP de usuario con la separacion real entre HTTP MCP, stdio MCP y otras superficies soportadas; ademas persiste el warning heredado `PytestDeprecationWarning` de `pytest-asyncio`, fuera de scope de `5.5`.
- Nota 2026-05-05: Fase 6.1 `[COMPLETA]` cerrada en `G:\_Proyectos\esdata\.worktrees\next-task`. Resultado: `docs/architecture.md` ya separa `REST/OpenAPI`, `HTTP MCP` y `stdio MCP` como superficies de integracion activas; `docs/manual-usuario/06-api-y-ejemplos.md` queda REST-only; `docs/manual-usuario/07-mcp-y-clientes.md` explicita `HTTP MCP` vs `stdio MCP` con anclajes concretos a `apps/api/mcp_catalog.py`/`HTTP_MCP_OPERATIONS` y `apps/api/mcp_stdio.py`/`consulta_fiscal`; y `docs/integrations/opencode-local-and-vps.md` queda reducida al contrato `OpenCode -> HTTP MCP` por `url` + `X-API-Key`. Evidencia fresca del cierre: `python -c "from pathlib import Path; text=Path(r'docs/architecture.md').read_text(encoding='utf-8'); print('ok architecture' if all(s in text for s in ['REST/OpenAPI','HTTP MCP','stdio MCP']) else 'missing architecture split')"` -> `ok architecture`; `python -c "from pathlib import Path; text=Path(r'docs/manual-usuario/06-api-y-ejemplos.md').read_text(encoding='utf-8'); print('chapter 06 rest-only ok' if '/mcp' not in text else 'chapter 06 mentions /mcp')"` -> `chapter 06 rest-only ok`; `python -c "from pathlib import Path; text=Path(r'docs/manual-usuario/07-mcp-y-clientes.md').read_text(encoding='utf-8'); checks=['apps/api/mcp_catalog.py','HTTP_MCP_OPERATIONS','apps/api/mcp_stdio.py','consulta_fiscal']; print('chapter 07 split ok' if all(c in text for c in checks) else 'chapter 07 split missing refs')"` -> `chapter 07 split ok`; `python -c "from pathlib import Path; text=Path(r'docs/integrations/opencode-local-and-vps.md').read_text(encoding='utf-8'); ok=all(c in text for c in ['OpenCode','HTTP MCP','/mcp','X-API-Key: <MCP_API_KEY>']) and 'No cubre' in text and 'stdio MCP' in text; print('opencode http-only ok' if ok else 'opencode scope mismatch')"` -> `opencode http-only ok`. Riesgo residual explicitado: la separacion documental queda ya cerrada, pero `Fase 6.2` sigue pendiente para automatizar este contrato con tests o un verificador dedicado.
- Nota 2026-05-03 19:05Z: historico de inicio del slice en `fix/mcp-phase-0-1`; la Fase 1.4 quedo reclamada en ese momento con estos archivos de trabajo: `apps/api/services/query_audit.py`, `apps/api/services/persistence.py`, `apps/api/routers/consulta.py`, `apps/api/mcp_stdio.py`, `apps/api/tests/test_query_audit.py`, `apps/api/tests/test_mcp_stdio_audit.py`, `alembic/versions/20260503_0055_query_audit_response_payload.py`. Objetivo exacto de ese slice: persistir en `query_audit_log` un `response_payload` suficiente para reconstruir lo que vio el usuario en `/v1/consulta` y en `stdio`, con TDD primero y migracion Alembic minima.
...
- Nota 2026-05-03 18:55Z: Fase 0.1 en ejecucion sobre worktree limpio `fix/mcp-phase-0-1`. Hallazgo inicial confirmado con evidencia: el repo base distinguia de facto dos superficies MCP (`HTTP` y `stdio`), pero la documentacion seguia presentandolas como si compartieran catalogo. Ademas, `mcp_stdio.py` importaba `get_stdio_tool_definitions()` sin definicion presente en `mcp_catalog.py` de esta rama base. El slice 0.1 se limita a explicitar y alinear esa superficie antes de tocar contratos, audit trail o retrieval.
- Nota 2026-05-03 19:20Z: Fase 0.1 completada en `fix/mcp-phase-0-1`. Resultado: el repo ya separa explicitamente `HTTP MCP` y `stdio MCP` tanto en codigo como en documentacion viva. `mcp_catalog.py` vuelve a definir `get_stdio_tool_definitions()` para la superficie stdio, el guard de `GET /mcp` corta con `406` antes de tocar el transporte si falta `Accept: text/event-stream`, y el manual deja de presentar `consulta_fiscal`/`agente_consulta` como si fueran tools del endpoint HTTP. Evidencia fresca del slice: `python -m pytest apps/api/tests/test_mcp_private.py apps/api/tests/test_mcp_contract.py apps/api/tests/test_agent_layer.py -q` -> `25 passed`.
- Nota 2026-05-03 20:35Z: Fase 0.2 completada en `fix/mcp-phase-0-1`. Resultado: `query_audit` fija ya el contrato minimo obligatorio de respuesta MCP. `apps/api/services/query_audit.py` persiste y normaliza `tool_name`, `sources`, `confidence`, `completeness` y `verified`; `apps/api/routers/query_audit.py` expone ese contrato completo; `apps/api/schemas.py` centraliza el modelo compartido; `apps/api/mcp_catalog.py` infiere nombres estables por superficie; y `apps/api/mcp_stdio.py` deja constancia explicita cuando una llamada queda no verificada. Evidencia fresca del slice: `python -m pytest apps/api/tests/test_query_audit.py apps/api/tests/test_query_audit_http.py -q` -> `13 passed`; `python -m pytest apps/api/tests/test_governance_http.py -q` -> `16 passed`.
- Nota 2026-05-03 21:25Z: Fase 0.3 completada en `fix/mcp-phase-0-1`. Resultado: ya existe una suite fija de regresion para preguntas MCP historicamente peligrosas y el runtime responde en fail-closed con senal explicita `NO VERIFICADO` cuando falta base suficiente. Ademas, `modelos` deja de aparentar completitud: `apps/api/tests/test_mcp_truth_regressions.py` fija 5 preguntas de alto riesgo y exige abstencion con `NO VERIFICADO`; `apps/api/tests/test_modelos_truth_contract.py` fija que un modelo sin `instrucciones` o `casillas` oficiales suficientes quede `completeness="parcial"` y `verified=false`; `apps/api/routers/consulta.py` y `apps/api/services/grounding.py` unifican el aviso conservador; `apps/api/routers/modelos.py`, `apps/api/services/modelos.py` y `apps/api/schemas.py` exponen el contrato explicito de verdad/completitud para detalle y campana operativa. Evidencia fresca del slice: `python -m pytest apps/api/tests/test_mcp_truth_regressions.py apps/api/tests/test_modelos_truth_contract.py -q` -> `7 passed`; `python -m pytest apps/api/tests/test_smoke.py -q -k "modelo_campana_operativa or modelo_articulos_endpoint or modelo_inexistente_404"` -> `3 passed`; `python -m pytest apps/api/tests/test_integration.py -q -k "modelos_get_detail or modelos_campana_operativa"` -> `2 passed`.
- Nota 2026-05-03 22:05Z: Fase 1.1 completada en `fix/mcp-phase-0-1`. Resultado: la superficie HTTP MCP prioritaria ya deja rastro auditable completo en runtime real. Se añade `apps/api/tests/test_http_mcp_audit_phase_1_1.py` con 10 casos end-to-end que fijan audit trail para `buscar_legislacion`, `get_norma`, `get_articulo`, `get_articulo_historial`, `get_doctrina`, `get_modelo`, `get_modelo_casillas`, `get_modelo_claves`, `get_modelo_instrucciones` y `get_modelo_fuentes_oficiales`; `apps/api/routers/buscar.py` audita tambien el alias `/v1/legislacion/buscar`; `apps/api/routers/legislacion.py`, `apps/api/routers/doctrina.py` y `apps/api/routers/modelos.py` persisten `request_id`, `tool_name`, `sources`, `confidence`, `completeness` y `verified` por endpoint sin depender de inferencia por path. Evidencia fresca del slice: `python -m pytest apps/api/tests/test_http_mcp_audit_phase_1_1.py -q` -> `10 passed`; `python -m pytest apps/api/tests/test_query_audit.py apps/api/tests/test_query_audit_http.py -q` -> `13 passed`; `python -m pytest apps/api/tests/test_integration.py -q -k "modelos_get_detail or modelos_campana_operativa or doctrina_search_returns_results"` -> `3 passed`; `python -m ruff check apps/api/routers/buscar.py apps/api/routers/legislacion.py apps/api/routers/doctrina.py apps/api/routers/modelos.py apps/api/tests/test_http_mcp_audit_phase_1_1.py` -> `All checks passed!`. Riesgo residual explicitado: sigue existiendo un rojo heredado en `apps/api/tests/test_smoke.py::test_doctrina_buscar_filtra_por_tipo` por mezcla `doctrina + BOE` cuando `include_boe=true`; no se ha tocado en este slice.
- Nota 2026-05-03 22:20Z: Fase 1.2 completada en `fix/mcp-phase-0-1`. Resultado: la superficie `stdio` ya deja audit trail E2E correlado con el runtime REST en lugar de una huella sintetica aislada. `apps/api/mcp_stdio.py` genera un `request_id` real por `tools/call`, lo propaga a cada subrequest interno con `x-request-id` y `x-user-id`, reutiliza la entrada HTTP correlada para persistir en `/mcp/tools/<tool_name>` los `retrieved_chunks`, `sources`, `confidence`, `completeness`, `verified`, `grounding_status` y `grounding_summary` reales, y deja de tragarse fallos de auditoria devolviendo `-32603` explicito cuando no puede persistir la fila stdio. Se añade `apps/api/tests/test_mcp_stdio_audit.py` con 3 casos que fijan correlacion `stdio -> REST` para `consulta_fiscal` y `agente_consulta`, herencia del audit trail real y fail-closed ante error de persistencia. Evidencia fresca del slice: `python -m pytest apps/api/tests/test_query_audit.py apps/api/tests/test_mcp_stdio_audit.py apps/api/tests/test_agent_layer.py -q` -> `27 passed`; `python -m ruff check apps/api/mcp_stdio.py --select F` -> `All checks passed!`; `python -m ruff check apps/api/tests/test_mcp_stdio_audit.py --select E,F` -> `All checks passed!`.
- Riesgo residual explicitado: siguen existiendo rojos heredados fuera de scope en pruebas antiguas del transporte HTTP `/mcp` (`apps/api/tests/test_mcp_audit.py`, `apps/api/tests/test_mcp_transport.py`) porque mezclan surfaces ya separadas (`consulta_fiscal` via HTTP MCP) o dependen de inicializacion parcial del `StreamableHTTPSessionManager`; no son regresion del slice `stdio`.
- Nota 2026-05-03 22:42Z: Fase 1.3 completada en `fix/mcp-phase-0-1`. Resultado: `/v1/ai/query-audit` ya queda fijado con evidencia como superficie operativa de confianza real, no solo de contrato nominal. El runtime no requirio refactor adicional: `apps/api/routers/query_audit.py` y `apps/api/schemas.py` ya exponian `grounding_status`, `prompt_injection_detected`, `grounding_summary`, `completeness` y `verified`; el cierre del slice anade cobertura HTTP en `apps/api/tests/test_query_audit_http.py` para demostrar que esos campos salen con valores reales persistidos por `/v1/consulta`, tanto en listado filtrado por `path` como en lookup por `request_id`. Evidencia fresca del slice: `python -m pytest apps/api/tests/test_query_audit.py apps/api/tests/test_query_audit_http.py -q` -> `15 passed`; `python -m pytest apps/api/tests/test_query_audit_http.py -q -k "runtime_grounding_and_truth_fields"` -> `2 passed`; `python -m ruff check apps/api/tests/test_query_audit_http.py --select E,F` -> `All checks passed!`.
- Nota 2026-05-03 23:05Z: Fase 1.4 completada en `fix/mcp-phase-0-1`. Resultado: `query_audit_log` ya persiste `response_payload` suficiente para reconstruir lo que vio el usuario en `/v1/consulta` y en la superficie `stdio`. Se añade migracion `alembic/versions/20260503_0055_query_audit_response_payload.py`; `apps/api/services/persistence.py` repara en runtime la nueva columna para SQLite/tests y deriva de esquemas legacy; `apps/api/services/query_audit.py` persiste/mapea `response_payload`; `apps/api/routers/consulta.py` guarda el `ConsultaFiscalResponse` final serializado exactamente como sale al cliente; y `apps/api/mcp_stdio.py` guarda el payload JSON-RPC final emitido por `tools/call`. La fase no amplía `apps/api/routers/query_audit.py` ni `apps/api/schemas.py`: el plan pedia persistencia, no exposicion del campo nuevo por la superficie HTTP de auditoria. Evidencia fresca del slice: `python -m pytest apps/api/tests/test_query_audit.py apps/api/tests/test_mcp_stdio_audit.py -q` -> `10 passed`; `python -m pytest apps/api/tests/test_query_audit_http.py -q` -> `8 passed`. Nota operativa: en Windows estas suites deben correr en secuencial por el SQLite compartido de `apps/api/tests/conftest.py`.
- Nota 2026-05-03 23:20Z: Fase 2.1 `[COMPLETA]` cerrada en `fix/mcp-phase-0-1`. Resultado: `apps/api/db.py` fija `SessionLocal(... close_resets_only=False)` para que `Session.close()` sea terminal y aflore el reuso invalido de sesiones; `apps/api/routers/consulta.py` limita la DB al scope vivo y reabre una sesion propia para el bloque tardio de modelos; `apps/api/routers/legislacion.py` y `apps/api/routers/jurisprudencia.py` sustituyen usos de `next(get_db())` por `with db_session() as db:`. En el cierre de review del mismo slice tambien se corrigieron dos huecos conectados a la cadena de confianza MCP: `apps/api/routers/modelos.py` ya audita tambien respuestas `200` vacias cuando una campana no existe, `alembic/versions/20260503_0055_query_audit_response_payload.py` ya migra las columnas del contrato MCP pendientes (`tool_name`, `sources`, `confidence`, `completeness`, `verified`) y `apps/api/services/query_audit.py` deja de ejecutar DDL al import con singleton lazy. Ademas, `apps/api/routers/consulta.py` ahora normaliza `relevance_score`/`claim confidence` y conserva `source_url` en citas serializadas. Para mantener verde el slice real tambien se alinean `apps/api/main.py`, `apps/api/schemas.py`, `apps/api/tests/conftest.py`, `apps/api/tests/test_integration.py`, `apps/api/tests/test_reranker.py` y `apps/api/tests/test_db.py` con el contrato/runtime actual del API. Evidencia fresca: `python -m pytest apps/api/tests/test_db.py -q` -> `1 passed`; `python -m pytest apps/api/tests/test_integration.py apps/api/tests/test_query_audit.py apps/api/tests/test_reranker.py apps/api/tests/test_http_mcp_audit_phase_1_1.py apps/api/tests/test_alembic_integrity.py -q` -> `61 passed`; `python -m ruff check apps/api/db.py apps/api/main.py apps/api/mcp_stdio.py apps/api/routers/buscar.py apps/api/routers/consulta.py apps/api/routers/doctrina.py apps/api/routers/jurisprudencia.py apps/api/routers/legislacion.py apps/api/routers/modelos.py apps/api/routers/query_audit.py apps/api/schemas.py apps/api/services/grounding.py apps/api/services/modelos.py apps/api/services/persistence.py apps/api/services/query_audit.py apps/api/tests/conftest.py apps/api/tests/test_alembic_integrity.py apps/api/tests/test_db.py apps/api/tests/test_http_mcp_audit_phase_1_1.py apps/api/tests/test_integration.py apps/api/tests/test_query_audit.py apps/api/tests/test_query_audit_http.py apps/api/tests/test_reranker.py --select F,I` -> `All checks passed!`. Nota operativa: en Windows estas suites deben ejecutarse en secuencial por el SQLite compartido de `apps/api/tests/conftest.py`.
- Nota 2026-05-04 00:05Z: Fase 2.2 `[COMPLETA]` cerrada en `fix/mcp-phase-0-1`. Resultado: `apps/api/routers/consulta.py` deja de tragarse fallos criticos del retrieval integrado y entra en abstencion conservadora explicita (`200` + `NO VERIFICADO`, `review_required=true`, `faithfulness_score=0.0`, `resultados/cited_chunks/claim_citations=[]`) cuando falla `search_legislacion` o el retrieval unificado solicitado por `sources`. `apps/api/services/unified_multi_source_search.py` deja de ocultar errores por fuente y ahora devuelve `source_errors` estructurado para que el router pueda cerrar en fail-closed sin perder trazabilidad minima del fallo. Se añade `apps/api/tests/test_consulta_fail_closed.py` y se amplía `apps/api/tests/test_unified_multi_source_search.py` para fijar ambos contratos. Evidencia fresca: `python -m pytest apps/api/tests/test_consulta_fail_closed.py apps/api/tests/test_unified_multi_source_search.py apps/api/tests/test_query_audit.py apps/api/tests/test_reranker.py apps/api/tests/test_mcp_truth_regressions.py -q` -> `70 passed`; `python -m ruff check apps/api/routers/consulta.py apps/api/services/unified_multi_source_search.py apps/api/tests/test_consulta_fail_closed.py apps/api/tests/test_unified_multi_source_search.py --select F,I` -> `All checks passed!`.
- Nota 2026-05-04 00:35Z: Fase 2.3 `[COMPLETA]` cerrada en `fix/mcp-phase-0-1`. Resultado: `apps/api/services/unified_multi_source_search.py` deja de mezclar todos los dominios 31.x cuando el usuario pide solo uno: el dispatcher ahora pasa `source` explicito al handler 31.x y `_31x_fulltext`/`_31x_vector` filtran por `df.documento_origen_tipo = :source_type` en vez de usar un `IN (...)` global. En el mismo slice se corrigen los bugs concretos del SQL 31.x (`LOWER(df.texto)` en vez de alias roto y `:_31x_ts_query` en vez de `:ts_query`) y se elimina el swallowing interno de fallos de embedding en los helpers vectoriales para que el agregador pueda reportarlos via `source_errors`. `apps/api/tests/test_unified_multi_source_search.py` fija los tres contratos: SQL/parametros correctos para 31.x, filtrado real por dominio pedido y propagacion de errores de embedding hasta `source_errors`; `apps/api/tests/test_consulta_fail_closed.py` sigue verde como guardarrail del contrato fail-closed de 2.2. Evidencia fresca del slice: `python -m pytest apps/api/tests/test_unified_multi_source_search.py apps/api/tests/test_consulta_fail_closed.py -q` -> `49 passed`; `python -m ruff check apps/api/services/unified_multi_source_search.py apps/api/tests/test_unified_multi_source_search.py --select F,I` -> `All checks passed!`.
- Nota 2026-05-04 01:10Z: Fase 2.4 `[COMPLETA]` cerrada en `fix/mcp-phase-0-1`. Resultado: la claim-level abstention ya usa una sola implementacion compartida en runtime real. `apps/api/services/grounding.py` pasa a devolver en `grounding_summary` el contexto enriquecido que su propio `apply_claim_level_abstention()` necesita, y el helper acepta opcionalmente `enriched_items` explicitos para consumir directamente la salida de `validate_claim_grounding()`. `apps/api/routers/consulta.py` deja de mantener logica propia paralela para el filtrado por claims y delega `_apply_claim_level_abstention(...)` en `services.grounding.apply_claim_level_abstention(...)`, reutilizando los `claim_citations` enriquecidos y preservando el `aviso`/estado de confianza del servicio compartido. `apps/api/tests/test_grounding_e2e.py` añade un caso de pipeline real `validate_claim_grounding -> apply_claim_level_abstention` sin `_enriched_items` fabricados y `apps/api/tests/test_reranker.py` fija que `consulta` delega ese filtrado al servicio compartido. Evidencia fresca del slice: `python -m pytest apps/api/tests/test_grounding_e2e.py -q` -> `34 passed`; `python -m pytest apps/api/tests/test_reranker.py -q -k "delegates_claim_level_abstention_to_grounding_service or grounding_abstention_keeps_official_full_article_results_when_faithfulness_is_high or grounding_abstention_rejects_results_when_a_query_term_is_missing_from_all_results"` -> `3 passed, 8 deselected`; `python -m ruff check apps/api/services/grounding.py apps/api/routers/consulta.py apps/api/tests/test_grounding_e2e.py apps/api/tests/test_reranker.py --select F,I` -> `All checks passed!`. Nota operativa: en Windows esta verificacion debe correr en secuencial; lanzar dos `pytest` en paralelo reabre la trampa conocida de `tests/test_esdata.sqlite3`.
- Nota 2026-05-04 01:45Z: Fase 2.5 `[COMPLETA]` cerrada en `fix/mcp-phase-0-1`. Resultado: `faithfulness_score` deja de tener poder decisional duro en `/v1/consulta` y pasa a ser una señal advisory mientras siga calculandose sobre un `answer_proxy` y no sobre la respuesta final real. `apps/api/routers/consulta.py` ya no usa `check_review_required(...)` ni umbrales de faithfulness para vaciar resultados, hacer bypass de grounding o disparar `review_required`; las decisiones duras vuelven a depender de senales de evidencia real (`NO VERIFICADO`, fail-closed por retrieval, ausencia de grounding/citas y ausencia final de evidencia para consultas con modelos resueltos). `apps/api/services/faithfulness.py` se mantiene estable como scorer auxiliar y `apps/api/tests/test_faithfulness.py` se amplía para fijar que un `faithfulness_score` bajo por si solo no provoca abstencion ni `review_required`. Evidencia fresca del slice: `python -m pytest apps/api/tests/test_faithfulness.py -q` -> `6 passed`; `python -m pytest apps/api/tests/test_reranker.py -q -k "grounding_abstention_keeps_official_full_article_results_when_faithfulness_is_high or grounding_abstention_rejects_results_when_a_query_term_is_missing_from_all_results or consulta_out_of_scope_abstains_even_if_model_suggestions_exist"` -> `3 passed, 8 deselected`; `python -m pytest apps/api/tests/test_mcp_truth_regressions.py apps/api/tests/test_consulta_fail_closed.py -q` -> `6 passed`; `python -m ruff check apps/api/services/faithfulness.py apps/api/routers/consulta.py apps/api/tests/test_faithfulness.py --select F,I` -> `All checks passed!`. Nota operativa: en Windows la verificacion sigue siendo secuencial por el SQLite compartido de `apps/api/tests/conftest.py`.
- Nota 2026-05-04 02:20Z: Fase 3.1 `[COMPLETA]` cerrada en `fix/mcp-phase-0-1`. Resultado: el repo MCP ya fija una sola via canonica de seed AEAT y deja visibles las rutas legacy/no autoritativas sin romper compatibilidad. `scripts/seed-modelos.py` queda documentado como paso 1 canonico y `scripts/seed-modelos-v2.py` como paso 2 canonico; ambos explicitan precondiciones y modo seguro. `scripts/data/seed_modelos.py`, `scripts/data/seed_aeat_models.py`, `scripts/data/seed_modelo_articulo.py` y `scripts/seed-fiscal-modelos.sql` quedan marcados como `LEGACY / NO AUTORITATIVO`, y `scripts/data/seed_all.py` advierte explicitamente que no es el flujo productivo canonico AEAT. Se añade `scripts/tests/test_aeat_seed_canonical_flow.py` para fijar por TDD el contrato textual minimo de canonico vs legacy. Evidencia fresca: `python -m pytest scripts/tests/test_aeat_seed_canonical_flow.py -q` -> `3 passed`. Riesgo residual explicitado: `scripts/seed-modelos-v2.py` sigue dependiendo de `aeat_modelo` y por tanto no es un bootstrap standalone; el endurecimiento del linking de `modelo_articulo` queda para Fase 3.2.
- Nota 2026-05-04 09:05Z: Fase 3.2 `[COMPLETA]` cerrada con evidencia fresca del set 3.2 completo y secuencial en Windows/SQLite. Resultado: `modelo_articulo` persiste ya `(norma, numero, metodo_enlace, confianza_enlace)` en escritura, las filas legacy sin enlace fuerte quedan ocultas del runtime, y `articulos_count` cuenta solo enlaces fuertes. Evidencia exacta del cierre: `python -m pytest scripts/tests/test_seed_modelo_articulo.py -q` -> `5 passed`; `python -m pytest scripts/tests/test_aeat_seed_canonical_flow.py -q` -> `4 passed`; `python -m pytest apps/api/tests/test_modelos_truth_contract.py -q` -> `5 passed`; `python -m pytest apps/api/tests/test_smoke.py -q -k "test_modelo_articulos_endpoint"` -> `1 passed`; `python -m pytest apps/api/tests/test_alembic_integrity.py -q` -> `6 passed`.
...
- Riesgos restantes:
  ...
  - la propagacion efectiva a produccion requiere cargar `/etc/esdata/esdata.env` real del host y provisionar URLs reales de `HC_PING_URL_CRON_*`

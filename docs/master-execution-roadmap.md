...
- 2026-05-24 - `[COMPLETADO LOCAL / PENDIENTE PR+VPS]` Familia AEAT capital mobiliario `123/124/193`. Alcance: endurecer `/v1/modelos/por-supuesto` sin promover obligaciones ni cargar nuevas fuentes; `193` queda como referencia saneada, `123/124` siguen `partial`. Auditoria local/VPS previa: `123/124` no tienen filas `obligacion_perfil` ni `safe_to_answer=true`, pero `124` aparecia como candidato para `clientes_residentes=true` y `tipo_renta=capital_mobiliario` aunque la fuente AEAT lo acota a transmision, amortizacion, reembolso, canje o conversion de activos financieros. Cambio local: regresion para excluir `124` en capital mobiliario residente generico y permitirlo solo con `tipo_operacion` especifica de activos financieros; doc nuevo `docs/aeat-capital-mobiliario-123-124-193-routing.md`; matriz/closeout actualizados; `prd.json` Sprint R queda con `R-01 passes=true`. Validacion: RED confirmado; `python -m pytest apps/api/tests/test_modelos_truth_contract.py apps/api/tests/test_mcp_tool_layer_separation.py apps/api/tests/test_modelo_obligation_context.py -q` => `35 passed`; `python -m ruff check apps/api/services/modelos.py apps/api/tests/test_modelos_truth_contract.py --select F,I` => OK; `python scripts/maintenance/verify-doc-contracts.py` => OK; `git diff --check` => OK. Siguiente paso exacto: commitear, abrir PR y validar en VPS que `124` queda excluido en supuesto residente generico y candidato solo con activos financieros especificos.

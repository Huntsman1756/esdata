...
- 2026-05-18 20:30 Europe/Madrid - `[EN CURSO LOCAL]` Sprint I RTS 1/RTS 2 MiFIR en rama `feat/sprint-i-rts1-rts2`. I-01 completado: Docker Desktop reparado tras diagnosticar `Wsl/Service/CreateInstance/MountDisk/HCS/E_ACCESSDENIED` en `D:\Docker\DockerDesktopWSL\main\ext4.vhdx` y corregir ACL de VM; RTS 1 `32017R0587` y RTS 2 `32017R0583` verificados en EUR-Lex y cargados en `norma` mediante seed idempotente con `norma_padre_celex='32014R0600'`. Verificacion local Compose/Postgres: `RTS 1+2: 2`, `PASS`. Caveat: la DB local `deploy` estaba vacia antes del seed (`norma=0`, `obligacion_perfil=0`), no replica el corpus completo del prompt; siguiente paso exacto: I-02, cargar o confirmar prerequisitos de perfiles/base MiFIR y sembrar obligaciones condicionadas de `sociedad_valores`.

...
- 2026-05-29 - `[EN CURSO]` `ESMA-MIFIR-REPORTING-429-RECOVERY-01`: cerrar unidad fallida `cron-esma-mifir-reporting-weekly`. Causa VPS reproducida en relanzamiento manual: ESMA devuelve HTTP `429 Too Many Requests` durante `fetch_reporting_documents()` tras varias descargas consecutivas. Alcance: anadir retry/backoff respetando `Retry-After` y throttle minimo entre peticiones ESMA; reconstruir `cron-esma-mifir-reporting-weekly` y validar systemd. No cambiar contratos de datos ni degradar errores HTTP no-429.

...
- 2026-05-24 - `[EN CURSO]` `MCP-DATA-03`: aplicabilidad fiscal y routing de perfil para `sociedad_valores`, usando Modelo 202 como obligacion fiscal de prueba. RED primero: confirmar en VPS/read-only `modelo_202_all_profiles_loaded`, `perfil_sociedad_valores_fiscal_routing_contract` y el arrastre en `profile_applicability_contracts`; extraer estado real de `obligacion_perfil` 202 por perfil y respuesta API de `/v1/perfil/sociedad_valores/obligaciones?dominio=FISCAL` + `/v1/modelos/aeat/202`. No tocar CASP, emisor token ni `all_profiles_pct_verified_ge_70` como objetivo directo; no promover `verified=true` sin `source_hash`, `capture_date` y fuente primaria.

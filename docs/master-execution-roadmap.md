...
- 2026-05-31 - `[VALIDADO LOCAL]` `GH-SCRAPER-AEPD-BDE-01`: implementacion de las fases seguras derivadas de `GH-PUBLIC-SCRAPER-REVIEW-01` para mejorar fuentes parciales sin copiar codigo externo. Cambios locales: `apps/workers/aepd.py` anade discovery de resoluciones AEPD desde RSS oficial y enumeracion acotada opcional por patron de PDF, con limites `AEPD_MAX_*` y sin elevar `coverage_status=partial`; `apps/workers/bde.py` anade discovery opcional de circulares BdE desde el listado oficial vigente, metadata de discovery si la tabla tiene columna `metadata`, telemetria partial cuando no descubre URLs, y conserva seeds existentes. `infra/deploy/docker-compose.prod.yml`, `infra/deploy/compose.env.example`, `.env.example` y docs de entorno/despliegue cablean `AEPD_DISCOVER_RESOLUTIONS`, `AEPD_ENUMERATE_RESOLUTIONS`, `BDE_DISCOVERY_ENABLED` y limites operativos. Validacion local: tests focales AEPD/BDE `26 passed`; `py_compile` OK; Ruff focal OK; `verify-doc-contracts.py` OK; `source_assurance_gate.py` OK; `docker compose ... config --quiet` OK; tests focales deploy `4 passed`; `git diff --check` OK. Pendiente: commit/push, deploy VPS, run manual de `cron-aepd-weekly`/`cron-bde-weekly` y gates MCP.

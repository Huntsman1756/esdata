...
- 2026-05-17 11:55 Europe/Madrid - `[EN CURSO LOCAL]` Sociedad de Valores Wave 1 source expansion: repo implementa soporte inicial para los cinco bloques priorizados antes de cargar produccion: `RD_813_2023` (`BOE-A-2023-22763`) en `apps/workers/boe.py` + alias search; CNMV familias `normativa_esi`/`modelos_esi` con tipos `normativa_esi_cnmv` y `modelo_esi_cnmv`; SEPBLAC discovery por defecto de normativa nacional/comunitaria y obligaciones oficiales; nuevo worker `apps/workers/eu_sanctions.py` + cron `cron-eu-sanctions-weekly` para XML oficial FSF de la Comision Europea; ESMA MiFIR reporting amplia documentos a hub, ISRB Article 26 y Q&A index. Docs actualizadas: `docs/source-expansion-backlog-2026-05-17.md` y `docs/source-manifests/sociedad-valores-wave-1.md`. Verificacion local: `pytest apps/workers/tests/test_cnmv.py apps/workers/tests/test_sepblac.py apps/workers/tests/test_eu_sanctions.py apps/workers/tests/test_worker_esma_mifir_reporting.py apps/workers/tests/test_boe.py::test_default_normas_include_itpajd -q` => `86 passed, 1 skipped`; `ruff check ... --select F,I` OK; `py_compile` focal OK. Pendiente: ejecutar workers en VPS y validar filas reales; EU sanctions requiere confirmar URL XML FSF alcanzable/configurada porque el portal oficial puede devolver 403/500 sin token/sesion.

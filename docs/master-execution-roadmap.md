...
- 2026-06-01 - `[EN CURSO]` `AEAT-CAMPAIGN-EVIDENCE-LANES-SCHEMA-01`: cambio de contrato solicitado tras separar campaña legal, campaña operativa y vigencia de diseño tecnico. Alcance: convertir `campana_evidence` en objeto compuesto derivado con carriles `legal`, `operational` y `design`, conservar campos planos legacy como derivados, y evitar que cobertura tecnica por si sola active `campana_safe_to_assert`. Archivos reclamados: `apps/api/services/modelos.py`, `apps/api/schemas.py`, `apps/api/tests/test_modelos_truth_contract.py`, `docs/master-execution-roadmap.md`.

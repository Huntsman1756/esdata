...
- 2026-06-01 - `[EN CURSO]` `AEAT-MODELO-303-DIRECT-CAMPAIGN-ASSERTION-01`: continuacion solicitada por el usuario sobre modelos de Hacienda, empezando por el 303. Objetivo: usar instrucciones/formularios oficiales con metadato explicito de campana y hash como prueba directa de campana, sin permitir que disenos tecnicos o recursos transaccionales promuevan campanas por inferencia. Archivos reclamados: `apps/api/services/modelos.py`, `apps/api/tests/test_modelos_truth_contract.py`, `scripts/maintenance/mcp_validation_suite.py`, `scripts/tests/test_maintenance_agents.py`, `docs/master-execution-roadmap.md`. Validacion local inicial: test rojo reprodujo `campana_afirmable=None`; cambio minimo deja `303` `resolved_strong` solo cuando hay `modelo_recurso:instrucciones` directo; tests focales `66 passed`; Ruff focal OK; `py_compile` OK; `git diff --check` OK.

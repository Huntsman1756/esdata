...
- 2026-05-12 20:35 Europe/Madrid - `[EN CURSO LOCAL]` S-13 Ralph, discovery oficial AEPD: `apps/workers/aepd.py` deja de depender solo de `AEPD_SEED_URLS`, consulta por defecto `https://www.aepd.es/guias-y-herramientas/guias`, extrae enlaces oficiales `/documento/*.pdf`, `/guias/*.pdf` y paginas internas de guias, filtra externos/filtros/obsoletos/feed, deduplica y limita por `AEPD_MAX_URLS_PER_RUN`/`AEPD_DISCOVERY_PAGES`. Compose prod pasa `AEPD_SEED_URLS` a opcional y declara `AEPD_DISCOVER_FROM_INDEX`, `AEPD_GUIDES_INDEX_URL`, `AEPD_MAX_URLS_PER_RUN`, `AEPD_DISCOVERY_PAGES`. Si no hay URLs escribe `sync_log status=partial` en vez de salir silenciosamente; si fallan URLs individuales, continua y marca `partial` con contador de errores. Evidencia oficial local: portal AEPD `Guías` responde `200 OK`, indica `107 resultados` y expone documentos descargables oficiales. Evidencia local: probe discovery devuelve PDFs oficiales AEPD; `pytest apps\workers\tests\test_aepd.py -q --basetemp .pytest-tmp` => `8 passed`; `ruff check apps\workers\aepd.py apps\workers\tests\test_aepd.py --select F,I` => OK; `compileall` OK. Pendiente inmediato: commit, push, deploy VPS, run acotado `cron-aepd-weekly`, verificar filas `tipo_fuente='aepd'`, `/v1/aepd`, `/status`, `mcp_validation_suite` y deep audit.

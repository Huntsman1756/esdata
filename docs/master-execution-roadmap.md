...
- Nota 2026-05-04: el estado activo de este bloque queda resumido en las lineas inmediatas siguientes; el resto del bloque se conserva como evidencia de slices ya cerrados.
- Objetivo actual: **Fase 3.2** del plan MCP: endurecer `modelo_articulo` para exigir `(norma, numero)` y provenance fuerte.
- Estado actual: `fix/mcp-phase-0-1` deja cerradas las fases 0.1-3.1 del plan MCP con evidencia fresca y el merge local sobre `main` queda en remate final dentro de `G:\_Proyectos\esdata\.worktrees\next-task`.
- Estado del agente activo: conflictos de codigo del merge ya resueltos y verificados; queda pendiente solo el cierre documental minimo del roadmap y la creacion del merge commit local, sin `push`.
- Reclamo actual: cierre local de `fix/mcp-phase-0-1 -> main`; siguiente slice pendiente tras el merge: **Fase 3.2** del plan MCP.
...
- Nota 2026-05-03 19:05Z: Fase 1.4 `[EN CURSO]` reclamada en `fix/mcp-phase-0-1`. Archivos activos del slice: `apps/api/services/query_audit.py`, `apps/api/services/persistence.py`, `apps/api/routers/consulta.py`, `apps/api/mcp_stdio.py`, `apps/api/tests/test_query_audit.py`, `apps/api/tests/test_mcp_stdio_audit.py`, `alembic/versions/20260503_0055_query_audit_response_payload.py`. Objetivo exacto: persistir en `query_audit_log` un `response_payload` suficiente para reconstruir lo que vio el usuario en `/v1/consulta` y en `stdio`, con TDD primero y migracion Alembic minima.
...
- Siguiente paso exacto: ejecutar **Fase 3.2** del plan MCP: endurecer `modelo_articulo` para exigir `(norma, numero)` y provenance fuerte.

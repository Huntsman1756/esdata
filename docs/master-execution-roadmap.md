...
- 2026-05-24 - `[EN CURSO LOCAL]` Sprint P Modelo 193 rentas residentes. Se ejecuta el candidato seleccionado en O-03 sin abrir Modelo 100 ni recalcular retenciones. Auditoria produccion: Modelo `193` esta `complete`/`curado` con `71` casillas, `38` claves, `5` instrucciones y recursos con hash; dividendos residentes quedan trazados por doble clave `PERCEPCION_A` + `NAT_A_02`; intereses residentes por `PERCEPCION_B` + `NAT_BD_01`; todas las claves auditadas tienen URL, `source_hash` y `capture_date=2026-05-14`. Cambios locales: migracion `20260524_0090_aeat_193_income_type_rules` persiste reglas `CONDICIONAL`; `/v1/modelos/por-supuesto` proyecta evidencia de `modelo_clave` para `193` solo con doble clave hash/captura y mantiene `evidence_limited`, `verified=false`, `review_required=true`; docs nuevas en `docs/aeat-modelo-193-rentas-residentes-contract.md`; `prd.json` reemplazado por sprint P con `P-01 passes=true`. Validacion focal inicial: RED para `193` confirmado y GREEN `python -m pytest apps/api/tests/test_modelos_truth_contract.py -k "193_income_key or 296_income_key" -q` => `3 passed`. Siguiente paso exacto: ejecutar suite focal completa, commitear, abrir PR y desplegar `0090` en VPS.

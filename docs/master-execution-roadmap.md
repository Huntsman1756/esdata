...
- 2026-05-20 06:55 Europe/Madrid - `[BLOQUEADO]` Auditoria stale workers A-04 en rama `fix/full-audit-stale-workers-20260520`. Compose vivo en VPS deriva 14 persistent workers (`worker-*` sin profile cron); ninguno carece de `--run-once` cuando se usa `WORKER_CMD` correctamente. Smoke 300s: 10 OK, 3 timeout (`worker-cnmv`, `worker-dgt`, `worker-modelos`) y 1 fallo real (`worker-eurlex`, duplicado `norma_boe_id_key` para `EUR-CELEX-32014L0065`, seguido de duplicado dead-letter `uq_dead_letter_worker_entity`). A-04 no se marca pass. Informe: `docs/persistent-worker-smoke-20260520.md`. Siguiente paso exacto: decidir si se corrige/defer `worker-eurlex` y si se sube timeout o se crea smoke acotado para workers largos antes de A-05.

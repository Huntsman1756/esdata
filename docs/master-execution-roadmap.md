...
- Siguiente paso exacto: ejecutar **Fase 0.2** del plan MCP: fijar el contrato minimo obligatorio de respuesta MCP (`request_id`, `tool_name`, `sources`, `confidence`, `completeness`, `verified/no_verificado`) y alinear `apps/api/schemas.py`, `apps/api/mcp_catalog.py`, `apps/api/services/query_audit.py`, `apps/api/routers/query_audit.py` y `docs/architecture.md` antes de crear la suite de preguntas de alto riesgo.

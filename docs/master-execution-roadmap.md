...
- 2026-05-31 - `[EN CURSO LOCAL]` `CNMV-SANCIONES-EXPANSION-01`: expansion acotada solicitada por el usuario para mejorar CNMV y ordenar siguientes fuentes sin copiar codigo externo. Alcance actual: `apps/workers/cnmv.py` descubre el registro publico oficial de sanciones CNMV con limite `CNMV_SANCIONES_MAX_PAGES`, normaliza `sancion_cnmv`/`sanciones_cnmv`, conserva severidad de infraccion y referencia BOE cuando existe, y respeta `ambito` de metadata para evitar falsos positivos por heuristica textual. `apps/api/routers/cnmv.py` expone la familia como `partial_loaded`; despliegue cableado en `.env.example`, `infra/deploy/compose.env.example`, `infra/deploy/docker-compose.prod.yml` y `docs/environment-variables.md`. Validacion local parcial: worker CNMV `79 passed, 1 skipped`; API CNMV `14 passed`; Ruff focal `F,I` OK; `py_compile` OK. Pendiente antes de cerrar: compose config, gates docs/source assurance, merge/despliegue VPS, run acotado de `cron-cnmv-weekly` para `sanciones`, `/status`, `/health`, `mcp_validation_suite` y `mcp_deep_contract_audit`.

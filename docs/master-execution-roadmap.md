...
- 2026-06-05 - `[EN CURSO]` `HERMES-MONITOR-STALE-CONTAINER-0122`: diagnostico de Hermes tras run nocturno `2026-06-05 03:41-03:46 CEST`. Causa del limite de 3 modelos: guardarrail deliberado `HERMES_ESDATA_MAX_MODELS_PER_RUN=3` en `hermes-esdata-curator.service`, introducido para rate-limit/coste; no es fallo de cobertura. Hallazgo monitor: contenedor `deploy-hermes-1` lleva imagen antigua y registra parciales como `within grace period (95.0h < 2h)` aunque `AUTO_RESTART_ENABLED=false`; el checkout actual ya tiene rama de logging correcta para `auto-restart disabled`. Cambio local: regression test `test_hermes_monitor_logs_restart_disabled_before_grace` para impedir volver a clasificar ese caso como grace period. Pendiente: commit, rebuild/recreate solo `hermes` en VPS y validar logs vivos.

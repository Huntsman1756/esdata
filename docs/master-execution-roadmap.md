...
- Nota 2026-05-06 06:37Z: remediado el drift operativo de scheduler/observabilidad en produccion. El unit instalado `esdata-job@.service` vuelve a `docker compose --env-file /etc/esdata/esdata.env -f /srv/esdata/infra/deploy/docker-compose.prod.yml run --rm %i` sin `--no-deps`; `infra/observability/alerts.yml` ya evalua `worker_stale_status == 1`; `deploy-prometheus-1` se recreo y la regla viva queda verificada por `GET /api/v1/rules` con `WorkerSilent=worker_stale_status == 1`. Se corrige ademas el drift de Compose/systemd: los `cron-*` weekly ya quedan declarados con `depends_on postgres` + `networks: esdata-internal`, y el repo vuelve a contener los timers `esdata-bde-weekly.timer`, `esdata-cendoj-weekly.timer`, `esdata-aepd-weekly.timer` y `esdata-eurlex-weekly.timer` que estaban instalados en el VPS pero ausentes en Git. Reruns secuenciales verificados para `cron-dgt-weekly`, `cron-teac-weekly`, `cron-bdns-weekly`, `cron-borme-weekly`, `cron-bde-weekly`, `cron-cendoj-weekly`, `cron-aepd-weekly` y `cron-eurlex-weekly`: todos cierran con `Result=success`, `ExecMainStatus=0`, nuevas filas `sync_log` `status=ok` entre `2026-05-06 06:19Z` y `2026-05-06 06:35Z`, y sin reaparicion de `not connected to the network deploy_esdata-internal` ni `no such service`. Estado fresco post-fix: `/status` devuelve `ok` y `stale=false` para `cron-dgt-weekly`, `cron-teac-weekly`, `cron-bdns-weekly`, `cron-borme-weekly`, `cron-bde-weekly`, `cron-cendoj-weekly`, `cron-aepd-weekly`, `cron-eurlex-weekly`, `cron-cnmv-weekly` y `cron-sepblac-weekly`; `/metrics` exporta `worker_stale_status{worker="..."} 0.0` para los diez weekly; `/api/v1/alerts` queda sin `WorkerSilent` activo; y `systemctl list-timers --all 'esdata-*'` pasa de 12 a 15 lineas tras reinstalar los timers faltantes en repo/host.

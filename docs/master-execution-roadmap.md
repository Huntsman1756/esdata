...
- 2026-05-24 - `[PLANIFICADO LOCAL]` Sprint O Ralph para desbloquear siguientes historias fiscales tras AEAT IRNR 216/296. Se reemplaza `prd.json` activo por cola Ralph `sprint-o-irnr-rentas-cdi-aeat` con tres historias: O-01 granularizar `216/296` por tipo de renta IRNR empezando por dividendos/intereses; O-02 preparar CDI solo despues de renta domestica trazada; O-03 seleccionar siguiente modelo AEAT limpio sin abrir Modelo 100 por defecto. Prompt activo: `scripts/ralph/sprint-o-irnr-rentas-cdi-aeat.md`; plan operativo: `docs/irnr-rentas-cdi-aeat-ralph-plan.md`; `progress.txt` arranca el nuevo bloque. Regla: una historia por iteracion, audit-first, no `complete` por inferencia, no tocar MCP salvo exposicion fiscal necesaria. Siguiente paso exacto: ejecutar Ralph/Opencode para O-01 o implementar manualmente O-01 con el mismo contrato.

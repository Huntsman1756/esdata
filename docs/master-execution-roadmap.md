...
- 2026-05-23 - `[COMPLETADO LOCAL / PENDIENTE PR+VPS]` Bloque D-02 IVA intracomunitario con fuente oficial real. Auditoria produccion localiza DGT `V0963-25` como fuente principal defendible: documento `complete`, URL PETETE, `source_revision` con SHA-256 y `capture_date`, supuesto expreso de adquisicion intracomunitaria de bienes, `LIVA art. 13` y obligacion de declaracion recapitulativa/modelo 349; `V0236-26` queda descartada porque trata tipo impositivo/`LIVA art. 91`, no supuesto intracomunitario. Cambios locales: D-02 se acota a `adquisicion_intracomunitaria_bienes`, migracion aditiva `20260523_0086_doctrina_d02_intracomunitaria` persiste `documento_articulo` y `criterio_relacion` completa para `V0963-25`, docs/matriz actualizadas. Validacion: `python -m pytest apps/api/tests/test_criterio.py apps/api/tests/test_criterio_curacion.py apps/api/tests/test_doctrina_lineas_contract.py apps/api/tests/test_alembic_integrity.py -q` => `68 passed`; `python -m ruff check apps/api/routers/doctrina.py apps/api/tests/test_doctrina_lineas_contract.py apps/api/tests/test_alembic_integrity.py alembic/versions/20260523_0086_doctrina_d02_intracomunitaria.py --select F,I` => `All checks passed`; `alembic heads` => `20260523_0086_doctrina_d02_intracomunitaria`; `python scripts/maintenance/verify-doc-contracts.py` => `docs contracts verified`; `git diff --check` OK. Siguiente paso exacto: abrir PR, mergear y desplegar 0086 en VPS; validar que `lineas_complete=2`, D-02 `complete/safe_to_answer=true` y que no se extrapola a entregas intracomunitarias.

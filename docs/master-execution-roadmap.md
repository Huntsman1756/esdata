...
- 2026-05-24 - `[EN CURSO LOCAL]` Sprint U fail-closed AEAT `200/202/303`. Alcance: sanear obligaciones legacy persistidas que estaban `safe_to_answer=true` sin `source_hash` o `capture_date`; no promover `200/202/303` a `complete`, no crear obligaciones nuevas y no tocar `349/390`. Auditoria VPS previa: `200` tiene 6807 casillas, 5 instrucciones, 0 reglas y 6 obligaciones seguras sin hash; `202` tiene 118 casillas, 0 instrucciones, 0 reglas y 6 obligaciones seguras sin hash; `303` tiene 432 casillas, 5 instrucciones, 0 reglas y 5 obligaciones seguras sin hash. Cambios locales: revision `20260524_0094_aeat_fail_close_200_202_303_obligations` degrada esas filas a `partial`, `verified=false`, `safe_to_answer=false` y anota motivo en `notas`; doc nuevo `docs/aeat-fail-closed-200-202-303.md`; matriz y `prd.json` actualizados. Validacion inicial: RED confirmado para migracion inexistente; GREEN focal `python -m pytest apps/api/tests/test_alembic_integrity.py::test_aeat_200_202_303_legacy_obligations_fail_closed_in_revision_0094 apps/api/tests/test_modelo_obligation_context.py::test_complete_obligation_context_fails_closed_without_hash_or_capture -q` => `2 passed`. Siguiente paso exacto: ejecutar suite focal, ruff, docs contracts, diff check; commitear, PR, aplicar Alembic en VPS y validar `unsafe_200_202_303=0`.

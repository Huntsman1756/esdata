...
- 2026-06-06 - `[PENDIENTE]` `AEAT-MODELO-124-HERMES-TRACEABILITY-0125`: first-class follow-up para reescribir/verificar el excerpt del report Hermes del modelo `124` antes de cualquier promocion semantica. Origen: QA Hermes 111/211 del `2026-06-05`, donde la adjudicacion global marco `modelo 124` con `needs_report_rewrite`/blocking por trazabilidad de excerpt. Referencia viva: `docs/aeat-curation-modelo-124-2026-05-26.md`. Condicion de cierre: el excerpt reescrito mantiene cadena completa `official_source -> locator -> excerpt -> claim`, el adjudicador deja de clasificar `124` como `rewrite/traceability`, y el proximo run Hermes/adjudication retorna sin alerta de trazabilidad para `124`. Bloquea: cualquier promocion de `124` a `ASSERTABLE_DIRECT_OFFICIAL` o estado equivalente afirmable. No bloquea: trabajos en curso ni la respuesta fail-closed actual para `124`.

- 2026-06-05 - `[COMPLETADO VPS]` `HERMES-AEAT-QA-111-211-0124`: ejecutado run acotado del curator AEAT para modelos `111` y `211` en VPS, usando cola `/tmp/esdata-hermes-qa-111-211.queue` y limite `HERMES_ESDATA_MAX_MODELS_PER_RUN=2`, con checkout `/srv/esdata` en `c3e6745`. Reports nuevos: `modelo-111-20260605-164038.json` y `modelo-211-20260605-164803.json`; summary batch `/srv/esdata/hermes-curator/reports/aeat-campaign-curation-json/_batch-json-20260605-164038-summary.csv` marca ambos `OK`. Adjudicacion con `scripts/maintenance/adjudicate_aeat_hermes_batch.py --verify-sources --latest-per-model` genera historico `/srv/esdata/hermes-curator/reports/aeat-adjudication-metrics/20260605T145143Z.json`: `assertable_candidates_total=0`, `human_review_required_total=0`, `auto_accepted_total=6`, `rewrite_or_reject_total=1`. Resultado focal: `111` y `211` quedan en `decision=UNKNOWN`, `assertion_gate.campana_safe_to_assert=false`, `campana_afirmable=null`, `campana_assertion_code=NOT_ASSERTABLE_INFERRED_INTERNAL`, cero `official_source_claims.proves_campaign=true`, y los JSON no contienen `ASSERTABLE_DIRECT_OFFICIAL`; adjudicador los clasifica como `auto_accept_nonassertable_evidence` en bucket `draft/nonassertable`. Tendencia global `summarize_aeat_adjudication_history.py`: alertas por `blocking_error_ratio=0.142857`, `rewrite_ratio=0.142857` y `repaired_excerpt_ratio=1.285714`, atribuibles al batch global y especialmente a `modelo 124` (`needs_report_rewrite`/blocking), no a 111/211. No se modifica prompt ni codigo en este bloque porque el riesgo consultado queda descartado por evidencia VPS; deuda `124` separada en `AEAT-MODELO-124-HERMES-TRACEABILITY-0125`.

- 2026-06-05 - `[COMPLETADO VPS]` `WORKER-STATUS-FETCH-DIAGNOSTICS-0123`: corregidos dos ruidos operativos detectados por Hermes sin relajar la vigilancia fail-closed. BOE: lock miss esperado de `cron-boe-daily`/`worker-boe` registra `sync_log.status='skipped'`, no `partial`, conservando `error_msg='BOE sync already in progress'`; Hermes no requiere cambio porque solo trata `error`/`partial` y stale como unhealthy. CNMV: el resumen pasa a `Skipped N CNMV candidate URLs after fetch failures` y se persiste `sync_log.diagnostic_details JSONB` con `failed_candidates[{url,status_code,error_type}]` para triage humano sin crear tabla de retries. Migracion Alembic `20260605_0120_sync_log_diagnostic_details` queda como head unico. Commits funcionales: `c80d1e64` BOE y `c61f267e` CNMV. Validacion local: TDD rojo-verde BOE/CNMV; tests focales `176 passed, 1 skipped`; Ruff focal `F,I` OK; `compileall` OK; `alembic heads` unico en `20260605_0120_sync_log_diagnostic_details`; `verify-doc-contracts.py` OK; `verify-doc-artifacts.py --ci-baseline` OK; `source_assurance_gate.py` OK; `git diff --check` sin errores, solo avisos LF/CRLF. Deploy VPS: `git pull --ff-only` en `/srv/esdata`; `ops` reconstruido para ver la nueva revision; Alembic aplica `20260605_0120_sync_log_diagnostic_details (head)` antes de recrear workers; `worker-boe` y `worker-cnmv` reconstruidos y recreados, ambos healthy; smoke BOE run-once concurrente registra `cron-boe-daily | skipped | BOE sync already in progress`; sync CNMV nueva registra `worker-cnmv | partial | Skipped 20 CNMV candidate URLs after fetch failures | has_details=t`; Hermes ciclo posterior baja de 3 unhealthy a 2 al desaparecer `cron-boe-daily`; `systemctl --failed` limpio. Drift VPS no tocado: `infra/deploy/Caddyfile`, `audit_out/` y backups Caddyfile siguen pendientes de limpieza infra.

- 2026-06-05 - `[COMPLETADO VPS]` `HERMES-MONITOR-STALE-CONTAINER-0122`: diagnostico de Hermes tras run nocturno `2026-06-05 03:41-03:46 CEST`. Causa del limite de 3 modelos: guardarrail deliberado `HERMES_ESDATA_MAX_MODELS_PER_RUN=3` en `hermes-esdata-curator.service`, introducido para rate-limit/coste; no es fallo de cobertura. Hallazgo monitor: contenedor `deploy-hermes-1` llevaba imagen antigua y registraba parciales como `within grace period (95.0h < 2h)` aunque `AUTO_RESTART_ENABLED=false`; el checkout actual ya tenia rama de logging correcta para `auto-restart disabled`. Cambio local: regression test `test_hermes_monitor_logs_restart_disabled_before_grace` para impedir volver a clasificar ese caso como grace period. Validacion local: test focal Hermes `4 passed`; Ruff focal `F,I` OK; `verify-doc-contracts.py` OK; `verify-doc-artifacts.py --ci-baseline` OK; `git diff --check` OK. Deploy VPS commit `9ca2c77`: `git pull --ff-only`, rebuild/recreate solo `hermes`; el contenedor nuevo muestra `auto_restart=False`, API health OK, availability OK `empty=92 workflow=53 allowed=4 configured_unavailable=35`, DLQ 0, y los tres parciales (`cron-boe-daily`, `worker-cnmv`, `cron-cnmv-weekly`) se registran ahora como `auto-restart disabled, skipping restart`, no como grace period imposible.

- 2026-06-04 - `[COMPLETADO VPS]` `ESMA-MIFIR-TR-MCP-FCR-AIF-0120`: expuestos `list_esma_mifir_schemas` y `list_esma_mifir_transaction_reporting_fields` en el catalogo HTTP MCP, con `docs/openapi-gpt.json` y `docs/openapi-gpt-3.0.json` regenerados. Regla conceptual fijada en skills/policy: no sembrar obligacion MiFIR TR directa por estado FCR/AIF/SGIIC; condicionar a MiFID top-up/investment firm o ejecutor/transmisor obligado e instrumento reportable MiFIR art. 26. NAV/RTS 22 revisado queda como material de revision pendiente hasta contrato adoptado. Validacion local: TDD rojo->verde `test_mcp_catalog_exposes_expected_core_http_operations`; export OpenAPI full `95 paths/95 operations/108 schemas`; Ruff focal `F,I` OK; `verify-doc-contracts.py` OK; `source_assurance_gate.py` OK; `verify-doc-artifacts.py --ci-baseline` OK; `git diff --check` OK. Deploy VPS: commit `258aba14`, `git pull --ff-only` en `/srv/esdata`, rebuild/recreate `api`, health HTTP OK, OpenAPI servido contiene ambos operationId, endpoint `/v1/esma/mifir/transaction-reporting/fields` autenticado responde `total=168` con `quality_signal=official_esma_xsd`, busquedas ISO `Qty`/`Pric` devuelven campos oficiales, `mcp_validation_suite.py --read-only --base-url http://api:8000` OK y `mcp_deep_contract_audit.py --base-url http://api:8000` OK.

- 2026-06-04 - `[PENDIENTE]` `ESMA-MIFIR-TR-NAV-FIELD-GUIDE-0121`: preparar, cuando exista adopcion formal por Comision y publicacion DOUE del RTS 22/24 revisado, un contrato/field guide TR para instrumentos con precio basado en NAV: campo 33 `Price`, `PNDG` cuando el precio no esta disponible al momento de ejecucion, cancelacion/amendment posterior con precio efectivo, consistencia con cantidad/campo 30 y separacion entre RTS 22 vigente (`Reglamento Delegado 2017/590`) y texto revisado adoptado. Condicion de disparo: revisar cuando aparezca en EUR-Lex/DOUE el Reglamento Delegado que sustituya o modifique 2017/590 para RTS 22; mantener en radar H2 2026 segun calendario ESMA, sin sembrar antes como vigente ni mezclar con la regla FCR/AIF. Requiere fuente oficial adoptada y pruebas MCP/API antes de deploy.

- 2026-06-04 - `[COMPLETADO LOCAL + VPS]` `ESMA-FIRDS-METADATA-ONLY-0119`: corregir el alcance del cron `worker-esma-firds` tras fallo VPS `esdata-job@cron-esma-firds-daily.service` con `ExecMainStatus=137`. Diagnostico VPS: no hay `RuntimeMaxSec` ni `MemoryMax`; kernel confirma OOM a las `2026-06-04 04:20 CEST`, matando `python` con RSS anonimo aprox. `2.8 GB` durante descarga/procesado de `DLTINS_20260603_02of02.zip`; produccion tenia `50` filas `esma_firds_file` y `19.508` filas `esma_firds_instrument`. Decision de alcance: ESData/MCP no necesita ISIN reales FIRDS; solo metadatos oficiales de ficheros, esquemas ESMA, manuales y documentos/reporting. Cambio: `worker_esma_firds.py` queda metadata-only, `/v1/esma/firds/instruments` fail-closed sin consultar/exponer `esma_firds_instrument`, migracion `20260604_0119_firds_metadata_only` purga instrumentos y marca ficheros como no descargados/procesados, y los contratos MCP/deep exigen `official_esma_file_metadata`, `firds_instruments_not_loaded` y `esma_firds_instrument_rows_intentionally_zero`. Validacion local: `test_worker_esma_firds.py` + `test_alembic_integrity.py` + `test_maintenance_agents.py` => `76 passed`; `py_compile` OK; Ruff focal `F,I` OK; `alembic heads` unico en `20260604_0119`; `source_assurance_gate.py` OK; `verify-doc-artifacts.py --ci-baseline` OK; `verify-doc-contracts.py` OK; `git diff --check` OK. Evidencia VPS commit `aab1e69`: `git pull --ff-only`; imagenes `ops`, `api` y `cron-esma-firds-daily` reconstruidas; Alembic en `20260604_0119_firds_metadata_only`; API recreada y `/health` OK; `systemctl start esdata-job@cron-esma-firds-daily.service` ejecuta el camino real del timer y termina `Result=success`, `ExecMainStatus=0`, con log `[run-once] ESMA FIRDS: files=12 instruments=0 mode=metadata_only`; SQL confirma `esma_firds_file=52`, `downloaded_or_processed=0`, `esma_firds_instrument=0`, y ultimo `sync_log` `documentos_processed=12`, `articulos_upserted=0`; `/v1/esma/firds/files` devuelve `quality_signal=official_esma_file_metadata`; `/v1/esma/firds/instruments` devuelve `total=0`, `items=[]`, `quality_signal=firds_instruments_not_loaded`, `safe_to_answer=false`; `mcp_validation_suite.py --read-only --base-url http://api:8000` OK; `mcp_deep_contract_audit.py --base-url http://api:8000` OK; `source_assurance_gate.py` OK; `verify-doc-artifacts.py --ci-baseline` OK; `verify-doc-contracts.py` OK; `esdata-mcp-validation.service` relanzado tras el recreate queda `Result=success`; `systemctl --failed` limpio. Limites: no tocar Caddyfile drift, no cargar FULINS/DLTINS payloads, no borrar tablas de esquemas/manuales/reporting ESMA.

- 2026-06-04 - `[COMPLETADO LOCAL + VPS]` `AEAT-MODELO-193-HAC-1430-CONFLICT-0118`: resolver de forma acotada el `conflict` del modelo `193/2025` promoviendo la fuente legal directa `BOE-A-2025-25389` solo para `193`, con hash estable recapturado desde `doc.php`, `content_length=110846`, `capture_date=2026-06-03` y metadata `campaign_evidence_role='direct_legal'`. Diagnostico: el conflicto actual no lo causa la normativa historica `BOE-A-2024-23244` por si misma, sino la combinacion de evidencia AEAT 2025 con un PDF informativo `193/296` bajo ruta 2024; la fila `BOE-A-2025-25389` ya existe como recurso generico/inactivo y necesita promocion separada tras excluirse conscientemente de `0116`. Cambio: migracion `20260604_0118_promote_193_boe_25389_normativa` anade `modelo_normativa` para `193` y promueve el recurso a `normativa_hac_1430_2025`; `services.modelos` estrecha el carril `operational` para que solo recursos con `proves_campaign=True` puedan ser `resolved_strong_operational`, evitando que el PDF debil de anos mixtos se convierta en base operativa. Gate nuevo: `aeat_modelo_193_boe_25389_legal_conflict_resolution_contract`. Validacion local: `test_modelos_truth_contract.py` + `test_alembic_integrity.py` + `test_maintenance_agents.py` => `118 passed`; `py_compile` OK; Ruff focal `F,I` OK; `alembic heads` unico en `20260604_0118`; `source_assurance_gate.py` OK; `verify-doc-artifacts.py --ci-baseline` OK; `verify-doc-contracts.py` OK; `git diff --check` OK. Evidencia VPS commit `ac51556`: `git pull --ff-only`; imagenes `ops` y `api` reconstruidas; Alembic en `20260604_0118_promote_193_boe_25389_normativa`; API recreada y `/health` OK; `/v1/modelos/193/resumen-operativo` devuelve `resolved_strong`, `campana_afirmable=2025`, `campana_safe_to_assert=true`, `campana_assertion_basis=["legal"]`, `campana_conflict=false`, `campana_conflict_years=[]`, carril legal `source_url=https://www.boe.es/buscar/doc.php?id=BOE-A-2025-25389`, hash `45522b6eed4eca77673bffd87d7a4d744b9195e00ec4594a9fb9ae591b32421a`, `capture_date=2026-06-03`, y carril operativo `none`; SQL confirma una fila `modelo_recurso:normativa_hac_1430_2025` para `193/2025` y conserva dos `modelo_normativa` (`BOE-A-2024-23244` historica y `BOE-A-2025-25389` actualizada); `mcp_validation_suite.py --read-only --base-url http://api:8000` OK con el nuevo contrato; `source_assurance_gate.py` OK; `verify-doc-artifacts.py --ci-baseline` OK; `verify-doc-contracts.py` OK. Limites: no borrar normativa historica, no tocar `obligacion_perfil`, no abrir perfil CRS/DAC2 ni el carril `legal` opcional de otros modelos, no modificar `CAMPAIGN_BEARING_RESOURCE_TYPES`. Nota operativa: `systemctl --failed` no queda vacio por `esdata-job@cron-esma-firds-daily.service`, fallo ajeno al despliegue a las `2026-06-04 04:20 CEST` con `ExecMainStatus=137` durante el job FIRDS; no se hizo `reset-failed` para no ocultarlo.

- 2026-06-03 - `[COMPLETADO LOCAL + VPS]` `AEAT-BOE-STABLE-LEGAL-PROMOTION-0116-0117`: promocion acotada de fuentes BOE estables recapturadas desde `doc.php`, derivada del audit `docs/operations/boe-recapture-audit/boe_recapture_audit_20260603_203107.json`. Diagnostico: `BOE-A-2025-25389` y `BOE-A-2025-25390` son `stable` con hash y `content_length` verificados; los `hash_changed` restantes quedan como hash sin linaje verificable y no se promocionan. `193` se excluye porque `/v1/modelos/193/resumen-operativo` esta en `conflict` por evidencia AEAT 2025 combinada con PDF informativo `193/296` bajo ruta 2024; `190` se excluye porque ya tiene carril `operational` afirmable. Cambio local: `services.modelos` permite que una fuente BOE directa con `campaign_evidence_role='direct_legal'`, hash, `capture_date` y campana vinculada resuelva `resolved_strong` sin carril operativo; migracion `20260603_0116_promote_boe_25389_normativa` promueve `182/184/195/199/282/345` con `BOE-A-2025-25389`; migracion `20260603_0117_promote_boe_25390_normativa` promueve `270/347` con `BOE-A-2025-25390`. Validacion local: test rojo->verde `test_direct_legal_boe_evidence_can_assert_campaign_without_operational_lane`; tests focales `5 passed`; `test_alembic_integrity.py` + `test_maintenance_agents.py` => `71 passed`; `py_compile` OK; Ruff focal `F,I` OK; `alembic heads` unico en `20260603_0117`; `git diff --check` OK. Evidencia VPS commit `b75c4ee5`: `git pull --ff-only`; imagenes `ops` y `api` reconstruidas; Alembic aplicado secuencialmente, `0116` verificado por SQL con `resources=6`, `modelo_normativa=6` y `193_promoted=0`; `0117` verificado por SQL con `resources=2`, `modelo_normativa=2` y `190_promoted=0`; API recreada y `/health` OK; `/v1/modelos/{182,184,195,199,282,345}/resumen-operativo` devuelve `resolved_strong`, `campana_afirmable=2025`, `campana_assertion_basis=["legal"]`, `legal.source_url=https://www.boe.es/buscar/doc.php?id=BOE-A-2025-25389`; `/v1/modelos/{270,347}/resumen-operativo` devuelve `resolved_strong`, `campana_afirmable=2025` y fuente legal `BOE-A-2025-25390` (`347` conserva ademas `operational`); `193` sigue `conflict` con `campana_conflict_years=["2024","2025"]`; `190` sigue `resolved_strong` por `operational` sin carril legal. `mcp_validation_suite.py --read-only --base-url http://api:8000` OK con los dos contratos nuevos; `source_assurance_gate.py` OK; `verify-doc-artifacts.py --ci-baseline` OK; `verify-doc-contracts.py` OK; `esdata-mcp-validation.service` relanzado tras el recreate queda `SUCCESS`; `systemctl --failed` limpio.

- 2026-06-03 - `[COMPLETADO LOCAL + VPS]` `AEAT-MODELO-289-HAC-1430-LEGAL-0115`: correccion urgente del carril legal BOE del Modelo `289/2025` tras detectar que `BOE-A-2025-25389` (`Orden HAC/1430/2025`) existia en produccion solo como `modelo_recurso:normativa` generico inactivo, sin promocion al carril semantico `legal_campaign_evidence`. Diagnostico: `0110` habia cargado correctamente `Orden HAC/1504/2024` como evidencia legal directa, pero la Orden HAC/1430/2025 publicada en BOE el `2025-12-12` modifica los anexos I y II de la Orden HAP/1695/2016 del modelo 289 y es aplicable por primera vez a declaraciones informativas del ejercicio `2025`, por lo que debe ser la fuente legal actual del 289/2025. Cambio: migracion `20260603_0115_aeat_289_hac_1430_legal_campaign_evidence` anade `modelo_recurso:normativa_hac_1430_2025` activo, `row_completeness='complete'`, `row_provenance='official_exact'`, metadata `source_kind='legal_campaign_evidence'`, `campaign_evidence_role='direct_legal'`, `capture_date=2026-06-03`, `ejercicio_declarado=2025`, `anio_presentacion=2026`, y `modelo_normativa` para `BOE-A-2025-25389`. Fuente BOE verificada: `https://www.boe.es/buscar/doc.php?id=BOE-A-2025-25389`, hash SHA-256 `45522b6eed4eca77673bffd87d7a4d744b9195e00ec4594a9fb9ae591b32421a`, `content_length=110846`. Gate: `mcp_validation_suite.py` actualiza `aeat_modelo_289_legal_campaign_traceable_contract` para exigir la fuente HAC/1430/2025 y su fila `modelo_normativa`; fix posterior `d38c7eea` usa `COUNT(DISTINCT rec.id)` para evitar multiplicacion por join con `modelo_normativa`. Limites: no toca `obligacion_perfil`, no abre CRS/DAC2, no cambia `CAMPAIGN_BEARING_RESOURCE_TYPES`, no carga claves/instrucciones/reglas ni promueve `safe_to_answer`, `verified` o `completeness` de obligaciones. Validacion local: TDD rojo->verde; `test_alembic_integrity.py` + `test_maintenance_agents.py` => `70 passed`; `py_compile` OK; Ruff focal `F,I` OK; `alembic heads` unico en `20260603_0115`; `source_assurance_gate.py` OK; `verify-doc-artifacts.py --ci-baseline` OK; `git diff --check` OK. Evidencia VPS commits `7f2bce38` + `d38c7eea`: `git pull --ff-only`; imagen `ops` reconstruida antes de Alembic; Alembic aplicado y confirmado en `20260603_0115_aeat_289_hac_1430_legal_campaign_evidence`; API reconstruida y `/health` OK; SQL productivo confirma `normativa_hac_1430_2025` activa con hash `45522b6eed4eca77673bffd87d7a4d744b9195e00ec4594a9fb9ae591b32421a`, `content_length=110846`, `capture_date=2026-06-03`, `direct_legal`, y `modelo_normativa` para `BOE-A-2025-25389`; `/v1/modelos/289/resumen-operativo` devuelve `legal.source_url=https://www.boe.es/buscar/doc.php?id=BOE-A-2025-25389`, `campana_afirmable=2025`, `campana_assertion_basis=["legal","operational"]`; `mcp_validation_suite.py --read-only --base-url http://api:8000` OK con `aeat_modelo_289_legal_campaign_traceable_contract`; `source_assurance_gate.py` OK; `verify-doc-artifacts.py --ci-baseline` OK; `systemctl --failed` limpio. Barrido posterior: no quedan casos con el mismo patron fuerte (`normativa` BOE inactiva + hash + `content_length` + ausencia de `modelo_normativa`). Como candidatos debiles quedan 33 pares modelo/BOE de 2025 sin `content_length`, entre ellos `182/184/193/195/199/282/345` con `BOE-A-2025-25389`, `190/270/347` con `BOE-A-2025-25390`, `303` con `BOE-A-2025-25272`, `170/171/181/196` con `BOE-A-2025-14600`, `233` con `BOE-A-2025-13411`, `200/206/220` con `BOE-A-2025-12818`, `240/241/242` con `BOE-A-2025-21727`/`BOE-A-2025-6598`, y `780/781` con `BOE-A-2025-10597`/`BOE-A-2025-25041`; todos requieren revision fuente por fuente antes de promocion semantica. Hermes 2026-06-03 confirma regla de guardia: `NEEDS_HUMAN_REVIEW`; JSON recientes `124/126/128` no son promocionables (`safe=false`).

- 2026-06-02 - `[COMPLETADO LOCAL + VPS]` `AEAT-MODELO-190-FIELD-RULES-0114`: primer microciclo de reglas e instrucciones por campo del Modelo `190/2025`, limitado a posicion `78` (`clave`) y posiciones `79-80` (`subclave`). Diagnostico local: `0111` ya dejaba 3 instrucciones generales y 12 reglas de claves A-L; `0113` ya cargaba 77 subclaves jerarquicas con fuente PDF oficial; faltaba un gate especifico para reglas de obligatoriedad/no cumplimentacion de los campos clave/subclave. Cambio: migracion `20260602_0114_aeat_190_field_rules_key_subclave` anade 3 instrucciones trazables y 3 reglas `modelo_regla_inclusion`: clave obligatoria en posicion 78, subclave obligatoria solo para B, C, E, F, G, H, I, K y L, y no cumplimentar subclave para A, D y J. `mcp_validation_suite.py` anade `aeat_modelo_190_field_rules_key_subclave_contract` para exigir las 3 instrucciones, las 3 reglas, `casilla_referencia` `78`/`79-80`, decisiones `INCLUIR`/`CONDICIONAL`/`EXCLUIR` y fuente hasheada. Fuente unica: PDF oficial AEAT `DISENOS_LOGICOS_190_2025.pdf`, hash SHA-256 `a7d1092f78620431812354e560a5146a3ae244e0aed69d9d58c353370ba0134d`, `capture_date=2026-06-01`. Limites: no toca campana, no toca `obligacion_perfil`, no modifica claves principales A-L ni las 77 subclaves, no promueve `safe_to_answer`, `verified` ni `completeness`. Validacion local commit `8cf9bd17`: TDD rojo->verde; `test_alembic_integrity.py` + `test_maintenance_agents.py` => `69 passed`; `py_compile` OK; Ruff focal `F,I` OK; `alembic heads` unico en `20260602_0114`; `source_assurance_gate.py` OK; `verify-doc-artifacts.py --ci-baseline` OK; `git diff --check` OK. Evidencia VPS commit `8cf9bd17`: `git pull --ff-only`; `ops` reconstruido porque la imagen previa no veia la nueva revision; Alembic aplicado y confirmado en `20260602_0114_aeat_190_field_rules_key_subclave`; `api` reconstruida porque la imagen previa abortaba al arrancar con Alembic antiguo; `/health` OK; `mcp_validation_suite.py --read-only --base-url http://api:8000` OK con `aeat_modelo_190_field_rules_key_subclave_contract`; `source_assurance_gate.py` OK desde el checkout VPS; `verify-doc-artifacts.py --ci-baseline` OK desde el checkout VPS; `systemctl --failed` limpio. Nota operativa: los reruns de `source_assurance_gate.py` y `verify-doc-artifacts.py` dentro de `ops` fallan porque la imagen no contiene todos los documentos, por lo que se validaron desde `/srv/esdata`, que es el camino ya usado para gates documentales.

- 2026-06-02 - `[COMPLETADO LOCAL + VPS]` `AEAT-MODELO-190-SUBCLAVES-0113`: preparada y desplegada la migracion de datos `20260602_0113_aeat_190_subclaves_percepcion` para cargar las 77 subclaves oficiales del Modelo `190/2025` como filas `modelo_clave` de `nivel=2` bajo los padres B, C, E, F, G, H, I, K y L creados en `0111` y jerarquizados por `0112`. Fuente unica: PDF oficial AEAT `DISENOS_LOGICOS_190_2025.pdf`, hash SHA-256 `a7d1092f78620431812354e560a5146a3ae244e0aed69d9d58c353370ba0134d`, `capture_date=2026-06-01`. Datos: conteos exactos B=5, C=9, E=4, F=7, G=8, H=4, I=3, K=5, L=32; las descripciones normalizadas eliminan cabeceras/pies del PDF y quedan en ASCII para evitar deuda de encoding. Contrato: lookup de `parent_id` por codigo de clave, sin IDs hardcodeados; `tipo_clave/tipo='SUBCLAVE_PERCEPCION'`, `nivel=2`, `source_url/source_hash/capture_date` por fila; validacion interna exige `COUNT(*) = 77`, codigos exactos por clave via `array_agg(codigo::text ORDER BY codigo::text)`, descripciones no vacias y sin marcadores de cabecera. `mcp_validation_suite.py` anade `aeat_modelo_190_subclaves_77_hierarchical_contract` y acota los contratos previos de claves principales a `parent_id IS NULL`/`nivel=1` para no mezclar subclaves con padres. Limites: no cambia campana, no toca `obligacion_perfil`, no promueve `safe_to_answer`, `verified` ni `completeness`, y no altera claves principales A-L. Validacion local: TDD rojo->verde; `test_alembic_integrity.py` + `test_maintenance_agents.py` => `68 passed`; `py_compile` OK; Ruff focal `F,I` OK; `source_assurance_gate.py` OK; `verify-doc-artifacts.py --ci-baseline` OK; `git diff --check` OK; `alembic heads` unico en `20260602_0113`. Evidencia VPS commits `63f2f92` + `b68ab1f`: `git pull --ff-only`; imagen `ops` reconstruida; Alembic en `20260602_0113`; SQL productivo confirma B=5, C=9, E=4, F=7, G=8, H=4, I=3, K=5, L=32, total `77`, `with_evidence=77`, `bad_descriptions=0`; muestras `B/01`, `K/04`, `L/25` y `L/28` correctas; `/health` OK; `mcp_validation_suite.py --read-only` OK con `aeat_modelo_190_subclaves_77_hierarchical_contract`, `aeat_modelo_190_internal_keys_rules_traceable_contract` y `aeat_modelo_190_perception_keys_a_l_traceable_contract`; `source_assurance_gate.py` OK; `verify-doc-artifacts.py --ci-baseline` OK; `mcp_deep_contract_audit.py` OK; `esdata-mcp-validation.service` `Result=success`; `systemctl --failed` limpio.

- 2026-06-02 - `[COMPLETADO LOCAL + VPS]` `AEAT-MODELO-190-SUBCLAVE-HIERARCHY-SCHEMA-0112`: preparada y desplegada la migracion estructural minima para soportar jerarquia `clave -> subclave` en `modelo_clave`, sin cargar todavia las 77 subclaves del modelo `190`. Cambio: Alembic `20260602_0112_modelo_clave_hierarchy_schema` anade `parent_id` con FK self-referencing nombrada `modelo_clave_parent_id_fkey`, anade `nivel SMALLINT NOT NULL DEFAULT 1`, crea `ix_modelo_clave_parent_id`, elimina los unicos legacy `modelo_clave_campana_id_codigo_key` y `ux_modelo_clave_campana_tipo_codigo`, y los sustituye por indices unicos parciales `ux_modelo_clave_principal` (`parent_id IS NULL`) y `ux_modelo_clave_subclave` (`parent_id IS NOT NULL`). Contrato: filas existentes A-L quedan como nivel 1; las subclaves futuras podran repetir codigos como `01` bajo padres distintos sin parsear strings tipo `B:01`; no se toca `obligacion_perfil`, no se promociona `safe_to_answer`, no se carga `SUBCLAVE_PERCEPCION` ni cambia la campana del `190`. `mcp_validation_suite.py` anade `modelo_clave_hierarchy_schema_contract` para exigir columnas, FK, indices parciales y ausencia de los unicos legacy tras aplicar la migracion. Validacion local: TDD rojo->verde; `test_alembic_integrity.py` + `test_maintenance_agents.py` => `67 passed`; `py_compile` OK; Ruff focal `F,I` OK; `source_assurance_gate.py` OK; `verify-doc-artifacts.py --ci-baseline` OK; `git diff --check` OK; `alembic heads` unico en `20260602_0112`. Limite local: el smoke contra una DB temporal vacia (`esdata_0112_smoke`) no alcanzo 0112 porque la migracion previa `20260601_0109_aeat_190_internal_evidence` exige una campana activa `190/2025`. Evidencia VPS commit `d10c623`: `git pull --ff-only`; imagen `ops` reconstruida; Alembic en `20260602_0112`; DDL confirma `parent_id`, `nivel`, FK `modelo_clave_parent_id_fkey`, indices `ix_modelo_clave_parent_id`, `ux_modelo_clave_principal` y `ux_modelo_clave_subclave`, sin `ux_modelo_clave_campana_tipo_codigo` ni `modelo_clave_campana_id_codigo_key`; `/health` OK; `mcp_validation_suite.py --read-only` OK con `modelo_clave_hierarchy_schema_contract`; `source_assurance_gate.py` OK; `verify-doc-artifacts.py --ci-baseline` OK; `mcp_deep_contract_audit.py` OK; `esdata-mcp-validation.service` `Result=success`; `systemctl --failed` limpio.

- 2026-06-01 - `[COMPLETADO LOCAL + VPS]` `AEAT-MODELO-289-LEGAL-190-A-L-01`: foco solicitado por el usuario en dos cierres separados. Modelo `289`: se anade carril `legal` BOE sin ampliar `CAMPAIGN_BEARING_RESOURCE_TYPES`; solo recursos con `campaign_evidence_role='direct_legal'`, hash y `capture_date` pueden poblar el carril. Fuente BOE verificada: `BOE-A-2024-27528`, Orden HAC/1504/2024, hash SHA-256 `91c443efff4b5cca5403d5573be18061effa495cdd9769c6dfc305b3c9110c3c`, `content_length=152551`, texto "2025 y siguientes"; migracion `20260601_0110_aeat_289_legal_campaign_evidence` carga `modelo_recurso:normativa_hac_1504_2024`, anade `modelo_normativa` correcta y elimina la referencia local erronea `BOE-A-2024-24098` para `289` si existia. Modelo `190`: migracion `20260601_0111_aeat_190_perception_keys_expansion` amplia la auditoria interna desde claves A-D a claves principales A-L con el PDF oficial AEAT `DISENOS_LOGICOS_190_2025.pdf`, manteniendo hash `a7d1092f78620431812354e560a5146a3ae244e0aed69d9d58c353370ba0134d`, `capture_date=2026-06-01`, 3 instrucciones y 12 reglas `CONDICIONAL`. `mcp_validation_suite.py` exige `aeat_modelo_289_legal_campaign_traceable_contract` y `aeat_modelo_190_perception_keys_a_l_traceable_contract`. Limites: no se promueve `obligacion_perfil`, no se marca aplicabilidad por perfil como completa, no se carga el catalogo completo de subclaves 190. Validacion local: TDD rojo->verde; `test_modelos_truth_contract.py` + `test_alembic_integrity.py` + `test_maintenance_agents.py` => `110 passed`; `py_compile` OK; Ruff focal `F,I` OK; `git diff --check` OK; `alembic heads` unico en `20260601_0111`. Evidencia VPS commit `092062b`: `git pull --ff-only`; imagenes `api` y `ops` reconstruidas; Alembic en `20260601_0111`; `/health` OK; DB `190/2025` queda con `claves=12`, `instrucciones=3`, `reglas=12`, DB `289/2025` queda con `modelo_recurso:normativa_hac_1504_2024`; `/v1/modelos/289/resumen-operativo` devuelve `campana_assertion_basis=["legal","operational"]`, `legal.status=resolved_strong_legal`, `legal.source_hash=91c443efff4b5cca5403d5573be18061effa495cdd9769c6dfc305b3c9110c3c`, `campana_safe_to_assert=true`; `/v1/modelos/190/resumen-operativo` conserva `campana_assertion_basis=["operational"]`; `mcp_validation_suite.py --read-only` OK con los dos checks nuevos; `source_assurance_gate.py` OK; `verify-doc-artifacts.py --ci-baseline` OK; `mcp_deep_contract_audit.py` OK; `esdata-mcp-validation.service` `Result=success`; `systemctl --failed` limpio.

- 2026-06-01 - `[COMPLETADO LOCAL + VPS]` `AEAT-MODELO-190-INTERNAL-EVIDENCE-01`: cerrado el ciclo separado de auditoria interna del modelo `190`, sin tocar la campana ya resuelta en `0108`. Diagnostico VPS previo: campana `190/2025` ya devuelve `resolved_strong` por carril `operational`, pero las 4 claves A-D existentes en `modelo_clave` no tenian `source_url/source_hash/capture_date`, `modelo_instruccion` estaba vacio y `modelo_regla_inclusion` estaba vacio. Fuente verificada: PDF oficial AEAT `DISENOS_LOGICOS_190_2025.pdf`, hash SHA-256 `a7d1092f78620431812354e560a5146a3ae244e0aed69d9d58c353370ba0134d`, `content_length=1110488`, `capture_date=2026-06-01`. Cambio: migracion Alembic `20260601_0109_aeat_190_internal_evidence` sustituye las claves legacy A-D por claves oficiales de percepcion A-D con provenance completa, anade 3 instrucciones internas sobre diseno/clave/subclave y 4 reglas `CONDICIONAL` para el uso de esas claves; `mcp_validation_suite.py` anade `aeat_modelo_190_internal_keys_rules_traceable_contract`. Limite: no cambia `campana_resolution_status`, no toca `CAMPAIGN_BEARING_RESOURCE_TYPES`, no promueve `obligacion_perfil`, no marca aplicabilidad por perfil como completa. Validacion local: TDD rojo->verde; `test_alembic_integrity.py` + `test_maintenance_agents.py` => `64 passed`; `py_compile` OK; Ruff focal `F,I` OK; `git diff --check` OK; `alembic heads` unico en `20260601_0109`. Evidencia VPS commits `5bccad9b` + `b6ca6615`: `git pull --ff-only`; `ops` reconstruido; Alembic en `20260601_0109`; DB `190/2025` queda con `claves=4`, `claves_con_fuente=4`, `instrucciones=3`, `instrucciones_con_fuente=3`, `reglas=4`, `reglas_con_fuente=4`; API `/v1/modelos/190/claves` devuelve codigos `A,B,C,D` con el hash AEAT; `/v1/modelos/190/instrucciones` devuelve `diseno_registro_2025`, `clave_percepcion`, `subclave`; `/v1/modelos/190/resumen-operativo` conserva `campana_resolution_status=resolved_strong`, `campana_safe_to_assert=true`, `campana_afirmable=2025`, `campana_assertion_basis=["operational"]`; `mcp_validation_suite.py --read-only` OK con `aeat_modelo_190_internal_keys_rules_traceable_contract`; `source_assurance_gate.py` OK; `verify-doc-artifacts.py --ci-baseline` OK; `mcp_deep_contract_audit.py` OK; `esdata-mcp-validation.service` `Result=success`; `systemctl --failed` limpio.

- 2026-06-01 - `[COMPLETADO LOCAL + VPS]` `AEAT-MODELO-190-CAMPAIGN-EVIDENCE-01`: cerrado el gap de campana operativa del modelo `190`, manteniendo separada la auditoria interna de claves/instrucciones/reglas. Diagnostico VPS previo: `190` estaba en `resolved_weak` porque solo exponia `campana_evidence_items` debil desde `aeat_instrucciones` y los 7 recursos activos no tenian `proves_campaign=true`; las 4 claves A-D siguen sin `source_url/source_hash/capture_date` y quedan fuera de este ciclo. Cambio: migracion Alembic `20260601_0108_aeat_190_campaign_evidence` registra la pagina AEAT Sede `modelo-190.html` como `modelo_recurso:instrucciones` para `190/2025`, con hash `b154ea65d4a7679774842767de2e643f76c194e037237052647f43962073f205`, `content_length=30641`, `capture_date=2026-06-01`, label "Modelo 190. Ejercicio 2025. Gestiones activas en AEAT Sede.", `row_completeness='complete'` y `row_provenance='official_exact'`; no se toca `CAMPAIGN_BEARING_RESOURCE_TYPES`, no se promueven obligaciones, claves, instrucciones estructuradas ni reglas. `mcp_validation_suite.py` pasa de aceptar `190` debil a exigir `aeat_modelo_190_direct_instruction_asserts_campaign` con `resolved_strong` y fuente directa; `campana_assertion_basis` queda limitado a carriles afirmativos y no mezcla `technical_design` cuando ya existe `operational`. Validacion local: TDD rojo->verde; `test_alembic_integrity.py` + `test_maintenance_agents.py` + seleccion focal de `test_modelos_truth_contract.py` => `68 passed`; `py_compile` OK; Ruff focal OK; `git diff --check` OK. Evidencia VPS commits `a15f94c3`, `8072aa02`, `7b3cefca`, `85fe0f1`: `git pull --ff-only`; API/ops reconstruidos; Alembic en `20260601_0108_aeat_190_campaign_evidence`; `/health` OK; `/v1/modelos/190/resumen-operativo` devuelve `campana_resolution_status=resolved_strong`, `campana_safe_to_assert=true`, `campana_afirmable=2025`, `campana_assertion_basis=["operational"]`, `campana_conflict=false` y `operational_source_hash=b154ea65d4a7679774842767de2e643f76c194e037237052647f43962073f205`; `mcp_validation_suite.py --read-only` OK; `source_assurance_gate.py` OK; `verify-doc-artifacts.py --ci-baseline` OK; `mcp_deep_contract_audit.py` OK; `esdata-mcp-validation.service` `Result=success`; `systemctl --failed` limpio. Limite: la auditoria de claves A-D e instrucciones/reglas del `190` sigue pendiente y separada.

- 2026-06-01 - `[COMPLETADO LOCAL + VPS]` `AEAT-MODELO-123-CAMPAIGN-EVIDENCE-01`: cerrado el gap simple del modelo `123` con evidencia operativa AEAT directa, sin tocar el trabajo pendiente del `190`. Diagnostico VPS previo: la campana activa era `2024` y `campana_evidence_items` solo exponia `formulario_html` `https://www1.agenciatributaria.gob.es/wlpl/OVME-COMN/123/E2024/index.zul` como `resolved_weak`; los seis recursos activos eran `pagina_modelo`, ayuda generica, diseno de registro, formulario transaccional `E2024`, normativa BOE 2007 y accesibilidad, ninguno suficiente como prueba directa. Cambio: `ayuda_tecnica_presentacion` se acepta como evidencia directa solo con URL Sede, hash, modelo y ejercicio explicitos; migracion `20260601_0107_aeat_123_campaign_evidence` carga la pagina AEAT de ayuda de presentacion `modelo-123.html` con hash `4bdbd140ef841194407204e9f89a1a5ba3ca2f23af333dd1dbf82c81951b9ad6`, `capture_date=2026-06-01` y label "Presentacion del modelo 123. Ejercicio 2024 y siguientes"; seeds actualizados para no reintroducir la URL debil. Validacion local: TDD rojo->verde; `test_modelos_truth_contract.py` + `test_maintenance_agents.py` => `72 passed`; `py_compile` OK; Ruff focal OK; `verify-doc-artifacts.py --ci-baseline` OK; `verify-doc-contracts.py` OK; `source_assurance_gate.py` OK; `git diff --check` OK. Evidencia VPS commit `88d21e5`: `git pull --ff-only`; API/ops reconstruidos; Alembic en `20260601_0107`; `/v1/modelos/123/resumen-operativo` devuelve `campana_resolution_status=resolved_strong`, `campana_safe_to_assert=true`, `campana_afirmable=2024`, `campana_assertion_basis` incluye `operational`, y `campana_evidence.operational.source_hash` coincide; `/health` OK; `esdata-mcp-validation.service` `Result=success`; `esdata-mcp-deep-audit.service` `Result=success`; `source_assurance_gate.py` OK en repo VPS; `verify-doc-artifacts.py --ci-baseline` OK; `systemctl --failed` limpio. Limite: esto afirma despliegue operativo AEAT para `2024 y siguientes`, no cierra obligacion por perfil ni sustituye auditoria de reglas/casillas.

- 2026-06-01 - `[COMPLETADO LOCAL + VPS]` `ENV-EXAMPLE-BASELINE-DRIFT-01`: cerrada deuda aislada de `.env.example` detectada por `verify-doc-artifacts.py --ci-baseline` para variables `BDNS_*`, `BORME_*` y `HC_PING_*`, sin mezclarlo con fuentes AEAT `123/190`. Diagnostico: variables activas/optionales ya declaradas en Compose/docs/codigo; en VPS no estan en `/etc/esdata/esdata.env` porque BDNS/BORME usan defaults de Compose y los `HC_PING_URL_*` quedan vacios si no hay monitor configurado. Cambio: `.env.example` incorpora los mismos defaults/placeholders de `infra/deploy/compose.env.example`; `.gitignore` ignora `/hermes-curator` como runtime local/prod no versionado. VPS: `git pull --ff-only`; `hermes-curator/.env` se saco del arbol Git moviendo el runtime a `/var/lib/esdata/hermes-curator` y dejando symlink compatible en `/srv/esdata/hermes-curator`; `hermes-esdata-curator.timer` activo. No se leyeron ni tocaron valores secretos. Validacion local: `verify-doc-artifacts.py --ci-baseline` OK; `verify-doc-contracts.py` OK; `source_assurance_gate.py` OK; `git diff --check` OK. Validacion VPS: `verify-doc-artifacts.py --ci-baseline` OK.

- 2026-06-01 - `[COMPLETADO LOCAL + VPS]` `AEAT-MODELO-289-CAMPAIGN-DUALITY-01`: resuelto el conflicto semantico del modelo `289` separando `ejercicio_declarado=2025` y `anio_presentacion=2026`, corrigiendo la URL AEAT de campana a `campana-declaraciones-informativas-2025/normativa/modelo-289.html` en seeds/datos productivos y cargando evidencia operativa AEAT con hash `2502f41331ddd2eb1b39171bd3d54be661baf5cc69a6c4c54812d38c4f29867b`. Cambios: migracion Alembic `20260601_0106_aeat_289_campaign_duality`, API/schema/OpenAPI/types exponen la dualidad, el carril operacional prefiere evidencia directa hasheada frente a senales debiles, y los seeds dejan de reintroducir la URL 2024. Validacion local: TDD rojo->verde; tests focales `94 passed`; seleccion de hash `51 passed`; Ruff focal OK; `py_compile` OK; `verify-doc-contracts.py` OK; `source_assurance_gate.py` OK; `git diff --check` OK; `verify-doc-artifacts.py --ci-baseline` sigue fallando solo por drift historico `.env.example` BDNS/BORME/HC_PING. Evidencia VPS commits `7a55410` + `2d4e041`: `git pull --ff-only`; Alembic en `20260601_0106`; API/ops reconstruidos y API healthy; `/v1/modelos/289/resumen-operativo` devuelve `campana_resolution_status=resolved_strong`, `campana_safe_to_assert=true`, `campana_assertion_basis=["operational"]`, `campana_evidence.operational.source_hash` con el hash AEAT 2025, `campana_conflict=false`, `campana_conflict_evidence=[]`; `esdata-mcp-validation.service` `Result=success`; `esdata-mcp-deep-audit.service` `Result=success`; `source_assurance_gate.py` OK; `/health` OK; `systemctl --failed` limpio. Limite mantenido: el carril legal BOE sigue `none` y las obligaciones por perfil del `289` no se promueven.

- 2026-06-01 - `[COMPLETADO LOCAL + VPS]` `AEAT-CAMPAIGN-EVIDENCE-LANES-SCHEMA-01`: cambio de contrato solicitado tras separar campaña legal, campaña operativa y vigencia de diseño tecnico. Cambios: `campana_evidence` pasa de lista plana a objeto derivado con carriles `legal`, `operational` y `design`; `campana_evidence_items` conserva evidencia plana legacy/debug; `campana_assertion_basis` lista carriles aplicables; los campos planos (`campana_safe_to_assert`, `campana_assertion_code`, etc.) se conservan como derivados. Los recursos de diseño tecnico (`diseno_registro`, `xsd/wsdl`, `aeat_formato`) dejan de alimentar conflicto de campaña por si solos y solo activan `technical_design`. Documentacion AEAT y OpenAPI GPT regenerados. Validacion local: TDD rojo->verde; `test_modelos_truth_contract.py`, `test_maintenance_agents.py`, `test_mcp_campaign_assertion_contract.py`, `test_verify_doc_artifacts.py` => `92 passed`; `py_compile` OK; Ruff focal `F,I` OK; `verify-doc-contracts.py` OK; `source_assurance_gate.py` OK; `git diff --check` OK. `verify-doc-artifacts.py --ci-baseline` sigue fallando por drift preexistente de env vars `BDNS_*`, `BORME_*` y `HC_PING_*`, no introducido por este cambio. Evidencia VPS commits `3c20f3f` + `be457c2`: `git pull --ff-only`; API y ops reconstruidos donde aplica; API recreada healthy; `/v1/modelos/124/resumen-operativo` queda `resolved_weak`, `campana_safe_to_assert=false`, `campana_assertion_basis=["technical_design"]`, sin `campana_conflict`; `190/123` siguen `resolved_weak`; `289` mantiene `conflict` por señales no puramente tecnicas; `esdata-mcp-validation.service` `Result=success`; `esdata-mcp-deep-audit.service` `Result=success`; `/health` y `/status` OK; `systemctl --failed` limpio.
- 2026-06-01 - `[COMPLETADO LOCAL + VPS]` `AEAT-190-123-124-289-FOCUS-01`: foco solicitado por el usuario en modelos AEAT `190`, `123`, `124` y `289`. Diagnostico VPS: ninguno permite afirmar campana con maxima exactitud fiscal (`190/123=resolved_weak`, `124/289=conflict`); datos productivos trazables: `123` tiene 44 casillas/2 reglas/6 recursos exactos, `124` 39 casillas/2 reglas/6 recursos exactos, `190` 72 casillas/4 claves/7 recursos exactos, `289` 161 casillas/5 instrucciones/6 reglas/12 recursos exactos; `obligacion_perfil` de `289` mantiene 4 perfiles fail-closed (`verified=false`, `safe_to_answer=false`, sin `source_hash`). Cambios: `mcp_validation_suite.py` anade gates `aeat_modelo_123_design_traceable_partial_contract`, `aeat_modelo_124_design_traceable_partial_contract`, `aeat_modelo_289_documental_traceable_fail_closed_contract` y contratos API `*_campaign_not_overclaimed_contract` para `190/123/124/289`; no se poblaron seeds ni se promocionaron campanas/obligaciones. Validacion local: TDD rojo->verde; `test_maintenance_agents.py` `30 passed`; `py_compile` OK; Ruff focal `F,I` OK; `verify-doc-contracts.py` OK; `source_assurance_gate.py` OK; `git diff --check` OK. Evidencia VPS commit `80cb446`: `git pull --ff-only` OK; imagen `ops` reconstruida; `esdata-mcp-validation.service` `Result=success` con nuevos checks verdes; `esdata-mcp-deep-audit.service` `Result=success`; `/health` OK; `/status` autenticado OK; `systemctl --failed` limpio. Limite: esto deja los cuatro modelos vigilados y fail-closed donde toca; no convierte 124/289 en campana afirmable ni resuelve aplicabilidad CRS/DAC2 por perfil del 289.
- 2026-06-01 - `[COMPLETADO LOCAL + VPS]` `AEAT-MODELO-303-DIRECT-CAMPAIGN-ASSERTION-01`: continuacion solicitada por el usuario sobre modelos de Hacienda, empezando por el 303. Cambios: `apps/api/services/modelos.py` usa instrucciones/formularios oficiales con metadato explicito de campana y hash como prueba directa de campana; disenos tecnicos siguen sin probar campana; recursos transaccionales no promueven campanas; una prueba directa oficial de la campana activa prevalece sobre recursos debiles/stale, salvo conflicto entre pruebas directas. Se cubre el caso real de produccion donde `instrucciones-2026.html` estaba como version historica por rotacion de `modelo_recurso` pero conserva hash y metadato oficial. `mcp_validation_suite.py` anade `aeat_modelo_303_direct_instruction_asserts_campaign`. Validacion local: test rojo reprodujo `campana_afirmable=None`; tests focales `67 passed`; Ruff focal OK; `py_compile` OK; `verify-doc-contracts.py` OK; `source_assurance_gate.py` OK; `git diff --check` OK. Evidencia VPS commits `cb304eb`, `bd2cbfb`: API redeploy OK; `/v1/modelos/303/resumen-operativo` devuelve `campana_afirmable=2026`, `campana_resolution_status=resolved_strong`, `campana_safe_to_assert=true`, `campana_assertion_code=ASSERTABLE_DIRECT_OFFICIAL` y fuente `Instrucciones 2026` con `campaign_evidence_role=direct_official`; `mcp_validation_suite.py --read-only` OK; `mcp_deep_contract_audit.py` OK; `/health` OK; `systemctl --failed` limpio. Limite: esto mejora el 303/campana 2026, no convierte AEAT ni todos los supuestos de obligacion en completos; `obligacion_perfil` del 303 sigue `parcial`/review para aplicabilidad por perfil hasta row-level proof.
- 2026-05-31 - `[COMPLETADO LOCAL + VPS]` `AEAT-PRIORITY-PARTIAL-EVIDENCE-GATE-01`: cerrado tras priorizacion explicita del usuario sobre AEAT. Objetivo: no promover AEAT como bloque unico, sino endurecer gates de modelos prioritarios con evidencia oficial parcial ya cargada. Cambios: `scripts/maintenance/mcp_validation_suite.py` anade contratos `aeat_modelo_303_iva_traceable_partial_contract`, `aeat_modelo_202_is_payment_traceable_partial_contract`, `aeat_modelo_190_withholding_traceable_partial_contract` y `aeat_priority_target_models_have_traceable_partial_evidence_7`; `scripts/tests/test_maintenance_agents.py` fija nombres y exige trazabilidad `official_exact` + `sha256_contenido`. Diagnostico VPS: `303` tiene evidencia oficial trazable pero `campana_safe_to_assert=false`; `202` mantiene conflicto de campana `2025/2026`; `190` queda trazable parcial; antiguos `target` (`180`, `184`, `190`, `232`, `347`, `349`, `390`) pasan a evidencia parcial trazable; `233` sigue pendiente por falta de contrato/plantilla determinista. Validacion local: `test_maintenance_agents.py` `28 passed`; Ruff focal OK; `py_compile` OK; `verify-doc-contracts.py` OK; `source_assurance_gate.py` OK; `git diff --check` OK. Evidencia VPS commits `171ffa2a`, `cfe3c564`: `mcp_validation_suite.py --read-only` OK con los cuatro checks verdes (`303=1`, `202=1`, `190=1`, `target_evidence=7`); `mcp_deep_contract_audit.py` OK; `/health` OK; `/status` autenticado OK; `systemctl --failed` limpio. Siguiente AEAT recomendado: auditoria quirurgica del Modelo 303 para decidir si puede cerrarse mejor como formulario/IVA sin convertir la campana 2026 en afirmacion fiscal.
- 2026-05-31 - `[COMPLETADO LOCAL + VPS]` `CNMV-REGISTRY-REPO-REVIEW-02`: abierto por instruccion explicita del usuario para continuar CNMV revisando `Ansvar-Systems/spanish-financial-regulation-mcp` y `BquantFinance/cnmv`. Decision: Ansvar es util como patron operativo de paginacion del registro oficial de sanciones CNMV; Bquant es snapshot CSV sin scraper/procedencia por fila suficiente para ingestion autoritativa, por lo que no se ingiere. Cambios: `CNMV_SANCIONES_MAX_PAGES` sube de `1` a `25` en worker/Compose/env docs; el worker corta al detectar pagina oficial sin filas o respuesta no-200; textos PDF corruptos se degradan a `partial`/`official_best_effort`; `docs/github-public-scraper-review-20260531.md` y backlog documentan la decision. Smoke live local: `cnmv.py --discover-only --familia sanciones --max-urls 200` descubre `59` documentos y se detiene en `page=19` sin pedir `page=20`. Validacion local: tests focales CNMV/API/vocabulario `139 passed, 1 skipped`; Ruff focal OK; `py_compile` OK; `verify-doc-contracts.py` OK; `source_assurance_gate.py` OK; compose config OK; `git diff --check` OK. Evidencia VPS commits `c296439`, `f5a6028`, `cdbc75c`, `47780de`: deploy/rebuild worker CNMV y cron OK; `cron-cnmv-weekly` acotado a sanciones descubre/procesa `59` URLs y almacena `19`; DB `sancion_cnmv/sanciones_cnmv=49`, `complete/official_exact=15`, `partial/official_best_effort=34`, `unreliable_complete_official_exact=0`; `/v1/cnmv/coverage` mantiene `sanciones_cnmv` como `partial_loaded` con `loaded_count=49`; `/health` OK; `/status` autenticado OK con `cron-cnmv-weekly` `ok`; Docker Compose healthy; `systemctl --failed` limpio; `source_assurance_gate.py` OK; `mcp_validation_suite.py --read-only` OK; `mcp_deep_contract_audit.py` OK. Backup/offsite restore sigue diferido.
- 2026-05-31 - `[COMPLETADO LOCAL + VPS]` `V2-EURLEX-ESMA-AEAT-SPRINT-01`: abierto por instruccion explicita del usuario para empezar por EUR-Lex/ESMA y AEAT sin relajar el freeze de veracidad. Cambios: `eurlex_market` incorpora MiFIR review 2024 (`32024R0791`), MiFID II review (`32024L0790`), MAR (`32014R0596`), RTS 1 (`32017R0587`) y RTS 2 (`32017R0583`) con textos oficiales EUR-Lex/Publications Office; `mica.py` carga CSV oficiales ESMA no-CASP (`OTHER.csv`, `ARTZZ.csv`, `EMTWP.csv`, `NCASP.csv`) en `mica_register_entry`, separada de CASP, con `source_url`, `source_hash`, `capture_date`, RLS y lectura `esdata_api`; `/v1/mica/registers` expone esas filas con `quality_signal=official_esma_register`; los gates MCP/deep/final verifican CELEX nuevos y MiCA no-CASP; AEAT anade regresion para que el 128 siga no afirmable por cobertura tecnica si no hay prueba oficial directa. No incluye descarga masiva FIRDS ni promocion de campanas AEAT por inferencia. Validacion local: `52 passed`; Ruff focal OK; `py_compile` OK; `verify-doc-contracts.py` OK; `source_assurance_gate.py` OK; compose config OK; `alembic heads` unico `20260531_0105_mica_register_api_rls`; `git diff --check` OK. Evidencia VPS commits `18873f2`, `36f2a61`, `7d5b5fb`, `1404f5c`: deploy Compose OK; Alembic aplicado hasta `20260531_0105`; `worker-eurlex-market` cargo 5 actos/90 articulos (`32024R0791=2`, `32024L0790=4`, `32014R0596=41`, `32017R0587=22`, `32017R0583=21`) con `verified=true` y `completeness=completa`; `cron-mica-weekly` sincronizo 208 CASP y 1055 filas MiCA no-CASP (`white_papers_other=867`, `emt_issuers=39`, `non_compliant_entities=149`; `art_issuers=0` oficial vacio); `/v1/mica/registers?register_type=emt_issuers&limit=1` devuelve `safe_to_answer=true`, `verified=true`, `source_hash` y `capture_date`; `/health` OK; `/status` OK con `cron-mica-weekly` y `worker-eurlex-market` verdes; final gate `ok=true`; `esdata-mcp-validation.service` `Result=success`; `esdata-mcp-deep-audit.service` `Result=success`; `systemctl --failed` limpio.
- 2026-05-31 - `[COMPLETADO LOCAL + VPS]` `CNMV-SANCIONES-EXPANSION-01`: expansion acotada solicitada por el usuario para mejorar CNMV y ordenar siguientes fuentes sin copiar codigo externo. Cambios: `apps/workers/cnmv.py` descubre el registro publico oficial de sanciones CNMV con limite `CNMV_SANCIONES_MAX_PAGES`, normaliza `sancion_cnmv`/`sanciones_cnmv`, conserva severidad de infraccion y referencia BOE cuando existe, y respeta `ambito` de metadata para evitar falsos positivos por heuristica textual; vocabularios `apps/workers`, `apps/api` y `libs/python/esdata_common` aceptan los nuevos valores; `mcp_validation_suite.py` excluye sanciones del gate de `sujeto_obligado` porque no son documentos de obligacion por perfil. `apps/api/routers/cnmv.py` expone la familia como `partial_loaded`; despliegue cableado en `.env.example`, `infra/deploy/compose.env.example`, `infra/deploy/docker-compose.prod.yml` y `docs/environment-variables.md`; `docs/source-expansion-backlog-2026-05-17.md` fija siguientes pasos BORME/EUR-Lex/ESMA/AEAT. Validacion local: worker/API CNMV `93 passed, 1 skipped`; vocabulario `45 passed`; maintenance agents `27 passed`; Ruff focal `F,I` OK; `py_compile` OK; `verify-doc-contracts.py` OK; `source_assurance_gate.py` OK; compose config OK; `git diff --check` OK. Evidencia VPS commit `285c1cf`: deploy Docker Compose OK; `cron-cnmv-weekly` acotado a `sanciones` procesa 4 URLs sin warnings de vocabulario; DB `sancion_cnmv/sanciones_cnmv=4`; `/v1/cnmv/coverage` muestra `sanciones_cnmv` `partial_loaded`, `loaded_count=4`; `/v1/cnmv?tipo_documento=sancion_cnmv&ambito=sanciones_cnmv` devuelve 4 filas con `row_provenance=official_exact`; `/health` OK; `/status` `api=ok`, `database=ok`, `workers=41`, `stale=0`, `errors=0`, `worker-cnmv` sigue `partial` solo por los 20 fetch failures externos ya conocidos; `systemctl --failed` limpio; `source_assurance_gate.py` OK en VPS; `esdata-mcp-validation.service` `ok=true`; `esdata-mcp-deep-audit.service` `ok=true`.
- 2026-05-31 - `[COMPLETADO LOCAL + VPS]` `GH-SCRAPER-AEPD-BDE-01`: implementacion de las fases seguras derivadas de `GH-PUBLIC-SCRAPER-REVIEW-01` para mejorar fuentes parciales sin copiar codigo externo. Cambios: `apps/workers/aepd.py` anade discovery de resoluciones AEPD desde RSS oficial y enumeracion acotada opcional por patron de PDF, con limites `AEPD_MAX_*` y sin elevar `coverage_status=partial`; `apps/workers/bde.py` anade discovery opcional de circulares BdE desde el listado oficial vigente, metadata de discovery si la tabla tiene columna `metadata`, telemetria partial cuando no descubre URLs, aislamiento por URL fallida, y conserva seeds existentes. `infra/deploy/docker-compose.prod.yml`, `infra/deploy/compose.env.example`, `.env.example` y docs de entorno/despliegue cablean `AEPD_DISCOVER_RESOLUTIONS`, `AEPD_ENUMERATE_RESOLUTIONS`, `BDE_DISCOVERY_ENABLED` y limites operativos. Fix operativo posterior: `scripts/ops/deploy-hetzner.sh` reconstruye explicitamente imagenes `cron-*` del profile `cron` para evitar cron jobs con codigo antiguo tras deploy; `scripts/tests/test_deploy_hetzner.py` fija el contrato. Validacion local: tests focales AEPD/BDE `26 passed`; `py_compile` OK; Ruff focal OK; `verify-doc-contracts.py` OK; `source_assurance_gate.py` OK; `docker compose ... config --quiet` OK; tests focales deploy OK; `bash -n scripts/ops/deploy-hetzner.sh` OK; `git diff --check` OK. Evidencia VPS commit `c284716`: deploy Docker Compose OK con reconstruccion explicita de todos los `cron-*`; imagenes cron verificadas con `aepd_has_rss=True` y `bde_has_discovery=True`; `cron-aepd-weekly` acotado con seeds vacios y RSS oficial procesa `5`, almacena `5`; `/v1/aepd?tipo_documento=resolucion_aepd&limit=1` devuelve `AEPD-ps-00143-2025`; `cron-bde-weekly` con discovery oficial procesa `2`, almacena `2`; `/v1/bde?limit=3` devuelve circulares `BDE-1-2025` y `BDE-1-2024`; `/status` `api=ok`, `database=ok`, `workers=41`, `stale=0`, `errors=0`; `/health` OK; `systemctl --failed` limpio; `esdata-mcp-validation.service` `ok=true`; `esdata-mcp-deep-audit.service` `ok=true`.
- 2026-05-31 - `[COMPLETADO LOCAL]` `GH-PUBLIC-SCRAPER-REVIEW-01`: revision de repositorios GitHub publicos para fuentes parciales/mejorables de ESData. Nuevo `docs/github-public-scraper-review-20260531.md` contrasta candidatos por dominio y licencia: AEPD resoluciones (`worldwidelaw/legal-sources` AGPL como patron, `ibernale/ai-lawyer` sin licencia), BDE circulares (`ibernale/ai-lawyer`), CENDOJ (`edusu/spanish-jurisprudence-search`, `GuiGel/mcp-cendoj`), CNMV sanciones (`worldwidelaw/legal-sources`, `vitamina-k/es-acc`), ESMA FIRDS (`European-Securities-Markets-Authority/esma_data_py`, `opensanctions/opensanctions`, `bunburya/eu_finreg_data`) y BORME (`PabloCastellano/bormeparser`, `ComputingVictor/MCP-BOE`). Decision: no copiar codigo AGPL/GPL ni corpus sin licencia; reimplementar solo patrones oficiales. Siguiente bloque recomendado: `GH-SCRAPER-AEPD-BDE-01` para discovery propio de resoluciones AEPD y circulares BDE con limites, tests, deploy VPS y gates MCP.
- 2026-05-31 - `[COMPLETADO LOCAL + VPS]` `OPS-SOURCES-JOBS-REVIEW-V1.15.4-01`: revision viva de proyecto/fuentes/jobs tras el cierre administrativo. Hallazgos: contenedores Docker Compose sanos, `/health` OK, `systemctl --failed` limpio, `/status` con `41` workers, `0` stale y `0` errors; fuentes principales pobladas y trazadas; `cron-aepd-weekly` limpiado con run manual OK; `worker-cnmv` seguia parcial por 20 fetch failures externos y el run CNMV revelo dos defectos internos. Fixes: `apps/workers/cnmv.py` trata binarios Office legacy (`.doc/.xls/.ppt`/OLE) como metadata oficial parcial para evitar bytes NUL en PostgreSQL, y etiqueta `modelos_esi`/`normativa_esi` con `sujeto_obligado=['sociedad_valores']`; tests `apps/workers/tests/test_cnmv.py` cubren ambos casos. Saneamiento VPS: 38 filas `CNMV-MODELO-ESI-*` actualizadas con `sujeto_obligado`; `cnmv_rows_missing_sujeto_obligado=0`. Validacion local: `pytest apps\workers\tests\test_cnmv.py -q` => `75 passed, 1 skipped`; Ruff focal OK; `py_compile` OK; `verify-doc-contracts.py` OK; `source_assurance_gate.py` OK; `git diff --check` OK. Evidencia VPS commit `812c295`: deploy OK; `esdata-mcp-validation.service` `ok=true`; `esdata-mcp-deep-audit.service` `ok=true`; `/health` OK; `systemctl --failed` limpio. Queda como residual no bloqueante: `worker-cnmv` aparece `partial` en `/status` por 20 documentos CNMV que no responden al fetch externo; no hay error de transporte MCP ni fallo semantico. Backup/offsite restore sigue diferido.
- 2026-05-31 - `[COMPLETADO LOCAL + VPS]` `ADMIN-CLOSEOUT-V1.15.3-01`: cierre administrativo solicitado por el usuario despues de diferir backup/offsite restore. Cambios: `PROJECT_STATE.md` fija `v1.15.3` como tag administrativo actual y resume evidencia viva de BDNS/BORME, salud VPS y validacion MCP; `RELEASE-CLOSEOUT.md` anade snapshot de cierre y explicita que `BACKUP-OFFSITE-RESTORE-01` queda como bloque operativo separado, sin reclamo de disaster-recovery readiness. Validacion local: `verify-doc-contracts.py` OK; `source_assurance_gate.py` OK; `git diff --check` OK. Evidencia VPS reutilizada del cierre BDNS/BORME: `/health` OK, `systemctl --failed` limpio, `esdata-mcp-validation.service` `ok=true`, DB `weak_normas=0`, `cnmv_missing_sujeto=0`, `bdns_unknown=0`; VPS actualizado por `git pull --ff-only` al commit/tag de documentacion.
- 2026-05-31 - `[COMPLETADO LOCAL + VPS]` `V2-BDNS-BORME-STRUCTURED-INGESTION-01`: abierta por decision explicita del usuario tras revisar repos publicos (`bdns-fetch`, `bdns_etl`, `borme_db`, `mcp-govern` y relacionados) para reaprovechar patrones sin copiar codigo GPL/AGPL. Cambios: `worker-bdns` pagina endpoints oficiales estructurados de BDNS (`convocatorias`, `concesiones` y familias relacionadas), persiste metadatos/procedencia exacta en `documento_interpretativo`, mantiene `/v1/bdns` fail-closed con filtros estructurados y `safe_to_answer=false`; `worker-borme` conserva API oficial BOE/sumario, persiste metadatos heuristicas de empresas/personas/cargos desde PDF oficial y `/v1/borme` filtra por empresa relacionada; variables de despliegue, matriz de cobertura, registro de fuentes y OpenAPI GPT regenerados. Estabilizacion VPS: concesiones BDNS sin identificador oficial usan digest estable, se evita `BDNS-CONCESION-unknown`, los workers largos refrescan heartbeat, y se corrige drift recurrente DORA/MiCA/CNMV que dejaba `esdata-mcp-validation.service` fallando tras redeploy. Validacion local final: `170 passed, 1 skipped`; doc artifacts `20 passed`; `source_assurance_gate.py` OK; `verify-doc-contracts.py` OK; Ruff focal OK; `py_compile` OK; `git diff --check` OK. Evidencia VPS commit `905cadc`: deploy Docker Compose OK; `/health` OK; `systemctl --failed` limpio; `mcp_validation_suite.py --read-only` via systemd `ok=true`; DB `weak_normas=0`, `cnmv_missing_sujeto=0`, `bdns_unknown=0`; `/v1/bdns?tipo_documento=concesion_bdns&limit=1` devuelve `BDNS-CONCESION-concesion-*`; `/v1/borme?limit=1` devuelve acto BORME oficial.
- 2026-05-31 - `[COMPLETADO LOCAL + VPS]` `P0.6-ANSWER-VERIFIABILITY-01`: endurecida la fiabilidad de respuestas MCP/API para que toda respuesta accionable pueda verificarse desde la propia respuesta. Cambios: `/v1/consulta` propaga `source_url`/`source_hash` a `cited_chunks` y `claim_citations`; `result_metadata.source_verification` lista URLs verificables y regla de uso; si `safe_to_answer=true` no tiene fuente verificable se degrada a evidencia limitada; politica MCP exige citar fuente o abstenerse; nuevo `scripts/maintenance/response_verifiability_gate.py`; OpenAPI GPT regenerados. Validacion local: `response_verifiability_gate.py` OK; `source_assurance_gate.py` OK; `pytest` focal `20 passed`; `verify-doc-contracts.py` OK; Ruff focal OK; `py_compile` OK; `git diff --check` OK. VPS `d799a10`: API/ops reconstruidos; `/health` OK tras arranque; `systemctl --failed` limpio; `response_verifiability_gate.py` OK; `source_assurance_gate.py` OK; consulta real `IVA entregas de bienes` devuelve `safe_to_answer=true`, `source_verification.status=verifiable`, `verifiable_source_count=10`, `cited_chunks=5`, `claim_citations=12`; final gate `ok=true`; `mcp_validation_suite.py --read-only` OK con `tool_count=91`; `mcp_deep_contract_audit.py` OK con `expected_operations=91`, `tools_returned=91`, `warning_count=0`. Tag de cierre endurecido: `v1.15.2`.
- 2026-05-31 - `[COMPLETADO LOCAL + VPS]` `P1-MAINTENANCE-CLOSEOUT-01`: cerrado P1 tras auditoria exhaustiva sin abrir v2. Cambios: nuevo `esdata-mcp-deep-audit.service/timer` versionado para ejecutar `mcp_deep_contract_audit.py` diariamente a las `08:45 Europe/Madrid` desde el contenedor `ops`; `RELEASE-CLOSEOUT.md` exige deep audit verde; `PROJECT_STATE.md` fija v1 en mantenimiento y tag funcional `v1.15.0`; runbook Compose instala/habilita el nuevo timer. Validacion local: `pytest scripts\tests\test_deploy_hetzner.py::test_maintenance_agent_units_are_safe_by_default scripts\tests\test_deploy_hetzner.py::test_esdata_timers_pin_europe_madrid_timezone -q` => `2 passed`; `verify-doc-contracts.py` OK; Ruff focal OK; `git diff --check` OK. VPS `aecdf62`: `git pull --ff-only`; unit/timer instalados en `/etc/systemd/system`; `systemd-analyze verify` OK; `systemctl enable --now esdata-mcp-deep-audit.timer` programado con siguiente ejecucion `2026-06-01 08:45 CEST`; `systemctl start esdata-mcp-deep-audit.service` termina `Result=success`, `ExecMainStatus=0`, log `ok=true`. Backup/offsite restore sigue fuera por decision previa del usuario y queda como P0 diferido separado.
- 2026-05-31 - `[COMPLETADO LOCAL + VPS]` `P0.5-SOURCE-ASSURANCE-01`: auditoria de aseguramiento de fuentes para bloquear sobreclaims catastroficos. Cambios: nuevo `docs/source-assurance-certification-20260531.md` fija que ningun dominio se certifica como exhaustivo; `scripts/maintenance/source_assurance_gate.py` bloquea lenguaje peligroso tipo `all/full/complete` en docs/OpenAPI/API y exige estados concretos por dominio; CDI/GIIN/PSD2 quedan como inventario-only hasta anadir campos de procedencia por fila; manual, OpenAPI y descripciones MCP/API endurecidas contra claims amplios. Validacion local: `source_assurance_gate.py` OK; `pytest` focal `23 passed`; `verify-doc-contracts.py` OK; Ruff focal OK; `py_compile` OK; `git diff --check` OK. VPS `924f042`: `git pull --ff-only`; API/ops reconstruidos tras purgar cache de build Docker corrupta; `/health` OK; `systemctl --failed` limpio; `source_assurance_gate.py` OK; final gate `ok=true`, `30/30`; `mcp_validation_suite.py --read-only` OK con `tool_count=91`; `mcp_deep_contract_audit.py` OK con `expected_operations=91`, `tools_returned=91`, `warning_count=0`. Backup/offsite restore y revision legal de reutilizacion masiva quedan como riesgos separados, no como claims cubiertos por este gate.
- 2026-05-31 - `[COMPLETADO LOCAL + VPS]` `SOURCE-CLOSEOUT-AUDIT-01`: auditoria exhaustiva de fuentes solicitada explicitamente por el usuario antes de continuar el proyecto. Se contrastaron fuentes documentadas, workers/ingesta, tablas, API/MCP, gates y estado vivo VPS contra `docs/final-product-coverage-matrix.md`. Hallazgos corregidos: matriz final demasiado amplia en familias como EUR-Lex/ESMA/MiCA/screening, registro de compliance limitado a AEAT, BDNS/CENDOJ sin metadatos fail-closed suficientes, BDE/BDNS/CENDOJ/SEPBLAC ausentes de HTTP MCP, endpoint reference obsoleta, y gate final de solo `13` checks. Cambios: nuevo `docs/source-closeout-audit-20260531.md`; `docs/reference/source-compliance-register.md` ampliado; `docs/final-product-coverage-matrix.md` desglosa dominios y estados; `scripts/maintenance/final_product_acceptance_gate.py` sube a `30` checks canonicos; `apps/api/routers/bdns.py` y `apps/api/routers/cendoj.py` devuelven `coverage_status`, `safe_to_answer`, `row_completeness` y `row_provenance`; `apps/api/mcp_catalog.py` incluye BDE/BDNS/CENDOJ/SEPBLAC; `docs/openapi-gpt*.json` regenerados. Evidencia local: tests focalizados `16 passed`; Ruff focal OK; `py_compile` OK; `verify-doc-contracts.py` OK; `git diff --check` OK. Evidencia VPS: `/health` OK; `/status` `41` workers, `0` stale, `0` errors; `systemctl --failed` limpio; final gate `ok=true`, `30/30`; `mcp_validation_suite.py --read-only` `ok=true`, `tool_count=91`; `mcp_deep_contract_audit.py` `ok=true`. Backup/offsite restore queda fuera de este bloque por decision previa del usuario.
- 2026-05-31 - `[COMPLETADO LOCAL / SUPERSEDED GATE BREADTH]` `FINAL-PRODUCT-DOCS-CLOSEOUT-01`: cerrado el bloque documental no-backup tras el primer gate transversal. Nuevo `docs/final-product-coverage-matrix.md` fija el claim final: ESData es util como API/MCP regulatorio transversal para consulta con evidencia, auditoria y aceleracion de revision humana; no es motor automatico de decision fiscal/legal/regulatoria. La matriz clasificaba dominios desde evidencia viva (`usable`, `partial`, `partial_traceable`, `very_limited`) y referenciaba el gate VPS inicial `1873445` con `/health` OK, `/status` OK, `41` workers, `0` stale, `0` errors, `systemctl --failed` limpio y `13/13` checks OK. `SOURCE-CLOSEOUT-AUDIT-01` amplia y sustituye ese gate por `30/30`, por lo que esta entrada queda como hito historico del primer smoke. Manual actualizado: `05-limites-alcance-y-estado-actual.md` explica cierre funcional y backup deferred; `07-mcp-y-clientes.md` lista dominios validados y semantica fail-closed; `12-onboarding-rapido.md` anade comando del gate; indices `docs/README.md` y `manual-usuario/README.md` enlazan la matriz. Validacion local: `python scripts\maintenance\verify-doc-contracts.py` => `docs contracts verified`; `git diff --check` OK. Backup/offsite restore queda fuera de este cierre por decision explicita del usuario y debe retomarse como bloque operativo separado.
- 2026-05-31 - `[COMPLETADO VPS / SUPERSEDED GATE BREADTH]` `FINAL-PRODUCT-ACCEPTANCE-GATE-01`: abierto por decision explicita del usuario y cerrado como primer gate ejecutable transversal, no como curacion AEAT incremental. Nuevo `scripts/maintenance/final_product_acceptance_gate.py` valida `/health`, `/status`, `/v1/domain-availability` y muestra canonica multi-dominio: BOE, AEAT, doctrina DGT/TEAC, CNMV, EUR-Lex/ESMA, MiCA/CASP, screening, AEPD, SEPBLAC y BDE. El gate falla con exit non-zero si falta fuente, estado de evidencia, fail-closed explicito en respuesta vacia, o salud operativa. Ajustes de contrato API: AEPD/BDE/SEPBLAC exponen `coverage_status`, `safe_to_answer=false`, `items`, `total` y `row_completeness=partial`; Screening expone `coverage_status` y `safe_to_answer` para entradas oficiales cargadas. Validacion local: `python -m pytest scripts\tests\test_final_product_acceptance_gate.py apps\api\tests\test_aepd_router.py apps\api\tests\test_partial_domain_contracts.py apps\api\tests\test_screening.py::test_screening_entries_list -q` => `10 passed`; Ruff focal OK; `ruff apps/api/schemas.py --select F,I --ignore F811` OK por redefiniciones historicas preexistentes; `py_compile` OK; `git diff --check` OK. VPS `5d946d8`: API reconstruida/recreada; `/health` OK; `/status` OK con `41` workers, `0` stale y `0` errors; `systemctl --failed` => `0 loaded units listed`; primer gate => `ok=true`, `13/13` checks OK. `SOURCE-CLOSEOUT-AUDIT-01` amplia y sustituye este smoke por `30/30`. Queda como riesgo separado del cierre de producto: backup/offsite restore no configurado si se exige cierre operativo total.
- 2026-05-31 - `[COMPLETADO LOCAL]` `FINAL-PRODUCT-ACCEPTANCE-PLAN-01`: versionado `docs/final-product-acceptance-plan.md` como contrato de cierre transversal de producto, no como curacion AEAT incremental. Basado en inventario vivo inicial: VPS sin unidades fallidas, `/health` OK, `/status` OK con `41` workers, `0` stale y `0` error; OpenAPI con `413` paths; HTTP MCP con `83` operaciones; corpus reales en AEAT, BOE legislacion, DGT/TEAC, CNMV, EUR-Lex/ESMA, CASP, screening y dominios parciales. El plan exige matriz por dominio desde evidencia viva, smoke transversal con exit non-zero, manual MCP final y decision explicita sobre backup/restore. El PRD v2 AEAT queda como expansion fiscal definida pero no como cierre de producto. Para ejecutar el cierre debe abrirse `final-product-acceptance-gate` con commit `feat(final): open product acceptance gate`.
- 2026-05-31 - `[MAINTENANCE MODE]` `PROJECT-MAINTENANCE-ONLY-01`: proyecto en modo solo mantenimiento tras v1 cerrada/congelada y v2 PRD definido como `DEFINED_NOT_STARTED`. Acciones permitidas sin abrir v2: corregir fallos operativos, seguridad, regresiones del gate, alertas reales, dependencias necesarias para mantener el servicio, y documentacion de estado. Acciones no permitidas: nuevas features, nuevas fuentes, ampliacion de claim, nuevos sprints de curacion o promocion de campanas AEAT. Para salir de mantenimiento debe existir decision explicita y commit `feat(v2): open sprint 1` con seccion `v2-sprint-1` en este roadmap.
- 2026-05-31 - `[COMPLETADO LOCAL]` `V2-EXPANSION-PRD-01`: versionado `docs/v2-expansion-prd.md` como contrato de alcance v2 definido pero no iniciado. Mantiene intacto el freeze/claim v1: consulta/auditoria fail-closed, sin decision fiscal automatica ni promocion de campanas por inferencia. OKRs v2 acotados: cobertura `resolved_strong` medible para P1 solo con evidencia oficial directa, 124/126/128 solo salen de `CONFLICT` si aparece evidencia oficial fresca, 303/390 como Sprint 2, 289 documental separado de CRS/DAC2 como Sprint 3. Gate v2: v1 sin regresiones durante al menos 2 ciclos Hermes, `/health`/`/status`/`mcp_validation_suite` OK, `systemctl --failed` limpio, legal `SKIPPED` ambiguo en 0%, y evidencias con HTTPS/hash/captura menor de 30 dias. Implementacion v2 no iniciada; requiere PRD en `main`, PRD desplegado en VPS, seccion `v2-sprint-1` con acceptance criteria y verification commands, y commit explicito `feat(v2): open sprint 1`.
- 2026-05-31 - `[SCOPE FROZEN]` `V1-USEFUL-CLOSEOUT-FREEZE`: release util v1 cerrada y scope congelado tras `RELEASE-CLOSEOUT-GATE-01`. No se anaden expansiones funcionales, nuevas fuentes, nuevas superficies MCP/UI ni automatizaciones de curacion dentro de v1 salvo mantenimiento correctivo, seguridad, fallos operativos o regresiones del gate. Cualquier mejora de producto, ampliacion de cobertura o cambio de claim debe abrirse explicitamente como v2 con nuevo contrato de alcance y gate propio. Estado operativo esperado: mantener timers/workers/Hermes fail-closed, monitorizar alertas reales y no promover campanas AEAT por inferencia.
- 2026-05-31 - `[COMPLETADO VPS]` `RELEASE-CLOSEOUT-GATE-01`: cerrado gate util v1 con contrato fail-closed en `RELEASE-CLOSEOUT.md` y snapshot honesto en `PROJECT_STATE.md`. Cambios: `extract_aeat_hermes_json.py` normaliza URLs `http://www.boe.es` a `https://www.boe.es`; `run-legal-sources.sh` queda versionado y sustituye `SKIPPED` ambiguo por `SKIPPED_EXISTING_REPORT` con razon; `run-ops-health.sh` evita `git status` sobre arboles de secretos y reporta branch/hash/cambios tracked sin warnings de permisos; `run-writer-summary-json.sh` bloquea summaries con legal `SKIPPED` ambiguo o `ERROR`. Validacion local: `python -m pytest scripts\tests\test_aeat_hermes_adapter.py scripts\tests\test_aeat_hermes_nightly_scripts.py -q` => `22 passed, 1 skipped`; `ruff` focal OK; `bash -n` OK; `git diff --check` OK. VPS en `53319ef`: `systemctl --failed` => `0 loaded units listed`; `hermes-esdata-curator.service Result=success ExecMainStatus=0`; `esdata-mcp-validation.service Result=success ExecMainStatus=0`; `/health` OK; `/status` autenticado OK; legal smoke sin llamadas Hermes genera `_legal-20260531-075241-summary.csv` con cinco `SKIPPED_EXISTING_REPORT`; `ops-20260531-075447.md` queda `Decision=HEALTHY` y `stderr=0`; `summary-20260531-075556.md` queda `Decision=NEEDS_HUMAN_REVIEW`, referencia el ops limpio y mantiene 124/126/128 `OK` en lote, resto `DAILY_CAP_REACHED`. Criterio mantenido: si un criterio no puede verificarse, el gate falla; 124/126/128 solo pueden salir de `CONFLICT`/`campana_safe_to_assert=false`/`human_review_required=true` con evidencia oficial directa y verificable.
- 2026-05-29 - `[COMPLETADO VPS]` `ESMA-MIFIR-REPORTING-429-RECOVERY-01`: cerrada unidad fallida `cron-esma-mifir-reporting-weekly`. Causa VPS reproducida en relanzamiento manual: ESMA devuelve HTTP `429 Too Many Requests` durante `fetch_reporting_documents()` tras varias descargas consecutivas. Cambio: `worker_esma_mifir_reporting.py` usa retry/backoff para 429 respetando `Retry-After` y throttle minimo `ESMA_MIFIR_REPORTING_REQUEST_DELAY_SECONDS=1.0` entre peticiones ESMA; errores HTTP no-429 siguen siendo fatales. Validacion local: `python -m pytest apps\workers\tests\test_worker_esma_mifir_reporting.py -q` => `4 passed`; Ruff focal OK; `py_compile` OK; `git diff --check` OK. Despliegue VPS `d9edb65`: `cron-esma-mifir-reporting-weekly` reconstruido, `systemctl start esdata-job@cron-esma-mifir-reporting-weekly.service` termina `Result=success`, `ExecMainStatus=0`; journal muestra `files=4 fields=168 documents=7 validation_rules=...`; `sync_log worker-esma-mifir-reporting status=ok errors=0`.
- 2026-05-29 - `[COMPLETADO VPS]` `MCP-VALIDATION-SYSTEMD-OPS-01`: cerrada unidad fallida `esdata-mcp-validation.service`. Causa VPS confirmada: la unit ejecutaba `/usr/bin/python3` del host y fallaba por dependencias ausentes (`pydantic`, `sqlalchemy.dialects:postgresql.psycopg`), mientras el contrato historico del repo indica ejecutar la suite desde contenedor `ops`. Cambio: unit systemd usa `docker compose --env-file /etc/esdata/esdata.env -f /srv/esdata/infra/deploy/docker-compose.prod.yml run --rm --no-deps ops python /workspace/mcp_validation_suite.py --read-only --base-url http://api:8000` y fija `DOCKER_CONFIG=/tmp/esdata-mcp-validation-docker`. Validacion local focal: `python -m pytest scripts\tests\test_deploy_hetzner.py::test_maintenance_agent_units_are_safe_by_default -q` => `1 passed`; Ruff focal OK; `git diff --check` OK. Despliegue VPS `7149a22`: `ops` reconstruido, unit instalada, `daemon-reload`, `systemctl start esdata-mcp-validation.service` termina `Result=success`, `ExecMainStatus=0`.
- 2026-05-29 - `[COMPLETADO VPS]` `HERMES-CURATOR-P0-RECOVERY-01`: cerrado P0 operativo Hermes tras fallo nocturno. VPS: permisos corregidos en `aeat-campaign-curation-raw`, `aeat-campaign-curation-rendered` y `aeat-campaign-curation-json`; `hermescurator` puede escribir en los tres directorios y `safe.directory=/srv/esdata` queda configurado a nivel system git. Smoke manual `run-aeat-model-json.sh 124` como `hermescurator` genera raw, extrae JSON, valida schema y renderiza markdown. Causa adicional detectada: flakiness del agente con raw de solo bootstrap sin bloque `BEGIN_AEAT_HERMES_JSON`; `run-aeat-model-json.sh` reintenta una vez solo ante `ERROR_EXTRACT`. Segunda causa detectada: `126` duplico `official_sources.source_id=AEAT_GH06`; `extract_aeat_hermes_json.py` deduplica fuentes repetidas conservando la primera, sin relajar validacion ni claims. Validacion local: `python -m pytest scripts\tests\test_aeat_hermes_adapter.py scripts\tests\test_aeat_hermes_nightly_scripts.py -q` => `19 passed, 1 skipped`; Ruff focal OK; `bash -n` OK; `git diff --check` OK. Despliegue VPS `4a7240f`: raw fallido de `126` reextrae y valida; `systemctl start hermes-esdata-curator.service` termina `Result=success`, `ExecMainStatus=0`; batch `/srv/esdata/hermes-curator/reports/aeat-campaign-curation-json/_batch-json-20260529-075024-summary.csv` deja `124/126/128=OK` y resto `DAILY_CAP_REACHED`; resumen `summary-20260529-075831.md` queda `status=OK`, `Decision=NEEDS_HUMAN_REVIEW`. No se promueve ninguna campana ni se escriben datos productivos.
- 2026-05-29 - `[COMPLETADO VPS]` `REG-WATCH-IVA-RATES-OPTIONAL-01`: cerrada alerta Telegram `WorkerSyncErrors`/`WorkerCronErrors` para `cron-regulatory-daily`. Causa confirmada en VPS: ejecucion planificada `2026-05-29 07:00 CEST` falla con `psycopg.errors.UndefinedTable: relation "iva_rates" does not exist` durante comparacion LIVA en `regulatory_watch.py`; `iva_rates` no existe en produccion y se trata como tabla opcional, no como prerequisito runtime. Cambio: guard read-only `_table_exists()` salta comparacion LIVA si falta `iva_rates` y deja warning explicito, sin crear tablas en runtime. Validacion local: `python -m pytest apps\workers\tests\test_regulatory_watch.py -q` => `2 passed`; `python -m ruff check apps\workers\regulatory_watch.py apps\workers\tests\test_regulatory_watch.py --select F,I` => OK; `git diff --check` OK. Despliegue VPS: `/srv/esdata` actualizado a `16c876b`, imagen `cron-regulatory-daily` reconstruida, smoke real via `systemctl start esdata-job@cron-regulatory-daily.service` termina `status=0/SUCCESS`; journal muestra `LIVA rate comparison skipped: optional table iva_rates is missing`; `sync_log id=2638 status=ok rows_processed=5 errors=0`.
- 2026-05-28 - `[COMPLETADO VPS]` `HERMES-CURATOR-RATE-LIMIT-HARDENING-01`: endurecido Hermes curator contra consumo automatizado de GPU/modelos. Nuevo `scripts/hermes_curator/bin/common.sh` versionado con maximo `1` contenedor ESData Hermes concurrente (`HERMES_ESDATA_MAX_ESDATA_HERMES=1`), minimo `30s` entre llamadas (`HERMES_ESDATA_MIN_DELAY_SECONDS=30`) y lock local `hermes-rate-limit.lock`/`hermes-last-call.epoch`. `run-aeat-modelos-json.sh` limita AEAT a `HERMES_ESDATA_MAX_MODELS_PER_RUN=3` por ejecucion y marca el resto como `DAILY_CAP_REACHED`; default de sleep sube a `30s`. Nuevo unit/timer versionados `infra/deploy/systemd/hermes-esdata-curator.{service,timer}` con esos defaults. Despliegue VPS: `common.sh` instalado en `/srv/esdata/scripts/hermes_curator/bin/common.sh` y `/srv/esdata/hermes-curator/bin/common.sh`; unit instalado en `/etc/systemd/system/` y `systemctl daemon-reload`; no se reinicia worker ni se ejecuta curacion real. Validacion local: `python -m pytest scripts\tests\test_aeat_hermes_nightly_scripts.py -q` => `5 passed, 1 skipped`; `ruff` focal OK; `bash -n` OK. Smokes VPS sin modelo real: fake `docker/timeout/sleep` confirma sleep de `30s` y rechazo con `capacity_smoke_rc=72` si ya hay 1 contenedor ESData; fake batch confirma `3` filas `OK` y `2` filas `DAILY_CAP_REACHED`.
- 2026-05-28 - `[COMPLETADO VPS]` `HERMES-CURATOR-FAILURE-PROPAGATION-01`: corregido el falso SUCCESS del nightly Hermes. VPS: `/srv/esdata/hermes-curator/reports/aeat-campaign-curation-json` queda `hermescurator:hermescurator` y escribible por el timer. Nuevo wrapper versionado `scripts/hermes_curator/bin/run-once.sh` propaga fallo de tramos y termina en `exit 1` con `FAILED`; desplegado tambien como `/srv/esdata/hermes-curator/bin/run-once.sh`. `run-aeat-modelos-json.sh` devuelve `exit 1` si cualquier modelo falla aunque escriba la fila de error en el CSV. Validacion local: `python -m pytest scripts\tests\test_aeat_hermes_nightly_scripts.py -q` => `3 passed, 1 skipped` (skip por Bash/WSL no usable en Windows); `ruff` focal OK; `git diff --check` OK. Smoke VPS aislado en `/tmp` ejecutado como `hermescurator`: fallo simulado en `aeat-modelos` produce `run_once_rc=1`, `ERROR aeat-modelos` y `FAILED`; fallo simulado en modelo produce `batch_rc=1` y `ERROR aeat json summary`. No se ejecuta curacion real, no se escribe DB, no se reinician workers.
- 2026-05-27 - `[COMPLETADO VPS]` `HERMES-AEAT-STRICT-HTML-EXCERPT-01`: afinado el prompt Hermes para reducir reparaciones de excerpts HTML sin relajar el contrato fiscal. Cambio: `scripts/hermes_curator/prompts/aeat_model_json.md` incorpora "Regla estricta de excerpt HTML": excerpt literal corto desde HTML descargable, linea visible exacta, prohibicion de locators genericos (`titulo de pagina`, `titulo del recurso`, `pagina de ayuda`, `body text`), prohibicion de locators desnudos (`<title>`/`<h1>` sin texto visible concreto) y exclusion de fuentes que solo prueben navegacion/pantalla/formulario/ayuda operativa. Test nuevo en `scripts/tests/test_aeat_hermes_adapter.py`. Validacion local: `python -m pytest scripts\tests\test_aeat_hermes_adapter.py scripts\tests\test_aeat_hermes_nightly_scripts.py scripts\tests\test_adjudicate_aeat_hermes_batch.py scripts\tests\test_summarize_aeat_adjudication_history.py -q` => `30 passed`; Ruff focal OK; docs contracts/artifacts OK; `git diff --check` OK. Evidencia VPS: primera pasada controlada `113/128/210` con commit `340c44b` bajo `repaired_excerpt_ratio` a `0.0`, pero introdujo `rewrite_ratio=0.333333` y `blocking_error_ratio=1.333333` por locators vagos en `210` (`<title>` a secas), por tanto no fue aceptada como mejora. Ajuste posterior commit `f0daa07` exige locator concreto; regenerado `210` y adjudicado latest-per-model con history `/srv/esdata/hermes-curator/reports/aeat-adjudication-metrics/20260527T195354Z.json`: `reports_total=3`, `auto_accepted_total=3`, `rewrite_or_reject_total=0`, `blocking_errors_total=0`, `unused_source_warnings_total=0`, `assertable_candidates_total=0`, `repaired_excerpts_total=2`, `repaired_excerpt_ratio=0.666667`, alertas `[]`. No se escriben datos productivos ni se promueve campana.
- 2026-05-27 - `[COMPLETADO VPS]` `HERMES-AEAT-SIGNAL-QUALITY-210-113-01`: reducida la deuda de madurez detectada por el resumen de tendencias sin relajar el contrato fiscal. Diagnostico: `210` concentraba reparaciones por excerpts internos/cacheados en `official_sources.excerpt`; `113` incluia fuentes no usadas y formularios transaccionales `www1/.../OV16-M113` como evidencia. Cambios: `validate_aeat_hermes_report.py` rechaza excerpts oficiales con terminos internos (`cache`, `metadata`, `MCP`, `ESData`, `modelo_recurso`, `campana_activa`, `campana_persistida`) y URLs transaccionales AEAT como fuentes probatorias; el prompt Hermes exige fuentes usadas por claims, claims no afirmativos para identidad/cobertura oficial y prohibe BOE/normas no devueltas por MCP; `run-aeat-model-json.sh` publica JSON solo tras validacion (`*.json.tmp` + `mv`); `extract_aeat_hermes_json.py` sanea deterministicamente fuentes transaccionales/no referenciadas antes de validar y convierte claims transaccionales eliminados en `rejected_claims`. Validacion local final: `python -m pytest scripts\tests\test_validate_aeat_hermes_report.py scripts\tests\test_aeat_hermes_adapter.py scripts\tests\test_aeat_hermes_nightly_scripts.py scripts\tests\test_adjudicate_aeat_hermes_batch.py scripts\tests\test_summarize_aeat_adjudication_history.py -q` => `39 passed`; Ruff focal OK; `py_compile` OK; docs contracts/artifacts OK; `git diff --check` OK. Evidencia VPS commit `7d09a3d`: regenerados `210` y `113`; batch latest-per-model con history `/srv/esdata/hermes-curator/reports/aeat-adjudication-metrics/20260527T170043Z.json` => `reports_total=3`, `auto_accepted_total=3`, `rewrite_or_reject_total=0`, `blocking_errors_total=0`, `unused_source_warnings_total=0`, `assertable_candidates_total=0`, `repaired_excerpts_total=2`, ratios `repaired_excerpt_ratio=0.666667`, `rewrite_ratio=0.0`, `blocking_error_ratio=0.0`, `unused_source_warning_ratio=0.0`. Resumen de tendencia: `runs_total=5`, alertas `[]`, delta vs run anterior `repaired_excerpt_ratio=-0.333333`, `rewrite_ratio=-0.333333`, `blocking_error_ratio=-0.333333`. No se escriben datos productivos ni se promueve campana.
- 2026-05-27 - `[COMPLETADO VPS]` `HERMES-AEAT-ADJUDICATION-TRENDS-01`: anadido resumen operativo de tendencias sobre el historico persistido del adjudicador Hermes AEAT. Nuevo script `scripts/maintenance/summarize_aeat_adjudication_history.py`: carga historicos JSON, expone ultimo run, deltas vs run anterior y alertas por umbral para `repaired_excerpt_ratio`, `rewrite_ratio`, `blocking_error_ratio`, `unused_source_warning_ratio` y `assertable_candidate_ratio`; `--fail-on-alert` permite usarlo como gate si se quiere endurecer. Tests nuevos en `scripts/tests/test_summarize_aeat_adjudication_history.py`; docs actualizadas en `docs/aeat/hermes-curation-output-contract.md` y `docs/aeat/ai-agents.md`. Validacion local commit `37f7c7d`: `python -m pytest scripts\tests\test_summarize_aeat_adjudication_history.py scripts\tests\test_adjudicate_aeat_hermes_batch.py -q` => `14 passed`; Ruff focal OK; `py_compile` OK; docs contracts/artifacts OK; `git diff --check` OK. Evidencia VPS: `git pull --ff-only` a `37f7c7d`; `python3 scripts/maintenance/summarize_aeat_adjudication_history.py /srv/esdata/hermes-curator/reports/aeat-adjudication-metrics` => `runs_total=1`, ultimo historico `20260527T144300Z.json`, deltas `null` por falta de run previo, alertas warning `repaired_excerpt_ratio=2.0 > 1.0` y `unused_source_warning_ratio=0.333333 > 0.20`, sin alerta critica porque `blocking_error_ratio=0.0`. Lectura: pipeline estable pero calidad semantica aun depende demasiado de reparacion de excerpts; no hay promocion de campanas ni escritura productiva.
- 2026-05-27 - `[COMPLETADO VPS]` `HERMES-AEAT-ADJUDICATION-HISTORY-01`: persistido historico de madurez del adjudicador Hermes AEAT para medir tendencia, no ejecuciones aisladas. `adjudicate_aeat_hermes_batch.py` incorpora `--history-dir` para escribir JSON timestamped con `run_metadata`, hashes/versiones de schema/prompt/adjudicator, Git head, metricas agregadas, ratios (`repaired_excerpt_ratio`, `rewrite_ratio`, `blocking_error_ratio`, `unused_source_warning_ratio`, `assertable_candidate_ratio`) y drilldown por modelo solo para diagnostico. Regla: el dashboard primario es agregado; el drilldown se mira solo cuando el agregado degrada. Validacion local: `python -m pytest scripts\tests\test_adjudicate_aeat_hermes_batch.py scripts\tests\test_aeat_hermes_adapter.py scripts\tests\test_audit_aeat_hermes_integration.py scripts\tests\test_validate_aeat_hermes_report.py -q` => `28 passed`; Ruff focal OK; `py_compile` OK; docs contracts/artifacts OK; `git diff --check` OK. Evidencia VPS commit `57a8965`: generado `/srv/esdata/hermes-curator/reports/aeat-adjudication-metrics/20260527T144300Z.json` con `reports_total=3`, `auto_accepted_total=3`, `rewrite_or_reject_total=0`, `blocking_errors_total=0`, `repaired_excerpt_ratio=2.0`, `unused_source_warning_ratio=0.333333`, drilldown reparaciones `210=4`, `113=2` y warning no usado `113=1`.
- 2026-05-27 - `[COMPLETADO VPS]` `HERMES-AEAT-BATCH-ADJUDICATION-METRICS-01`: anadidas metricas operativas al adjudicador batch para evitar leer informe por informe y medir madurez real. `adjudicate_aeat_hermes_batch.py` emite `metrics` con recuentos por decision, machine_decision, bucket, estado recomendado, autoaceptados, candidatos afirmables, rewrites/rechazos, excerpts reparados, warnings de fuentes no usadas y errores bloqueantes. Regla documentada: `auto_accept_*` significa admision a draft/review, no evidencia canonica, no escritura DB y no resolucion de campana. Validacion local: `python -m pytest scripts\tests\test_adjudicate_aeat_hermes_batch.py scripts\tests\test_aeat_hermes_adapter.py scripts\tests\test_audit_aeat_hermes_integration.py scripts\tests\test_validate_aeat_hermes_report.py -q` => `27 passed`; Ruff focal OK; `py_compile` OK; docs contracts/artifacts OK; `git diff --check` OK. Evidencia VPS sobre `/srv/esdata/hermes-curator/reports/aeat-campaign-curation-json` con `--verify-sources --latest-per-model`: `reports_total=3`, `by_decision.CONFLICT=3`, `auto_accepted_total=3`, `human_review_required_total=0`, `rewrite_or_reject_total=0`, `assertable_candidates_total=0`, `repaired_excerpts_total=6`, `unused_source_warnings_total=1`, `blocking_errors_total=0`.
- 2026-05-27 - `[COMPLETADO VPS]` `HERMES-AEAT-BATCH-ADJUDICATION-01`: sustituida la revision humana modelo-a-modelo por adjudicacion automatica read-only de informes Hermes AEAT. Nuevo script `scripts/maintenance/adjudicate_aeat_hermes_batch.py`: valida schema, admission gate, URL oficial, locator y excerpt contra contenido oficial vivo; para fuentes binarias oficiales (`.pdf`, `.xls`, `.xlsx`, `.zip`) verifica alcanzabilidad y no exige excerpt literal; incorpora `--latest-per-model`, sugerencia automatica de excerpt literal cuando Hermes usa titulo/parafrasis no literal, y solo bloquea fallos de fuentes referenciadas por `official_source_claims` (fuentes no usadas quedan como warning). Se endurece `scripts/hermes_curator/prompts/aeat_model_json.md` para exigir excerpts literales verificables y se endurece `extract_aeat_hermes_json.py` para usar el ultimo bloque parseable cuando Hermes emite ruido de diff. Validacion local acumulada: `python -m pytest scripts\tests\test_aeat_hermes_adapter.py scripts\tests\test_adjudicate_aeat_hermes_batch.py scripts\tests\test_audit_aeat_hermes_integration.py scripts\tests\test_validate_aeat_hermes_report.py -q` => `26 passed`; Ruff focal OK; `py_compile` OK; docs contracts/artifacts OK; `git diff --check` OK. Evidencia VPS: primer run sobre 6 JSON historicos detecto 0 autoaceptados, 5 rewrites por excerpts parafraseados y 1 schema invalido; tras prompt/extractor/adjudicator, `128` se regenera y queda `auto_accept_conflict_evidence`; `113` se regenera y queda `auto_accept_conflict_evidence` con 1 warning de fuente no usada; `210` queda `auto_accept_conflict_evidence` tras reparacion automatica de excerpts. Batch final `--verify-sources --latest-per-model` sobre el directorio JSON: `reports=3`, `auto_accept_conflict_evidence=3`, `draft/conflict=3`, errores bloqueantes `0`. No escribe DB, no promueve `resolved_strong`, no relaja `ASSERTABLE_DIRECT_OFFICIAL`.
- 2026-05-27 - `[COMPLETADO LOCAL]` `HERMES-AEAT-HUMAN-REVIEW-CHECKLIST-HARDENING-01`: endurecido el checklist de revision humana de Hermes AEAT para evitar aceptaciones laxas. `docs/aeat/hermes-human-review-checklist-2026-05-27.md` incorpora rechazo automatico por claims sin URL primaria, locator, excerpt, mezcla MCP/API/cache/metadata, normas no presentes en fuentes, afirmaciones de campana con `safe_to_assert=false`, `proves_campaign=true` desde senales tecnicas o stale sin claim exacto. La plantilla de aceptacion ahora exige campos obligatorios por claim aceptado/rechazado, `automatic_rejection_checked=true`, `automatic_rejection_reason=none`, scope aceptado, razon de aceptacion y recuento explicito de claims que prueban campana. Si falta un campo obligatorio, solo se permite `needs_report_rewrite`. No se tocan datos ni estados.
- 2026-05-27 - `[COMPLETADO LOCAL]` `HERMES-AEAT-HUMAN-REVIEW-CHECKLIST-01`: creado checklist operativo para revision humana de los JSON Hermes AEAT admitidos por el gate. Nuevo documento `docs/aeat/hermes-human-review-checklist-2026-05-27.md`, enlazado desde `docs/aeat/ai-agents.md`. Alcance: modelos `210`, `128` y `113`, todos con `decision=CONFLICT`, `proves_campaign=0`, `campana_safe_to_assert=false`; el checklist fuerza revisar URL primaria, locator, excerpt, separacion de scope tecnico/legal/campana, y cierre en `accept_as_conflict_evidence`, `accept_as_stale_suspected_evidence`, `needs_report_rewrite` o `reject_report`. No se toca DB ni se promueve campana.
- 2026-05-27 - `[COMPLETADO VPS]` `HERMES-AEAT-EVIDENCE-ADMISSION-01`: creado gate explicito para decidir si los JSON validados de Hermes son admisibles como evidencia documental revisable. Nuevo script read-only `scripts/maintenance/audit_aeat_hermes_integration.py` y tests `scripts/tests/test_audit_aeat_hermes_integration.py`; documentacion actualizada en `docs/aeat/hermes-curation-output-contract.md` y `docs/aeat/ai-agents.md`. Regla: `integrable=true` no promueve campana ni escribe datos; solo significa que hay claims oficiales trazables para revision humana. Validacion local: `python -m pytest scripts\tests\test_audit_aeat_hermes_integration.py scripts\tests\test_validate_aeat_hermes_report.py -q` => `13 passed`; Ruff focal OK; `py_compile` OK; `verify-doc-contracts.py` OK; `verify-doc-artifacts.py --ci-baseline` OK; `git diff --check` OK. Validacion VPS commit `b20a774d`: `210`, `128` y `113` auditados con JSON validado. Resultados: `210` integrable como evidencia humana, `decision=CONFLICT`, `official_claims=4`, `proves_campaign=0`, `recommended_state=conflict`; `128` integrable, `decision=CONFLICT`, `official_claims=4`, `proves_campaign=0`, `recommended_state=conflict`; `113` no tenia JSON previo, se genero con runner estricto y queda integrable, `decision=CONFLICT`, `official_claims=4`, `proves_campaign=0`, `recommended_state=conflict`. No se toca DB, no se promueve `resolved_strong` y todos mantienen `campana_safe_to_assert=false`.
- 2026-05-27 - `[COMPLETADO VPS]` `EU-SANCTIONS-FSF-403-RECOVERY-01`: investigada y corregida la DLQ real que el monitor Hermes empezo a mostrar para `eu_sanctions | sync_list:EU_SANCTIONS`. Causa raiz: el worker `apps/workers/eu_sanctions.py` usaba el endpoint FSF XML 1.1 sin token (`.../xmlFullSanctionsList_1_1/content`), que desde VPS devuelve HTTP 403 y setea cookies/XSRF; el catalogo oficial `data.europa.eu` publica el `dcat:downloadURL` XML 1.1 completo con `?token=dG9rZW4tMjAxNw`. Verificacion oficial: pagina Comision/DG FISMA enlaza `Financial sanctions: Consolidated list` y el dataset `data.europa.eu` marca la distribucion XML como publica/downloadable. Probe VPS del URL con token: HTTP 200, `Content-Type: application/xml`, `Content-Length: 24749570`, `Last-Modified: Fri, 08 May 2026`, fichero `20260508-FULL-1_1(xsd).xml`, contenido XML valido. Cambio: default `EU_SANCTIONS_XML_URL` actualizado al `downloadURL` oficial con token, manteniendo override por entorno; tests nuevos bloquean regresion al URL sin token. Validacion local focal: `python -m pytest apps\workers\tests\test_eu_sanctions.py -q` => `4 passed`; Ruff focal OK; `py_compile` OK; `git diff --check` OK. Validacion VPS commit `7b735c1`: `cron-eu-sanctions-weekly` reconstruido y ejecutado una vez; `sync_log id=2448 status=ok`, `documentos_processed=5996`, `documentos_upserted=5996`, `errors=0`; `screening_lists.EU_SANCTIONS actualizada=2026-05-08`, `screening_entries EU_SANCTIONS=5996 activos`; `/v1/screening/entries?codigo=EU_SANCTIONS&limit=1` devuelve `total=5996` y fuente `European Commission - DG FISMA`; DLQ historica `id=6` marcada resuelta con nota del fix; monitor one-shot posterior: API OK, domain availability OK, `/status` lee 41 workers, unhealthy baja de 7 a 6, `cron-eu-sanctions-weekly` ya no aparece unhealthy y `DLQ: No entries exceeding max retries`. Nota: `scripts/tests/test_seed_screening_worker.py` no se uso como gate local porque requiere PostgreSQL local con rol `esdata`, inexistente en este entorno.
- 2026-05-27 - `[COMPLETADO VPS]` `HERMES-MONITOR-SERVICE-OBSERVABILITY-01`: reparado y desplegado el monitor Hermes versionado como fase separada del switch JSON. Diagnostico inicial VPS: `hermes-monitor.service` seguia en bucle `activating (auto-restart)` por `ExecStart=/usr/bin/python3 /usr/local/bin/hermes_monitor.py --interval 300 --api-url http://localhost:8000 --auto-restart`, argumento inexistente. Validacion previa: `systemd-analyze verify /srv/esdata/infra/deploy/systemd/esdata-hermes-monitor.service` OK; `--daemon` existe; usuario `deploy` puede leer `/etc/esdata/esdata.env` y escribir en `/var/log/esdata`. Despliegue: backup del unit viejo `/etc/systemd/system/hermes-monitor.service.bak-20260527-073325`, `hermes-monitor.service` parado/deshabilitado, `esdata-hermes-monitor.service` instalado desde repo, habilitado y arrancado. Hallazgo durante primer arranque: fallback DLQ fallaba por `DOCKER_CONFIG` bajo `ProtectHome=true` y el log de auto-restart decia falsamente `within grace period` aunque `AUTO_RESTART_ENABLED=false`; corregido en `scripts/hermes_monitor.py` y `infra/deploy/systemd/esdata-hermes-monitor.service` fijando `DOCKER_CONFIG=/tmp/esdata-hermes-monitor-docker` y mensajes de skip diferenciados. Evidencia VPS final commit `740e470`: servicio `esdata-hermes-monitor.service` `active (running)`, comando `/usr/bin/python3 /srv/esdata/scripts/hermes_monitor.py --daemon`, API health OK, domain availability OK (`empty=91`, `workflow=53`, `allowed=4`, `configured_unavailable=34`), `/status` lee `41` workers, `7` unhealthy observados, `restarted=0` por auto-restart deshabilitado, y DLQ fallback Docker operativo detecta 1 entrada real `eu_sanctions | sync_list:EU_SANCTIONS | retries=3/3 | HTTP Error 403: Forbidden`. Validacion local focal: `python -m pytest scripts\tests\test_maintenance_agents.py scripts\tests\test_deploy_hetzner.py::test_maintenance_agent_units_are_safe_by_default -q` => `27 passed`; Ruff focal OK; `py_compile scripts\hermes_monitor.py` OK; `verify-doc-contracts.py` OK; `git diff --check` OK. No se tocan datos productivos ni se reinicia ningun worker.
- 2026-05-27 - `[COMPLETADO VPS]` `HERMES-AEAT-JSON-NIGHTLY-SWITCH-01`: cambiado el nightly operativo AEAT de Hermes para usar JSON validado como fuente primaria y markdown solo como vista renderizada. Scripts versionados nuevos: `scripts/hermes_curator/bin/run-aeat-modelos-json.sh`, que ejecuta `run-aeat-model-json.sh` por cola y emite CSV de lote, y `scripts/hermes_curator/bin/run-writer-summary-json.sh`, que consume exclusivamente el CSV JSON mas reciente y marca el resumen legacy como `IGNORED`. Tests nuevos en `scripts/tests/test_aeat_hermes_nightly_scripts.py`. En VPS se hicieron backups de los wrappers antiguos en `/srv/esdata/hermes-curator/bin/run-aeat-modelos.sh.bak-20260527-071907` y `/srv/esdata/hermes-curator/bin/run-writer-summary.sh.bak-20260527-071907`; los wrappers operativos actuales delegan en los scripts JSON versionados del repo. Smoke controlado VPS con cola `210,128`: lote `/srv/esdata/hermes-curator/reports/aeat-campaign-curation-json/_batch-json-20260527-071917-summary.csv` con ambos modelos `OK`; resumen `/srv/esdata/hermes-curator/reports/daily-summary/summary-20260527-072155.md` en `mode: deterministic-json`, decision `NEEDS_HUMAN_REVIEW`, regla explicita `validated JSON is the primary artifact`, y legacy summary ignorado. Verificacion de contenido: `210` queda `CONFLICT` y `128` queda no afirmable; ambos con `campana_afirmable=null`, `campana_safe_to_assert=false`, `official_source_claims.proves_campaign=false`. No se escriben datos productivos, no se promueve ninguna campana, no se reinicia monitor y el timer no se cambia: la siguiente ejecucion nocturna usara los wrappers ya sustituidos.
- 2026-05-27 - `[COMPLETADO VPS]` `HERMES-AEAT-JSON-ADAPTER-01`: anadido adapter versionado para que Hermes no integre evidencia desde texto libre. Nuevos scripts: `scripts/hermes_curator/prompts/aeat_model_json.md`, `scripts/hermes_curator/bin/run-aeat-model-json.sh`, `scripts/maintenance/extract_aeat_hermes_json.py`, `scripts/maintenance/render_aeat_hermes_report.py` y `scripts/tests/test_aeat_hermes_adapter.py`. Flujo: Hermes debe emitir un bloque delimitado `BEGIN_AEAT_HERMES_JSON`/`END_AEAT_HERMES_JSON`; el extractor rechaza marcadores ausentes/multiples; el validador aplica `docs/aeat/hermes-curation-output.schema.json` + reglas Python; el markdown se renderiza solo desde JSON validado. Primer smoke VPS con modelo `210`: primer intento fallo por `python` ausente en VPS y se corrigio a `python3`; segundo intento produjo JSON estructuralmente cercano pero fallo por indice MCP inexistente, demostrando rechazo duro; tercer intento paso: JSON `/srv/esdata/hermes-curator/reports/aeat-campaign-curation-json/modelo-210-20260527-065744.json` validado, markdown renderizado desde JSON, decision `CONFLICT`, `campana_safe_to_assert=false`, `campana_afirmable=null`, `campana_assertion_code=NOT_ASSERTABLE_CONFLICT`, `official_source_claims.proves_campaign=false` en todos los claims y campanas 2019/2025/2026 solo como `rejected_claims`. Validacion local: `python -m pytest scripts\tests\test_aeat_hermes_adapter.py scripts\tests\test_validate_aeat_hermes_report.py -q` => `12 passed`; Ruff focal OK.
- 2026-05-27 - `[COMPLETADO LOCAL]` `HERMES-AEAT-STRUCTURED-EVIDENCE-CONTRACT-01`: definido contrato estructurado para que Hermes deje de producir evidencia integrable como markdown libre. Nuevos artefactos: `docs/aeat/hermes-curation-output.schema.json`, `docs/aeat/hermes-curation-output-contract.md`, `scripts/maintenance/validate_aeat_hermes_report.py` y `scripts/tests/test_validate_aeat_hermes_report.py`. Regla central: MCP/API es telemetria estructurada (`system_observed_claims`), no evidencia documental; `official_source_claims` solo puede existir si referencia una fuente oficial whitelisted con URL, localizador y excerpt, y no puede contener terminos internos como `MCP`, `metadata`, `cacheado`, `modelo_recurso` o `campana_activa`. `derived_claims` y `system_observed_claims` llevan `may_assert_campaign=false`; `decision=ASSERTABLE` exige el triple gate `campana_safe_to_assert=true`, `campana_afirmable!=null`, `campana_assertion_code=ASSERTABLE_DIRECT_OFFICIAL`; `human_review_required=true` siempre. Documentacion enlazada desde `docs/aeat/ai-agents.md` y `docs/aeat/precision-contract.md`. Validacion local: `python -m pytest scripts\tests\test_validate_aeat_hermes_report.py scripts\tests\test_mcp_campaign_assertion_contract.py -q` => `19 passed`; `python -m ruff check scripts\maintenance\validate_aeat_hermes_report.py scripts\tests\test_validate_aeat_hermes_report.py --select F,I` => OK; `verify-doc-contracts.py` y `verify-doc-artifacts.py --ci-baseline` => OK.
- 2026-05-26 - `[COMPLETADO VPS]` `FDSR-DORA-MICA-CLEANUP-01`: preparado y aplicado script idempotente con guardas para eliminar duplicados debiles DORA/MiCA que bloqueaban `mcp_validation_suite.py`. Diagnostico VPS read-only previo: `DORA_2022_2535` y `MICA_2023_1114` existian solo como `norma.codigo`; no tenian referencias en `obligacion_perfil.norma_codigo`, `obligacion_fuente.codigo_referencia`, `criterio_relacion.norma_codigo`, `norma.boe_id`, `norma.celex` ni `norma.norma_padre_celex`. Script: `scripts/data/seed_fdsr_dora_mica_cleanup_20260526.sql`, que exige canonicos `32022R2554` y `32023R1114`, aborta si hay referencias dependientes y solo borra filas `norma` debiles no referenciadas. Evidencia VPS: script aplicado via `psql` en contenedor `postgres`; verificacion SQL deja solo `32022R2554` y `32023R1114`; `mcp_validation_suite.py --read-only --base-url http://api:8000` => `ok=true`, con `dora_weak_duplicate_removed=0` y `mica_weak_duplicate_removed=0`; `mcp_deep_contract_audit.py --base-url http://api:8000` => `ok=true`.
- 2026-05-26 - `[COMPLETADO VPS]` `AEAT-TECHNICAL-COVERAGE-INACTIVE-RESOURCES-01`: corregido el gap detectado en la curacion del modelo 126 sin tocar datos productivos. Causa: el XLSX oficial de diseno de registro del 126 (`Ejercicios 2020 y siguientes`) existe en `modelo_recurso`, pero con `activa=false` porque el recurso activo del mismo tipo es la pagina HTML de disenos; la derivacion de `technical_exercise_coverage` solo miraba recursos activos y perdia esa evidencia tecnica. Cambio: `list_campaign_technical_coverage_recursos()` lee recursos inactivos `diseno_registro` con `metadata.label` que contenga `Ejercicios YYYY`, solo como input de seleccion de campana; no los anade a `fuentes_oficiales` ni a `artefactos`, no marca `proves_campaign=true` y no alimenta `campana_afirmable`. Ajuste adicional: si una etiqueta contiene anos de orden normativa y texto `Ejercicios YYYY`, la extraccion de anos de campana prioriza el ano de ejercicios y no usa el ano de la orden (ej. no contar `2007` en `Orden EHA/3435/2007 (Ejercicios 2020 y siguientes)`). Validacion local: `python -m pytest apps\api\tests\test_modelos_truth_contract.py scripts\tests\test_mcp_campaign_assertion_contract.py -q` => `46 passed, 2 warnings`; `python -m ruff check apps\api\services\modelos.py apps\api\tests\test_modelos_truth_contract.py --select F,I` => OK. Evidencia VPS tras rebuild/recreate de API: `/v1/modelos/126/fuentes-oficiales` y `/v1/modelos/126/resumen-operativo` devuelven `campana_resolution_status=conflict`, `campana_safe_to_assert=false`, `campana_afirmable=null`, `campana_assertion_code=NOT_ASSERTABLE_CONFLICT`, `campana_conflict_years=['2013','2020']` y `technical_exercise_coverage` con `from_year=2020`, `label='Ejercicios 2020 y siguientes'`, `proves_campaign=false`.

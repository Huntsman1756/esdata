...
- 2026-05-24 - `[EN CURSO LOCAL]` Sprint T valores AEAT `187/198`. Alcance: mantener `187/198` como modelos documentales completos, pero cerrar el hueco de obligaciones de perfil legacy que estaban `safe_to_answer=true` sin `source_hash`/`capture_date`; no crear obligaciones nuevas ni cerrar aplicabilidad universal por casillas/claves. Auditoria VPS: `187` tiene 50 casillas, 28 claves, 5 instrucciones y 0 reglas; `198` tiene 72 casillas, 46 claves, 7 instrucciones y 0 reglas; `obligacion_perfil` mostraba `187=3` y `198=4` filas seguras sin hash/captura. Cambios locales: revision `20260524_0093_aeat_valores_187_198_contract` degrada esas obligaciones a `partial`, `verified=false`, `safe_to_answer=false` y persiste reglas de alcance `iic_transmisiones_reembolsos_187`, `iic_obligacion_perfil_no_confirmada_187`, `activos_financieros_valores_mobiliarios_198` y `activos_financieros_obligacion_perfil_no_confirmada_198` solo si `modelo_recurso` oficial tiene `sha256_contenido` y `last_seen_at`; doc nuevo `docs/aeat-valores-187-198-contract.md`; `prd.json` actualizado. Validacion inicial: RED confirmado para migracion inexistente; GREEN focal `python -m pytest apps/api/tests/test_alembic_integrity.py::test_aeat_187_198_valores_contract_is_seeded_in_revision_0093 apps/api/tests/test_modelo_obligation_context.py::test_complete_obligation_context_fails_closed_without_hash_or_capture -q` => `2 passed`. Siguiente paso exacto: ejecutar suite focal completa, ruff, docs contracts, diff check; commitear, PR, aplicar Alembic en VPS y validar que no quedan obligaciones seguras 187/198 sin evidencia.

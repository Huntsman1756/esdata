...
- 2026-05-29 - `[EN CURSO]` `MCP-VALIDATION-SYSTEMD-OPS-01`: cerrar unidad fallida `esdata-mcp-validation.service`. Causa VPS confirmada: la unit ejecuta `/usr/bin/python3` del host y falla por dependencias ausentes (`pydantic`, `sqlalchemy.dialects:postgresql.psycopg`), mientras el contrato historico del repo indica ejecutar la suite desde contenedor `ops`. Alcance: cambiar la unit systemd a `docker compose ... run --rm --no-deps ops python /workspace/mcp_validation_suite.py --read-only --base-url http://api:8000`, desplegar, reconstruir `ops` si procede y validar la unit; no mutar datos ni relajar checks.

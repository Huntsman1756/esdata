...
- 2026-05-14 12:20 Europe/Madrid - `[EN CURSO LOCAL]` CNMV consolidation hardening: se cierra el gap detectado en circulares `vigente_modificado`, donde `documento_version` probaba version interna pero no texto BOE consolidado. Alcance: migracion Alembic aditiva para `es_consolidado` y estado de verificacion, exposicion en `/v1/cnmv/{referencia}/versions`, script operativo `scripts/maintenance/audit_cnmv_consolidated_versions.sh` y documentacion del contrato. Regla: ninguna circular modificada se tratara como consolidada salvo `es_consolidado=true` y `consolidated_verification_status='consolidated'`.

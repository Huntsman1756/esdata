...
- 2026-05-29 - `[EN CURSO]` `REG-WATCH-IVA-RATES-OPTIONAL-01`: diagnosticar y cerrar alerta Telegram `WorkerSyncErrors`/`WorkerCronErrors` para `cron-regulatory-daily`. Causa observada en VPS: ultima ejecucion planificada `2026-05-29 07:00 CEST` falla con `psycopg.errors.UndefinedTable: relation "iva_rates" does not exist` durante comparacion LIVA en `regulatory_watch.py`; `iva_rates` no existe en produccion y debe tratarse como tabla opcional, no como prerequisito runtime. Alcance: guard fail-closed/read-only para saltar esa comparacion si falta la tabla, test focal, despliegue VPS y smoke real del worker; no crear tabla en runtime ni escribir esquema ad hoc.

...
- 2026-05-29 - `[EN CURSO]` `HERMES-CURATOR-P0-RECOVERY-01`: cierre P0 operativo Hermes tras fallo nocturno. VPS: permisos corregidos en `aeat-campaign-curation-raw`, `aeat-campaign-curation-rendered` y `aeat-campaign-curation-json`; `hermescurator` puede escribir en los tres directorios y `safe.directory=/srv/esdata` queda configurado a nivel system git. Smoke manual `run-aeat-model-json.sh 124` como `hermescurator` genera raw, extrae JSON, valida schema y renderiza markdown. Ejecucion systemd completa ya no falla por permisos, pero detecta flakiness del agente: raw `modelo-124-20260529-073104.txt` contiene solo bootstrap de Hermes y no bloque `BEGIN_AEAT_HERMES_JSON`; `126` y `128` salen OK. Alcance actual: endurecer retry acotado ante `ERROR_EXTRACT` por salida incompleta, desplegar y repetir systemd smoke; no relajar validacion semantica ni promover campanas.

...
- 2026-05-24 - `[COMPLETADO LOCAL / PENDIENTE VPS]` `MCP-DATA-06`: reconciliacion del umbral de `sociedad_valores` sin forzar cobertura agregada. RED VPS commit `6cb1ca5`: `mcp_validation_suite.py --read-only --base-url http://api:8000` => `ok=false`, `checks=133`, `failures=2`, fallos `sociedad_valores_verified_ge_24` (`value=4`, `minimum=24`) y `all_profiles_pct_verified_ge_70` (`value=8`); `mcp_deep_contract_audit.py --base-url http://api:8000` => `ok=false`, `checks=12`, `failures=3`, con `eu_norm_contracts` fallando por `sociedad_valores_verified_count=4<20`. Inventario productivo: `sociedad_valores=38` obligaciones, `4` verificadas con `source_hash`/`capture_date`, `34` fail-closed explicitas con `verified=false`, `safe_to_answer=false`, `source_hash=NULL`, `capture_date` y nota `fail-closed`. Cambio local: `sociedad_valores_verified_or_fail_closed_ge_24` reemplaza el umbral historico `verified=true`; deep audit conserva `sociedad_valores_verified_count` como detalle y falla solo si `sociedad_valores_verified_or_fail_closed_count<20`. No se tocaron datos productivos, CASP, `emisor_token`, Modelo 303 ni `all_profiles_pct_verified_ge_70`. Validacion local: `pytest scripts/tests/test_maintenance_agents.py -q` => `24 passed`; `py_compile` OK; `verify-doc-contracts.py` OK; `verify-doc-artifacts.py --ci-baseline` OK.

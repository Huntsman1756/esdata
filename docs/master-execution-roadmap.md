...
- 2026-05-31 - `[COMPLETADO LOCAL / VPS PENDIENTE]` `CNMV-REGISTRY-REPO-REVIEW-02`: abierto por instruccion explicita del usuario para continuar CNMV revisando `Ansvar-Systems/spanish-financial-regulation-mcp` y `BquantFinance/cnmv`. Decision: Ansvar es util como patron operativo de paginacion del registro oficial de sanciones CNMV; Bquant es snapshot CSV sin scraper/procedencia por fila suficiente para ingestion autoritativa, por lo que no se ingiere. Cambios: `CNMV_SANCIONES_MAX_PAGES` sube de `1` a `25` en worker/Compose/env docs; el worker corta al detectar pagina oficial sin filas o respuesta no-200; `docs/github-public-scraper-review-20260531.md` y backlog documentan la decision. Smoke live local: `cnmv.py --discover-only --familia sanciones --max-urls 200` descubre `59` documentos y se detiene en `page=19` sin pedir `page=20`. Validacion local: tests focales CNMV/vocabulario `137 passed, 1 skipped`; tests nuevos de paginacion OK; Ruff focal OK; `py_compile` OK; `verify-doc-contracts.py` OK; `source_assurance_gate.py` OK; compose config OK; `git diff --check` OK. Siguiente paso: commit, deploy VPS y ejecutar `cron-cnmv-weekly` acotado a sanciones para confirmar conteos/API/gates.

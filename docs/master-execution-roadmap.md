...
- 2026-05-24 - `[EN CURSO LOCAL]` Sprint W recuperacion selectiva `obligacion_perfil` `111/115`. Auditoria VPS tras Sprint V: `111/115/196` tienen hashes candidatos en `modelo_recurso`, pero las URLs tienen varios hashes distintos (`111`=2, `115`=2, `196`=4), por lo que `modelo_recurso` no es base univoca; `111` y `115` si tienen una unica `source_revision` exacta por `source_entity_id` (`AEAT-MODELO-111`, `AEAT-MODELO-115`); `290` queda bloqueado porque la URL FATCA tiene dos hashes `source_revision` (`FATCA` y `FATCA_IGA_ES`). Cambio local: revision `20260524_0096_obligacion_perfil_recover_111_115` recupera solo `111/115` si `COUNT(DISTINCT content_hash_sha256)=1`, carga `source_hash`, conserva/usa `capture_date`, restaura `verified=true`, `completeness='completa'` y `safe_to_answer=true`; `196/290` permanecen fail-closed. Validacion inicial: RED confirmado para migracion inexistente; GREEN minimo `python -m pytest apps/api/tests/test_alembic_integrity.py::test_obligacion_perfil_111_115_recovery_uses_unique_source_revision_in_0096 -q` => `1 passed`. Siguiente paso exacto: suite focal, ruff, docs contracts/artifacts, `alembic heads`, PR, aplicar 0096 en VPS y validar `111/115 safe_true=6/6`, `196/290 safe_true=0`.

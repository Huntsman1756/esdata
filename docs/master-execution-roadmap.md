...
- 2026-06-01 - `[COMPLETADO LOCAL]` `AEAT-MODELO-190-CAMPAIGN-EVIDENCE-01`: cerrado en repo local el gap de campana operativa del modelo `190`, manteniendo separada la auditoria interna de claves/instrucciones/reglas. Diagnostico VPS previo: `190` estaba en `resolved_weak` porque solo exponia `campana_evidence_items` debil desde `aeat_instrucciones` y los 7 recursos activos no tenian `proves_campaign=true`; las 4 claves A-D siguen sin `source_url/source_hash/capture_date` y quedan fuera de este ciclo. Cambio local: migracion Alembic `20260601_0108_aeat_190_campaign_evidence` registra la pagina AEAT Sede `modelo-190.html` como `modelo_recurso:instrucciones` para `190/2025`, con hash `b154ea65d4a7679774842767de2e643f76c194e037237052647f43962073f205`, `content_length=30641`, `capture_date=2026-06-01`, label "Modelo 190. Ejercicio 2025. Gestiones activas en AEAT Sede.", `row_completeness='complete'` y `row_provenance='official_exact'`; no se toca `CAMPAIGN_BEARING_RESOURCE_TYPES`, no se promueven obligaciones, claves, instrucciones estructuradas ni reglas. `mcp_validation_suite.py` pasa de aceptar `190` debil a exigir `aeat_modelo_190_direct_instruction_asserts_campaign` con `resolved_strong` y fuente directa. Validacion local: TDD rojo->verde; `test_alembic_integrity.py` + `test_maintenance_agents.py` + seleccion focal de `test_modelos_truth_contract.py` => `66 passed`; `py_compile` OK; Ruff focal OK; `git diff --check` OK. Pendiente para cierre VPS: desplegar migracion/API/ops y verificar `/v1/modelos/190/resumen-operativo`, `mcp_validation_suite.py`, `mcp_deep_contract_audit.py`, `/health` y `systemctl --failed` en produccion.

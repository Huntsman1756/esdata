...
- 2026-05-27 - `[EN CURSO]` `HERMES-AEAT-ADJUDICATION-TRENDS-01`: anadir resumen operativo de tendencias sobre el historico persistido del adjudicador Hermes AEAT. Objetivo: convertir `repaired_excerpt_ratio`, `rewrite_ratio`, `blocking_error_ratio`, `unused_source_warning_ratio` y `assertable_candidate_ratio` en un dashboard JSON con deltas y alertas por umbral, sin leer informes modelo a modelo y sin escribir datos productivos. Archivos reclamados: `scripts/maintenance/summarize_aeat_adjudication_history.py`, `scripts/tests/test_summarize_aeat_adjudication_history.py`, `docs/aeat/hermes-curation-output-contract.md`, `docs/aeat/ai-agents.md`, `docs/master-execution-roadmap.md`.

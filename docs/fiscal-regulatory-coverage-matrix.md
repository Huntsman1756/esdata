...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| AEAT | Modelos fiscales | `implemented_partial` | Sede AEAT, recursos oficiales de modelos | 219 modelos y 31.685 casillas en baseline A-14; closeout prioritario: 6 `complete`, 9 `partial`, 8 `target`; familia capital mobiliario `123/124/193` con routing semantico documentado y reglas condicionales para `123/124`; familia valores `187/198` con reglas de alcance y obligaciones legacy fail-closed si falta hash/captura; `200/202/303` saneados para no mantener obligaciones legacy seguras sin evidencia; `289` tiene PRD documental separado y 0098 desplegada en VPS para HAP/GI42/XSD, pero auditoria 2026-05-24 lo mantiene parcial | si | si | Mixta: varios modelos con `verified` y fuentes oficiales; estado por modelo en `docs/aeat-priority-model-closeout.md`, `docs/aeat-capital-mobiliario-123-124-193-routing.md`, `docs/aeat-valores-187-198-contract.md`, `docs/aeat-fail-closed-200-202-303.md`, `docs/aeat-289-documental-closeout-prd.md` y `docs/aeat-289-documental-audit-2026-05-24.md` | si | AEAT no esta cerrado como bloque unico; quedan modelos parciales y target; `124` sigue parcial y condicionado a operacion especifica de activos financieros; `187/198` son completos como modelos, pero obligacion por perfil exige evidencia normalizada; `200/202/303` siguen parciales; `289` no pasa aun las 8 condiciones documentales porque la suite live falla en obligaciones de perfil no verificadas/sin hash | Mantener `289` en partial; no promocionar sin auditoria separada de perfil/supuesto o contrato CRS/DAC2 operativo |
...
| AEAT | Instrucciones, claves y reglas | `implemented_partial` | Sede AEAT, BOE, XSD/XLS/PDF oficiales | 179 claves y 70 instrucciones en baseline A-14; 187/193/198/216/290/296 completos en informe; 200/303 parciales; reglas de alcance nuevas para `187/198` sin promover obligacion de perfil | si | si | Mixta por modelo: `verified`, instrucciones, claves y reglas donde existen | si | Cobertura desigual entre modelos; falta contrato uniforme para todos los prioritarios y aplicabilidad por perfil sigue separada del cierre documental del modelo | Crear checklist por modelo: instrucciones, claves, reglas, casillas y fuente |
| AEAT | CDI y fiscalidad internacional | `implemented_partial` | AEAT, Hacienda CDI y BOE cuando exista referencia oficial | Worker `apps/workers/cdi.py`; evidencia historica VPS con 86 convenios en `irs_dta_convention`, pendiente de auditoria por pais | si | si | Parcial: fuentes AEAT/Hacienda localizadas, pero falta contrato uniforme `verified/completeness/source_url/capture_date` por pais, articulo y tipo de renta | parcial | CDI existe como superficie parcial, pero no como producto fiscal completo; falta cierre por convenio, articulo, protocolo y tipo de renta | Ejecutar `docs/cdi-coverage-prd.md` y no devolver tasas definitivas sin evidencia articulo/protocolo |
| AEAT | CRS/FATCA | `implemented_partial` | AEAT GI42/GI38, XSD/WSDL oficiales, IRS donde aplique | Modelos 289/290 con reglas e instrucciones parciales/completas segun modelo | si | si | Parcial: 290 fuerte; 289 con evidencia documental local reforzada en 0098, pero sin cierre operativo CRS/DAC2 | si | Falta expansion procedimental y fuente IRS explicita para familia FATCA; CRS/DAC2 no debe mejorar ni degradarse automaticamente por el estado documental del 289 | Ejecutar PRD `docs/crs-dac2-coverage-prd.md` como familia operativa separada con fuentes AEAT/BOE/OCDE/UE/IRS segun proceda |
| Doctrina administrativa | Lineas de criterio DGT/TEAC | `implemented_partial` | DGT/PETETE, Hacienda/TEAC DYCTEA | Auditoria/despliegue 2026-05-21, matriz operativa 2026-05-22 y curaciones D-02/D-04 2026-05-23: corpus existe; VPS expone D-01..D-09; `/v1/doctrina/lineas/coverage` devuelve `lineas_total=16`, `lineas_complete=3` con 0087 desplegada y validada | parcial | parcial | Parcial: contrato proyecta URL/hash/capture date desde `source_revision`; D-01 queda `complete`; D-02 queda acotada a adquisicion intracomunitaria de bienes (`LIVA art. 13`, modelo 349); D-03 queda parcial con `LIS art. 18` persistido y sin modelo 232; D-04 queda `complete` para CRS/FATCA acotado (`LGT art. vigésimo segunda`, modelo 289); las demas lineas no exponen articulo/modelo si el supuesto no esta probado | si | D-03 y D-05..D-09 siguen sin cierre completo; TEAC en D-01 queda soporte parcial, no cierre propio; D-02 no cubre entregas intracomunitarias ni IVA generico; D-04 no cubre procedimiento completo de reporte CRS/FATCA | Buscar evidencia modelo 232 para D-03 y usar `docs/doctrina-operational-coverage-matrix.md` como matriz viva |
| DGT | Consultas vinculantes | `implemented_partial` | DGT/PETETE | Auditoria 2026-05-21: 18.631 consultas, 18.621 `complete`, 10 `partial`, 18.631 con URL y hash en `source_revision` | si | si | Parcial: documento trazable, pero no toda consulta esta curada como linea utilizable | si | La completitud depende de limites de discovery; falta curacion por linea de criterio | Curar lote piloto por impuesto, articulo, modelo y tema |
| TEAC | DYCTEA/resoluciones | `implemented_partial` | Hacienda/TEAC DYCTEA | Auditoria 2026-05-21: 558 resoluciones, 290 `complete`, 268 `partial`, 558 con URL y hash en `source_revision` | si | si | Parcial: URLs oficiales y completitud por fila; pocas relaciones de articulo exactas | si | Falta curacion por linea de criterio y cobertura historica mas amplia | Curar TEAC solo como soporte cuando el texto y articulo sean suficientes |
...
| Screening/sanciones | UE sanctions | `implemented_loaded` | Comision Europea FSF XML via `data.europa.eu` downloadURL oficial | 5.996 entradas UE cargadas en VPS desde XML FSF 1.1 con `actualizada=2026-05-08`; cron semanal operativo tras corregir URL con token oficial | si | parcial | Si para UE cargado: fuente oficial DG FISMA/FSF y parser dedicado | si | No extrapolar UE a UN/SEPBLAC ni a screening PEP; mantener fuente separada | Mantener freshness/cadencia y vigilar cambios del `downloadURL` oficial |
...
1. Cierre AEAT prioritarios por modelo, incluido `docs/aeat-289-documental-closeout-prd.md`.
2. CDI y fiscalidad internacional: `docs/cdi-coverage-prd.md`.
3. Lineas de criterio DGT/TEAC: `docs/doctrina-coverage-prd.md`.
4. Fiscalidad territorial/foral.
5. Jurisprudencia tributaria minima.
6. DORA operativo.
7. PBC/FT por sujeto obligado.
8. CRS/DAC2 operativo: `docs/crs-dac2-coverage-prd.md`.

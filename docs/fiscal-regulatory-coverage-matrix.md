...
| AEAT | CDI y fiscalidad internacional | `implemented_partial` | AEAT, Hacienda CDI y BOE cuando exista referencia oficial | Worker `apps/workers/cdi.py`; evidencia historica VPS con 86 convenios en `irs_dta_convention` | si | si | Parcial: fuentes AEAT/Hacienda localizadas, pero falta contrato uniforme `verified/completeness/source_url/capture_date` por pais, articulo y tipo de renta | parcial | CDI existe como superficie parcial, pero no como producto fiscal completo; falta cierre por convenio, articulo, protocolo y tipo de renta | Ejecutar `docs/cdi-coverage-prd.md` y no devolver tasas definitivas sin evidencia articulo/protocolo |

...
| BOE diario / BORME / BOE datos abiertos | `www.boe.es` | `cron-boe-diario-daily`, `worker-borme`, `/v1/boe-diario`, `/v1/borme` | `partial` | `official_public_needs_reuse_review` | BORME extraction is heuristic/partial from official PDFs and summary API; related companies/person appointments are aids, not certified registry state. BOE diario is non-consolidated and must not imply vigente consolidated law. |
...
| BDNS | `www.infosubvenciones.es`, `pap.hacienda.gob.es` | `worker-bdns`, `/v1/bdns` | `partial_loaded` | `official_public_needs_reuse_review` | Structured official rows are loaded for configured endpoint families; no broad or exhaustive subsidy coverage claim. |

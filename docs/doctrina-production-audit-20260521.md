...
| D-02 IVA intracomunitario | `partial` | DGT `V0236-26`; TEAC `00/02766/2015/00/00` | DGT complete con URL/hash pero trata tipo impositivo y `LIVA art. 91`; no se usa como anclaje intracomunitario. TEAC ROI es partial y sin articulo trazable. El contrato de cierre exige supuesto intracomunitario expreso, `LIVA art. 25` si se cierra como entrega intracomunitaria, modelo 349 por relacion persistida y vigencia | Alto volumen DGT y utilidad fiscal | Buscar fuente principal que trate expresamente operacion intracomunitaria; no exponer 349 sin supuesto |
...
- D-02 no usa `V0236-26` como cierre porque el documento trata tipo impositivo (`LIVA art. 91`), no supuesto intracomunitario expreso; la API deja `articulo_referencia=null` y `modelo_aeat_referencia=null` salvo que exista relacion persistida completa con supuesto intracomunitario expreso, anclaje legal correcto, modelo 349, hash/captura y vigencia.
- D-03 queda como parcial fuerte: expone `LIS art. 18` desde `V0144-26` y lo persiste como relacion parcial, pero no declara modelo 232 ni `safe_to_answer=true`.
- D-04 conserva modelo 289 como evidencia parcial CRS/FATCA y lo persiste como relacion parcial, pero sin articulo/supuesto normalizado ni vigencia cerrada.
...
| D-02 IVA intracomunitario | `partial` | DGT `V0236-26` | TEAC `00/02766/2015/00/00` parcial | Bloqueado: `LIVA art. 91` no corresponde al supuesto intracomunitario | Bloqueado: `349` no esta ligado al supuesto | La fuente principal trata tipo impositivo, no operacion intracomunitaria expresa. El contrato solo permitiria cierre para entrega intracomunitaria si constan `LIVA art. 25`, relacion `modelo_supuesto` 349, hash/captura y vigencia | Nueva consulta/resolucion con supuesto intracomunitario expreso; si el supuesto no es entrega intracomunitaria, ajustar el anclaje antes de cerrar |
...
1. Curar D-02 IVA intracomunitario con fuente oficial real: si el supuesto es entrega intracomunitaria, cerrar solo con `LIVA art. 25`, modelo 349 por relacion `modelo_supuesto`, vigencia/historico, hash/captura y tests de abstencion.

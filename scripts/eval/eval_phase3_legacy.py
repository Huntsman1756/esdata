...
async def main(args: argparse.Namespace) -> None:
    ...
    if args.local:
        ...
        async with httpx.AsyncClient(
            ...
            # Ejecutar queries en paralelo con concurrencia controlada y retries
            # 70 queries x 6 endpoints = 420 llamadas API
            # Concurrency=3 con retry=2 para evitar sobrecarga
            CONCURRENCY = 3
            MAX_RETRIES = 2
            ...
            async def _request_with_retry(client, url, params, timeout=60.0):
                """HTTP request con retry exponencial."""
            ...
            async def _eval_with_semaphore(q, idx, total):
                async with semaphore:
                    ...
                    # Re-implementar evaluate_query con retry
                    pregunta = q["pregunta"]
                    criterios = q["criterios"]
                    
                    # consulta
                    consulta_resp = await _request_with_retry(client, "/v1/consulta", {"q": pregunta})
                    # buscar
                    buscar_params = {}
                    norma_filtro = criterios.get("norma_filtro")
                    if norma_filtro:
                        buscar_params["norma"] = norma_filtro
                    buscar_resp = await _request_with_retry(client, "/v1/legislacion/buscar", {"q": pregunta, **buscar_params})
                    # doctrina
                    doctrina_resp = await _request_with_retry(client, "/v1/doctrina/buscar", {"q": pregunta})
                    # hybrid
                    hybrid_resp = await _request_with_retry(client, "/v1/legislacion/buscar/hybrid", {"q": pregunta, "hybrid_weight": 0.3, **buscar_params})
                    # borme
                    borme_resp = await _request_with_retry(client, "/v1/borme", {"q": pregunta})
                    # bdns
                    bdns_resp = await _request_with_retry(client, "/v1/bdns", {"q": pregunta})
                    
                    # Calcular metricas inline
                    fuentes_buscar = _extraer_fuentes(buscar_resp)
                    fuentes_consulta = _extraer_fuentes(consulta_resp)
                    fuentes_encontradas = fuentes_buscar | fuentes_consulta
                    fuentes_esperadas = criterios.get("fuente_esperada", [])
                    ...
                    result = {
                        ...
                        "dominio": q["dominio"],
                        "endpoints": {
                            "consulta": {"status": "ok" if "_error" not in consulta_resp else "error", "data": consulta_resp},
                            "buscar": {"status": "ok" if "_error" not in buscar_resp else "error", "data": buscar_resp},
                            "doctrina": {"status": "ok" if "_error" not in doctrina_resp else "error", "data": doctrina_resp},
                            "hybrid": {"status": "ok" if "_error" not in hybrid_resp else "error", "data": hybrid_resp},
                            "borme": {"status": "ok" if "_error" not in borme_resp else "error", "data": borme_resp},
                            "bdns": {"status": "ok" if "_error" not in bdns_resp else "error", "data": bdns_resp},
                        },
                        "metricas": {
                            "score_compuesto": round(score, 3),
                            "acierto_fuente": acierto_fuente,
                            "fuentes_encontradas": sorted(fuentes_encontradas),
                            "fuentes_esperadas": fuentes_esperadas,
                            "acierto_articulo": acierto_articulo,
                        },
                        "falla": falla,
                    ...
                    status_icon = "[OK]" if not falla else "[FAIL]"
                    print(
                        f"    {status_icon} score={score:.3f}  "
                        f"fuente={'OK' if acierto_fuente else 'FAIL'}"
                    )
                    return result
            ...
            all_results = [
                {"query_id": queries[i]["id"], "falla": True, "endpoints": {}, "metricas": {"score_compuesto": 0, "acierto_fuente": False, "fuentes_encontradas": [], "fuentes_esperadas": []}, "_error": str(r)} if isinstance(r, Exception) else r
